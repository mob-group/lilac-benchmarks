...
#else
  call calc_A_spmv_helper(h_dim,numh,c,listh,listhptr,As,Asc)
...
subroutine calc_A_spmv_helper(h_dim,numh,c,listh,listhptr,As,Asc)
  implicit none
  ...
  integer, intent(in) :: listhptr(:) ! pointer to start of row in listh
  real(dp), intent(in) :: As(:)  ! operator matrix in AO basis (sparse)
  real(dp), intent(out) :: Asc(:,:) ! work matrix
...
end subroutine calc_A_spmv_helper
