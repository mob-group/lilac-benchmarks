...
c  hand wiring smaller memory for OPTIM please use these parameters

      INTEGER NMRES
      integer AMHmaxsiz
      parameter (AMHmaxsiz=80)

      integer maxmem
      parameter(maxmem=40)

      integer maxres
      parameter(maxres=294)

c     maxcnt is the maximum number of (i,j)
c     interactions

        integer maxcnt
        parameter(maxcnt=(AMHmaxsiz**2-AMHmaxsiz)/2)
        integer max_well
...
c     hbscl is the scale factor for hbonds
        ...
        double precision :: anti_NHB(20,20,2),
     *  anti_HB(20,20,2),
     *  para_HB(20,20,2),para_one(20),
     *  anti_one(20)
...
!      integer n_Q_anneal
     ...
     *           Q0_inc_a,Q0_inc_b,Qvalue_a,Qvalue_b
      integer n_Q_anneal_a,n_Q_anneal_b
       ...
       double precision del_r_a(1:AMHmaxsiz),
     *           Q_ij_a(0:n_divs_max,AMHmaxsiz),
     *          dQ_dr_ij_a(1:n_divs_max,1:AMHmaxsiz)
     ...
     *        numconst_b,i_ixn_Qbias_a(maxres,maxres)
       ...
       double precision del_r_b(1:AMHmaxsiz),
     *           Q_ij_b(0:n_divs_max,AMHmaxsiz),
     *          dQ_dr_ij_b(1:n_divs_max,1:AMHmaxsiz)
...
c some variables for non-additive contact potential
      ...
      integer class_of_res_2(AMHmaxsiz),sort_non_add(2,2,2,2,2),
     *   class_2(20)
...
c     if iexcld, then turn on excluded-volume penalty
      ...
      double precision exvmin_gamma(AMHmaxsiz,AMHmaxsiz)
      double precision c_of_m_dist(AMHmaxsiz)
...
c     flags for random interactions
      ...
      integer ilong(maxcnt,2,maxtab),numlng(0:AMHmaxsiz,maxtab),
     *        ires(maxres),
     *        sa(maxres),tarpre(maxres,maxtab),
     *        mempre(maxres,maxtab),jres(maxres),crdixn(maxtab,2)

      integer ixn_from_site(AMHmaxsiz,AMHmaxsiz,maxtab)
 
      double precision vpotnt(0:maxr+1,maxcnt,maxtab),amh_gse,
     *     rinc(maxcnt,maxtab),
...
c     i_Etim=T if want to output energies every i_Etim_step timesteps
     ...
     *     bondln(AMHmaxsiz,maxcrd),target(AMHmaxsiz,3,maxcrd),
     *     prcord(AMHmaxsiz,3,maxpro,maxcrd),
     *     zrcord(AMHmaxsiz,3,maxpro,maxcrd),
     *     avep(maxpro,2,50),
     *     quench_crd(AMHmaxsiz,3,maxpro,maxcrd,maxmov)
...
! variables for replica force
      ...
      double precision rep_phi_exp(AMHmaxsiz),rep_lambda(AMHmaxsiz),
     *   rep_cut_off,rep_tol
      integer n_rep_con(AMHmaxsiz),rep_con_2_res(AMHmaxsiz,AMHmaxsiz)
