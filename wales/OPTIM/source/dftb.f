...
C
      SUBROUTINE DFTB(N,XS,deriv1st,ENERGY,GTEST,XMULREAL,BOXLA1,BOXLA2,BOXLZ)
      USE KEY
      IMPLICIT NONE
      LOGICAL GTEST
      INTEGER I, J, K, AI, AJ, NDIM,NMAX, I2, J2, K2, LWORK, INFO, J1, N, NDIAG, NA1, NA2, J3, J4
      DOUBLE PRECISION DIST(N,N),XMULREAL,WORK(320*N),DPRAND,S(4*N,4*N),Sorig(4*N,4*N),
     &                 ENERGY,XS(3*N),HB(4*N,4*N),HA(4*N,4*N),VA(4*N,4*N),VB(4*N,4*N)
      ...
      DOUBLE PRECISION BOXLA1,BOXLA2,BOXLZ
      DOUBLE PRECISION A1VEC(3), A2VEC(3), DUMMY, DMIN, A1DOT, A2DOT
      INTEGER MX(N,N), MY(N,N), MZ(N,N)

      DOUBLE PRECISION Ssssig,Sspsig,Sppsig,Spppi,EPSsac,EPSsbc,EPSpac,EPSpbc,Hsssiga,Hspsiga,
     &        Hppsiga,Hpppia,Hsssigb,Hspsigb,Hppsigb,Hpppib,VECTOR(3)
...
C     COMMON /RDIST/ R, DIRCOS, DIAGA, DIAGB
      ...
      LWORK=320*N
      XMUL=INT(XMULREAL)
      A1VEC(1)=0.8660254037844388D0
      A1VEC(2)=0.5D0
      A1VEC(3)=0.0D0
      A2VEC(1)=0.8660254037844388D0
      A2VEC(2)=-0.5D0
      A2VEC(3)=0.0D0
...
C  
C  Assume graphene and hexagonal underlying lattice.
C
C |Nearest Whole     | int(a+.5) if a>=0|     1    | ANINT  | ANINT   | Real     | Real     |
C |  Number          | int(a-.5) if a<0 |          |        | DNINT   | Double   | Double   |
C
C For a of type real or double precision, there are two cases: if |a| < 1, int(a)=0; if |a| >= 1, 
C int(a) is the integer whose magnitude is the largest integer that does not exceed the magnitude of a 
C and whose sign is the same as the sign of a. For example, int(-3.7) = -3
C
C However, in this case the two lattice vectors are not orthogonal!
C
C
      IF (BULKT) THEN
!            IF ((.NOT.FIXIMAGE).AND.(.NOT.NORESET)) THEN
! !           PRINT*,'Points before:'
! !           WRITE(*,'(3F15.7)') (XS(J1),J1=1,3*N)
!             DO J1=1,N
!                J2=3*(J1-1)
!                A1DOT=XS(J2+1)*A1VEC(1)+XS(J2+2)*A1VEC(2)
!                NA1=DNINT(A1DOT/BOXLA1)
!                A2DOT=XS(J2+1)*A2VEC(1)+XS(J2+2)*A2VEC(2)
!                NA2=DNINT(A2DOT/BOXLA2)
!                IF ((NA1.NE.0).OR.(NA1.NE.0)) THEN
!                   PRINT '(A,I6,A,3G20.10,A,2I4)','atom ',J1,' before: ',XS(J2+1),XS(J2+2),XS(J2+3),' NA1,NA2=',NA1,NA2
!                   PRINT '(A,I6,A,3G20.10)','atom ',J1,'  after: ',XS(J2+1)- BOXLA1*A1VEC(1)*NA1 - BOXLA2*A2VEC(1)*NA2,
!      &                                                            XS(J2+2)- BOXLA1*A1VEC(2)*NA1 - BOXLA2*A2VEC(2)*NA2,XS(J2+3)
!                ENDIF
!                XS(J2+1)=XS(J2+1) - BOXLA1*A1VEC(1)*NA1 - BOXLA2*A2VEC(1)*NA2
!                XS(J2+2)=XS(J2+2) - BOXLA1*A1VEC(2)*NA1 - BOXLA2*A2VEC(2)*NA2
!             ENDDO
! !           PRINT*,'Points after:'
! !           WRITE(*,'(3F15.7)') (XS(J1),J1=1,3*N)
!            ENDIF
...
C
         DO I=1,N
            ...
            DO J=I+1,N
               ...
               DO J3=-1,1
                  DO J4=-1,1
                     DUMMY=(XS(J2+1)-XS(I2+1)+J3*A1VEC(1)*BOXLA1+J4*A2VEC(1)*BOXLA2)**2+
     &                     (XS(J2+2)-XS(I2+2)+J3*A1VEC(2)*BOXLA1+J4*A2VEC(2)*BOXLA2)**2+
     &                     (XS(J2+3)-XS(I2+3))**2
                     IF (DUMMY.LT.DMIN) THEN
                        ...
                        DMIN=DUMMY
                     ENDIF
!                    PRINT '(A,4I6,2G20.10)','I,J,J3,J4,DUMMY,DMIN=',I,J,J3,J4,DUMMY,DMIN
                  ENDDO
               ENDDO

               DIST(I,J)=DMIN
               R(I,J)=SQRT(DIST(I,J))
!                    PRINT '(A,4I6,3G20.10)','I,J,NA1,NA2,DUMMY,DMIN,R=',I,J,NA1,NA2,DMIN,R(I,J)
               R(J,I)=R(I,J)
               DO K=1,3
!                 DIRCOS(I,J,K)=VECTOR(K)/R(I,J)
                  DIRCOS(I,J,K)=(XS(J2+K)-XS(I2+K)+NA1*A1VEC(K)*BOXLA1+NA2*A2VEC(K)*BOXLA2)/R(I,J)
                  DIRCOS(J,I,K)=-DIRCOS(I,J,K)
               ENDDO
C              WRITE(*,'(2I4,3F15.7)') (I,J,(DIRCOS(I,J,K),K=1,3))
               IF (I.EQ.1) THEN ! use first atom as a reference
                  IF ((.NOT.FIXIMAGE).AND.(.NOT.NORESET)) THEN
                     XS(J2+1)=XS(J2+1)+NA1*A1VEC(1)*BOXLA1+NA2*A2VEC(1)*BOXLA2
                     XS(J2+2)=XS(J2+2)+NA1*A1VEC(2)*BOXLA1+NA2*A2VEC(2)*BOXLA2
                  ENDIF
               ENDIF
            ENDDO
            DO K=1,3
...
C************ END PERIODIC BOUNDARY CONDITIONS ****************************
      ...
      DO I=1,N
       ...
       R(I,I)=0.0D0
          DO J=I+1,N
            J2=3*(J-1)
            DIST(I,J)=(XS(I2+1)-XS(J2+1))**2+(XS(I2+2)-XS(J2+2))**2+(XS(I2+3)-XS(J2+3))**2
             R(I,J)=SQRT(DIST(I,J))
             R(J,I)=R(I,J)
...
C  numerical differentiation. Single-sided!
C
C**************************************************************************
C
      SUBROUTINE SECDFTB(N,X,VNEW,XMULREAL,BOXLX,BOXLY,BOXLZ)
      USE MODHESS
      IMPLICIT NONE
      INTEGER N, J1, J2
      DOUBLE PRECISION X(3*N), 
     1                 DIF, FG1(3*N),V1,VNEW(3*N),XMULREAL,BOXLX,BOXLY,BOXLZ

      DIF=1.0D-6

      DO J1=1,3*N
         X(J1)=X(J1)+DIF
         CALL DFTB(N,X,FG1,V1,.TRUE.,XMULREAL,BOXLX,BOXLY,BOXLZ)
         X(J1)=X(J1)-DIF
         DO J2=J1,3*N
