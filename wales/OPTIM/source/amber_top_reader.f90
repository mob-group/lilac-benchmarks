...
98    READ(MYUNIT2,'(A)',END=99) ENTRY
      CALL READ_LINE2(ENTRY,NWORDS,ENTRIES)      !get all words in line
      IF (ENTRIES(2).EQ.'POINTERS') THEN        !get number of bonds
         ...
         CALL READ_LINE2(ENTRY,NWORDS,ENTRIES)
...
99 CLOSE(MYUNIT2)
!  DO J6=1,NBOND
!     WRITE(*,'(A,I8,A,I8)') 'readtopology> Bond between',BONDS(J6,1),' and',BONDS(J6,2)
!  ENDDO

END SUBROUTINE
  
SUBROUTINE READ_LINE2(LINE,NWORDS,WORDSOUT)
