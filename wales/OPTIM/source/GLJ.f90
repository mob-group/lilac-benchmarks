!   OPTIM: A program for finding global minima
...
!  ===================================================================
!
SUBROUTINE GLJ(NATOMS,X,GRAD,POT,GTEST,SSTEST)
  !
  USE KEY, ONLY : GLJEPS,GLJSIG,VT,NGLJSPECIES
  USE COMMONS, ONLY : PARAM1
  !
  IMPLICIT NONE
  !
  LOGICAL, INTENT (IN) :: GTEST, SSTEST
  ...
  DOUBLE PRECISION ABBC, ABAC, ACBC, ABI, ACI, BCI, R2(NATOMS,NATOMS), RX, RY, RZ, &
     &                 VEC(NATOMS,NATOMS,3), TEMP, RR2(NATOMS,NATOMS), TDOT, RAB, RRAB, RAC, RRAC, RBC, RRBC, &
     &                 ABJ, ACJ, BCJ, P3
  ...
  LT1: DO T1=1,NATOMS
     ...
     ND2=1
542 CONTINUE
     IF (ND1.GT.NGLJSPECIES(NT1)) THEN
        NT1=NT1+1
        ND1=1
     ...
     LT2: DO T2=1,T1-1
543     CONTINUE
        IF (ND2.GT.NGLJSPECIES(NT2)) THEN
           NT2=NT2+1
           ND2=1
        ...
        DUMMY = EPS4*(IR12 - IR6)
        POT = POT + DUMMY
!       WRITE(*,'(A,2I6,3F15.4)') 'T1,T2,eps,sig,pot=',T1,T2,GLJEPS(NT2,NT1),GLJSIG(NT2,NT1),POT
...
!  Add an Axilrod-Teller three-body term.
     ...
     &                         15*(ABI*RRAB + ACI*RRAC)*TDOT   )
               ...
               ENDDO
            ENDDO
            GRAD(3*(J1-1)+J2)=GRAD(3*(J1-1)+J2)+PARAM1*TEMP
         ENDDO
      ENDDO
      ENDIF
