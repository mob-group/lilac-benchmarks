...
SUBROUTINE ISO_COULOMB(X, POTLIST, N_ATOM_PARTNERS, Q, POTSCALE, PARAMS, ENERGY, GRAD, GTEST, STEST)
    ...
    ! Various powers of the distance between the atoms, and the atom radius
    DOUBLE PRECISION :: R, R2(NATOMS,NATOMS), R3
        ...
        IF (STEST) THEN  ! Gradient + Hessian
            ...
            DO J6=1,UBOUND(POTLIST,1)
                ...
                DO J5=2,N_ATOM_PARTNERS(J6)+1  ! Have to add the +1 because the first element in POTLIST(J1,:)
                    ...
                    TMP_ENERGY=TMP_ENERGY+Q(J1)*Q(J2)*R
...
SUBROUTINE ISOTROPIC_HESSIAN(N, X, G, F, R2, TMP_HESS)
    IMPLICIT NONE
    INTEGER, INTENT(IN)           :: N  ! Number of atoms
    DOUBLE PRECISION, INTENT(IN)  :: X(3*N), G(N,N), F(N,N), R2(N,N) ! NB: R2 must be an inverse square distance!!!!!
                                                                     ! This is a consequence of the way I've defined F.
...
!  Case III, different atoms, same cartesian coordinate.
    ...
    DO J1=1,N
        DO J2=1,3
            J3=3*(J1-1)+J2
            DO J4=J1+1,N
               ! Is it ok that I'm not initialising to 0 and using the TMP_HESS construction here?
