...
!      WRITE (*,*) 'Incoming coordinates, presumed polar'
!      DO J1 = 1, 3*NATOMS
!        WRITE (*,*) X(J1)
!      END DO

! jwrm2> make sure the radii are set to zero
!      DO J1 = 3, 3*NATOMS, 3
!        X(J1) = 0.0D0
!      END DO
...
!      WRITE (*,*) 'Coordinates converted to Cartesians'
!      DO J1 = 1, 3*NATOMS
!        WRITE (*,*) TMPCOORDS(J1)
!     END DO
      ...
      DO J1 = 1, NATOMS-1
         ...
         DO J2 = J1+1, NATOMS
            ...
            ELSEIF (GTHOMPOT .EQ. 7) THEN
               IF ((J1.LE.GTHOMSONBINN).AND.(J2.GT.GTHOMSONBINN)) THEN
                  ETHOMSON = ETHOMSON + 1.0D0/SQRT(DIST**2+GTHOMSONBIND**2/NATOMS) ! softened
               ELSE
                  ETHOMSON = ETHOMSON + 1.0D0/DIST ! Coulomb
               ENDIF
            ENDIF
            ...
            IF (GTEST) THEN
               ...
               ELSEIF (GTHOMPOT .EQ. 6) THEN
                  V(3*J1-2) = V(3*J1-2) + 2.0D0 * (1.0D0 - EXP(GThomsonRho*(GThomsonSigma-DIST))) &
                        ...
                        * GThomsonRho * DOT_PRODUCT(DR,Gradient11) / DIST ! Morse
                  V(3*J1-1) = V(3*J1-1) + 2.0D0 * (1.0D0 - EXP(GThomsonRho*(GThomsonSigma-DIST))) &
                        * EXP(GThomsonRho*(GThomsonSigma-DIST)) &
                        * GThomsonRho * DOT_PRODUCT(DR,Gradient12) / DIST ! Morse
               ELSEIF (GTHOMPOT .EQ. 7) THEN
                  IF ((J1.LE.GTHOMSONBINN).AND.(J2.GT.GTHOMSONBINN)) THEN
                     V(3*J1-2) = V(3*J1-2) - 1.0D0/(DIST**2+GTHOMSONBIND**2/NATOMS)**1.5D0 * DOT_PRODUCT(DR,Gradient11)
                     V(3*J1-1) = V(3*J1-1) - 1.0D0/(DIST**2+GTHOMSONBIND**2/NATOMS)**1.5D0 * DOT_PRODUCT(DR,Gradient12)
                  ELSE
                     V(3*J1-2) = V(3*J1-2) - 1.0D0/DIST**3 * DOT_PRODUCT(DR,Gradient11)
                     V(3*J1-1) = V(3*J1-1) - 1.0D0/DIST**3 * DOT_PRODUCT(DR,Gradient12)           
                  ENDIF
               ...
               ELSEIF (GTHOMPOT .EQ. 4) THEN
                  ...
                  V(3*J2-1) = V(3*J2-1) + ((12.0D0*GThomsonSigma**12/DIST**14) - (6.0D0*GThomsonSigma**6/DIST**8))&
                       * DOT_PRODUCT(DR,Gradient22)
               ELSEIF (GTHOMPOT .EQ. 5) THEN
                  V(3*J2-2) = V(3*J2-2) + (12.0D0*GThomsonSigma**12/DIST**14)* &
                                DOT_PRODUCT(DR,Gradient21) ! Repulsive LJ
                  V(3*J2-1) = V(3*J2-1) + (12.0D0*GThomsonSigma**12/DIST**14)* &
                                DOT_PRODUCT(DR,Gradient22) ! Repulsive LJ
               ELSEIF (GTHOMPOT .EQ. 6) THEN
                  V(3*J2-2) = V(3*J2-2) - 2.0D0 * (1.0D0 - EXP(GThomsonRho*(GThomsonSigma-DIST))) &
                                * EXP(GThomsonRho*(GThomsonSigma-DIST)) &
                                * GThomsonRho * DOT_PRODUCT(DR,Gradient21) / DIST
                  V(3*J2-1) = V(3*J2-1) - 2.0D0 * (1.0D0 - EXP(GThomsonRho*(GThomsonSigma-DIST))) &
                                * EXP(GThomsonRho*(GThomsonSigma-DIST)) &
                                * GThomsonRho * DOT_PRODUCT(DR,Gradient22) / DIST
               ELSEIF (GTHOMPOT .EQ. 7) THEN
                  IF ((J1.LE.GTHOMSONBINN).AND.(J2.GT.GTHOMSONBINN)) THEN
                     V(3*J2-2) = V(3*J2-2) + 1.0D0/(DIST**2+GTHOMSONBIND**2/NATOMS)**1.5D0 * DOT_PRODUCT(DR,Gradient21)
                     V(3*J2-1) = V(3*J2-1) + 1.0D0/(DIST**2+GTHOMSONBIND**2/NATOMS)**1.5D0 * DOT_PRODUCT(DR,Gradient22)
                  ELSE
                     V(3*J2-2) = V(3*J2-2) + 1.0D0/DIST**3 * DOT_PRODUCT(DR,Gradient21)           
                     V(3*J2-1) = V(3*J2-1) + 1.0D0/DIST**3 * DOT_PRODUCT(DR,Gradient22)
                  ENDIF
...
!      IF (GTEST) THEN
!        WRITE (*,*) 'GTHOMSON> printing gradients'
!        DO J1 = 1, 3*NATOMS
!          WRITE (*,*) V(J1)
!        END DO
!      END IF
      ...
      DO J1 = 1, NATOMS
         ...
         ELSE IF ( GTHOMMET .EQ. 5 )  THEN
            ...
            IF ( COORDS(3*J1) < 0.0D0 ) THEN               
               DUMMY=-COORDS(3*J1)/RADIUS
               IF (DUMMY.GT.1.0D0) DUMMY=1.0D0
               IF (DUMMY.LT.-1.0D0) DUMMY=-1.0D0
               P(3*J1-1) = 2*HALFPI - ACOS(DUMMY)
            ELSE
...
!----------------------------------------------------------------------------------------------------------------------------------
! Calculates the Hessian matrix of second derivatives of the potential
!
! X: the coordinates
! HESS: the Hessian
    ...
    SUBROUTINE GTHOMSONHESSIAN(X,HESS)
      ...
      DO J1 = 1, NATOMS
         ...
         DO J2 = J1+1, NATOMS
            ...
            ELSEIF (GTHOMPOT .EQ. 7) THEN
               IF ((J1.LE.GTHOMSONBINN).AND.(J2.GT.GTHOMSONBINN)) THEN
                  D2VDR2 =   3/(DIST**2+GTHOMSONBIND**2/NATOMS)**2.5D0
                  DVDR   = - 1/(DIST**2+GTHOMSONBIND**2/NATOMS)**1.5D0
               ELSE
                  D2VDR2 =   3/DIST**5
                  DVDR   = - 1/DIST**3
               ENDIF
...
!----------------------------------------------------------------------------------------------------------------------------------
! Calculates a numerical approximation of the Hessian matrix of second derivatives of the potential
!
...
!  jwrm2> Adapted slightly to allow any number of atoms
!
    SUBROUTINE ORTHOGGTH(VEC1,Q,OTEST)
      USE KEY
      USE COMMONS
      IMPLICIT NONE
      INTEGER J1, J2, J3, JTEMP
      DOUBLE PRECISION Q(*), VEC1(*), DUMMY1, VECX(3*NATOMS), VECY(3*NATOMS), VECZ(3*NATOMS)
      DOUBLE PRECISION VECNATOMS(3*NATOMS,NATOMS)
      LOGICAL OTEST
      ...
      ELSE
         ...
         DO JTEMP = 1, NATOMS
            DUMMY1=0.0D0
            DO J2=1,3*NATOMS
               DUMMY1=DUMMY1+VEC1(J2)*VECNATOMS(J2,JTEMP)
            ENDDO
            DO J2=1,3*NATOMS
               VEC1(J2)=VEC1(J2)-DUMMY1*VECNATOMS(J2,JTEMP)
            ENDDO
            IF (OTEST) CALL VECNORM(VEC1,3*NATOMS)
         ENDDO
...
!  jwrm2> Adapted slightly to allow for any number of atoms
!
    SUBROUTINE SHIFTHGTH(Q,NATOMS)
      USE KEY
      USE MODHESS
      IMPLICIT NONE
      INTEGER J1, J2, JTEMP, NATOMS
      ...
      ELSE
         ...
         DO J1 = 1, 3*NATOMS
            DO J2 = 1, 3*NATOMS
               HESS(J2,J1) = HESS(J2,J1)+SHIFTL(1)*VECX(J2)*VECX(J1) + SHIFTL(2)*VECY(J2)*VECY(J1) + SHIFTL(3)*VECZ(J2)*VECZ(J1)
               DO JTEMP = 1, NATOMS
                  HESS(J2,J1) = HESS(J2,J1) + SHIFTL(1)*VECNATOMS(J2,JTEMP)*VECNATOMS(J1,JTEMP)
               ENDDO
            ENDDO
         ENDDO
...
USE KEY,ONLY : NPERMGROUP, NPERMSIZE, PERMGROUP, NSETS, SETS, STOCKT, GEOMDIFFTOL, &
  &            NFREEZE, NABT, RBAAT, ANGLEAXIS2, BESTPERM, LOCALPERMDIST, PULLT, EFIELDT, NTSITES, &
  &            RIGIDBODY, PERMDIST, OHCELLT, LPERMDIST, LOCALPERMCUT, LOCALPERMCUT2, &
...
IF (.NOT. PERMDIST) THEN
   CALL NEWMINDIST(DUMMYB,DUMMYA,NATOMS,DISTANCE,BULKT,TWOD,ZSYMSAVE,.FALSE.,RIGIDBODY,DEBUG,RMAT)
   DISTANCE = DISTANCE**2
...
IF (NFREEZE.LE.0) THEN
      ...
      IF ((NRANROT.GT.0).AND.(NROTDONE.LE.NRANROT).AND.(NROTDONE.GT.0)) THEN
         IF (DEBUG) PRINT '(A,I6,A,G20.10)',' hkminpermdist> Trying random starting orientation number ',NROTDONE, &
  &                                         ' minimum distance=',SQRT(DBEST)
      ...
      ENDDO
!      IF (DEBUG) PRINT *, ' hkminpermdist> after initial call to MYORIENT distance squared = ', DISTANCE, ' for ',NATOMS,' atoms'
!  IF (DEBUG) PRINT '(A,10I8)',' hkminpermdist> size of orbits, selected atoms, random rotations, invert: ', &
! &       NORBIT1,NORBIT2, NORBITB1,NORBITB2,NCHOOSE1,NCHOOSE2,NCHOOSEB1,NCHOOSEB2,NROTDONE,INVERT
ELSE
   NORBIT1=1; NORBIT2=1; NORBITB1=1; NORBITB2=1
   CALL NEWMINDIST(DUMMYB,DUMMYA,NATOMS,DISTANCE,BULKT,TWOD,'AX    ',.FALSE.,RIGID,DEBUG,RMAT)
   IF (DEBUG) PRINT '(A,G20.10)',' hkminpermdist> after initial call to NEWMINDIST distance=',DISTANCE
   DISTANCE=DISTANCE**2
ENDIF
...
!     IF (DEBUG) WRITE(*,'(A,I5,A,I5)') ' hkminpermdist> move position ',NEWPERM(J3),' to ',J3
      NPERM=NPERM+1
   ENDIF
...
!  IF (DEBUG) WRITE(*,'(A,G20.10)') ' hkminpermdist> distance after NEWMINDIST=                     ', &
! &                                    SQRT(DISTANCE) 
...
! hk286
  ...
  CALL HKMINPERMDIST(XCOORDSB,XCOORDSA,NGTHORI,DEBUG,BOXLX,BOXLY,BOXLZ,BULKT,.FALSE.,DD,DD2,RIGID,RMATBEST)
...
!  IF (DD < DISTANCE .AND. DD < GEOMDIFFTOL) THEN
  ...
  IF (GTHOMMET < 5) THEN
     ...
     ENDDO
     CALL HKMINPERMDIST(XCOORDSB,XCOORDSA,NGTHORI,DEBUG,BOXLX,BOXLY,BOXLZ,BULKT,.FALSE.,DD,DD2,RIGID,RMATBEST2)
     !  IF (DD < DISTANCE .AND. DD < GEOMDIFFTOL) THEN
     IF (DD < DISTANCE) THEN
        DISTANCE = DD
        DIST2 = DD2
