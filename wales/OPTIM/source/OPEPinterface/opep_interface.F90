...
!******************************************************************************
  ...
  CONTAINS
    ...
    LOGICAL :: END_PDB
    L=0
    WRITE(*,*) ' OPEP_get_atoms> parsing conf_initiale.pdb'
...
40  CONTINUE
    ...
    WRITE(*,'(A,I6)') ' OPEP_get_atoms> Number of atoms in pdb: ',L
  ...
  SUBROUTINE OPEP_INIT(NATOM_,COORDS,ATMASS,RNAT)
#ifdef __OPEP
    USE PORFUNCS
    USE COMMONS, ONLY: DEBUG
    USE KEY, ONLY: MYUNIT
    ...
    DOUBLE PRECISION, INTENT(OUT) :: COORDS(3*NATOM_), ATMASS(NATOM_)
#ifdef __OPEP
    !variables needed for creating everything for the output routine later on
    INTEGER :: PDB_FILE_UNIT, GETUNIT, L, I
    ...
    IF (DEBUG) WRITE(*,*) ' OPEP_init> call OPEP definitions and initialisation'
    ...
    DO I=1,NATOMS
       ATMASS(I) = mass(3*I)/2390.0 !return the mass in amu
    ENDDO
    IF (DEBUG) WRITE(*,*) ' OPEP_init> get all information needed for writing output'
    PDB_FILE_UNIT = GETUNIT()
    OPEN(UNIT=PDB_FILE_UNIT,FILE="conf_initiale.pdb",ERR=40,STATUS='OLD',ACTION='READ')  !open pdb  
30  CONTINUE
    READ(PDB_FILE_UNIT,'(A)',ERR=40) LINE  
    READ(LINE,*,ERR=40) WORD
    IF (WORD.EQ."ATOM") THEN
      !pdb needs to be correct format as specified below
      READ(LINE,*,ERR=40) WORD,L,ATOM_NAMES(L),RES_NAMES(L),RES_NUMS(L),DX,DY,DZ
