...
!         write(*,*) "Exiting after ", maxiter, "iterations."
  ...
  subroutine minimize_rmsd(nat,posref,pos,delr,rmsd,npart,calpha_in,atomic_type_in)
    ...
    double precision, dimension(3*nat), intent(in), target :: posref
    double precision, dimension(3*nat), intent(inout), target :: pos
    double precision, dimension(3*nat), target :: posa, posb, pospb
