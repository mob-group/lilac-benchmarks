...
module md_defs
  ...
  subroutine findpar(parkey, parval)
  
  character(*), intent(in) :: parkey
  character(*), intent(out) :: parval
  
  integer i
  character(200) :: s
  character(100) s2,s3,s4
  
  open(unit=50, file= "parametres.dat", status="old", action="read",position="rewind")
  
  parval=""
    write(*,*) ":",parkey,":"
  do
    read(50,*,iostat=i) s2,s3,s4
!    write(*,*) ":",s3,":"
!    write(*,*) ":",s4,":"
    if(i /= 0) then
      exit
    endif
    !if(s3 .eq. parkey) then
    i= index(s3, parkey)
    if(i .gt. 0) then
      parval=s4
    write(*,*) ":",parval,":"
