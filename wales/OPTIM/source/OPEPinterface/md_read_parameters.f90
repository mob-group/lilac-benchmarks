subroutine read_parameters_md()
    ...
    call findpar('n_equilibration',dummy)
    if (dummy .eq. '') then
    ...
    call findpar('n_rescale_v_equil',dummy)
    if (dummy .eq. '') then
    ...
    call findpar('simulation_time',dummy)
    if (dummy .eq. '') then
       
       call findpar('n_production',dummy2)
       if (dummy2 .eq. '') then
    ...
    call findpar('n_correct_rotation',dummy)
    if (dummy .eq. '') then
    ...
    call findpar('n_correct_center_of_mass',dummy)
    if (dummy .eq. '') then
       n_correct_com = 0
    else
    ...
    call findpar('n_statistics',dummy)
    if (dummy .eq. '') then
    ...
    call findpar('n_stats_average',dummy)
    if (dummy .eq. '') then
    ...
    if (thermostat.eq.'berendsen') then
       call findpar('berendsen_time',dummy)
       if (dummy .eq. '') then
    ...
    if (thermostat.eq.'langevin') then
       call findpar('friction_coef',dummy)
       if (dummy .eq. '') then
    ...
    call findpar('n_save_configs',dummy)
    if (dummy .eq. '') then
       n_save_configs = 1000
    else
    ...
    call findpar('n_save_restarts',dummy)
    if (dummy .eq. '') then
    ...
    ! unrotated, however
    call findpar('save_unrotated_structures',dummy)
    if (dummy .eq. '') then
    ...
    call findpar('n_steps_thermalization',dummy)
    if (dummy .eq. '') then
    ...
    call findpar('FILECOUNTER', dummy)
    if (dummy .eq. '') then
       COUNTER  = 'filecounter'
    else
       read(dummy,*) COUNTER
    ...
    call findpar('RESTART_FILE', dummy)
    if (dummy .eq. '') then
    ...
    call findpar('confining_sphere',dummy)
    if (dummy .eq. '') then
    ...
    if (confining_sphere) then
      call findpar('radius_confining_sphere',dummy)
      if (dummy .eq. '') then
      ...
      call findpar('confining_force_constant',dummy)
      if (dummy .eq. '') then
          k_confine = 1.0d0
      else
    ...
    call findpar('Control_hydrogen',dummy)
    if (dummy .eq. '') then 
    ...
    ! envs for rattle
    call findpar('RATTLE',dummy)
    if (dummy .eq. '') then
      rattle = .false.
    else 
      read(dummy, *) rattle
      if (rattle) then
        call findpar('RATTLE_Bond_Length_Tolerance', dummy)
        if (dummy .eq. '') then
          epspos = 1.0d-6
        else
          read(dummy, *) epspos
        end if
        call findpar('RATTLE_Velocity_Tolerance', dummy)
        if (dummy .eq. '') then
          epsvel = 1.0d-12
        else
          read(dummy, *) epsvel
        end if
        call findpar('MAX_NUM_RATTLE_ITER', dummy)
        if (dummy .eq. '') then
          nrattle = 200
        else
          read(dummy, *) nrattle
