...
! hk286
      USE KEY, ONLY : LPDGEOMDIFFTOL, LOCALPERMDIST, PULLT, EFIELDT, ORBITTOL, PERMDIST, LPERMDIST, 
     &                TWOD, GTHOMSONT, GTHOMMET
      IMPLICIT NONE
     ...
     1                 COST, SINT, RDOTN, TX, TY, TZ, IT(3,3), IV(3,3), PROJ, DMAX2, CUTOFF1
      ...
      CUTOFF1=ORBITTOL
      IF (LOCALPERMDIST.OR.LPERMDIST) CUTOFF1=LPDGEOMDIFFTOL
C
...
!
! hk286
      IF (PULLT.OR.EFIELDT.OR.TWOD.OR.(GTHOMSONT .AND. (GTHOMMET < 5))) THEN
...
C     PRINT '(A,3F20.10)',('LA ',Q1(3*(J1-1)+1),Q1(3*(J1-1)+2),Q1(3*(J1-1)+3),J1=1,NATOMS)
C     PRINT '(I6)',NATOMS
C     PRINT '(A)','myorient> reoriented coordinates:'
C     PRINT '(A,3F20.10)',('LA ',T1(3*(J1-1)+1),T1(3*(J1-1)+2),T1(3*(J1-1)+3),J1=1,NATOMS)
C     DO J1=1,NATOMS
C        DO J2=1,3
C           DUMMY=0.0D0
C           DO J3=1,3
