...
SUBROUTINE ASAOOSPOT (NATOMS, X, ENERGY, G, SIGMAP, GTEST, SECT)

  USE MODHESS

  IMPLICIT NONE
  
  INTEGER          :: NATOMS, J, J1, J2, J3, X1, X2
  DOUBLE PRECISION :: X(3*NATOMS), G(3*NATOMS)
  DOUBLE PRECISION :: ENERGY, R, RSQ, ARG, V, DV, D2V, TEMP1, TEMP2
  ...
  DOUBLE PRECISION :: SIGMAC, Q, ZP, BETA, SIGMAPSQ, SIGMACSQ, RANGESQ, ONEPLUSQCB, PREFACTOR, VMIN, PFATH, PFATHD, SIGMAP
...
!  IF (SECT.EQ..TRUE.) THEN
    !PRINT*,'second derivatives not available for asaoos'
    !PRINT*,'FORTRAN STOP'
    !STOP
!  END IF
    ...
    DO J2 = J1+1, NATOMS
      ...
      !core - linear
        ...
        IF (GTEST) THEN
          ...
          IF (SECT) THEN
            ...
            D2V = 0.D0
            DO X1 = 1,3
              TEMP1 = DV * (-(RIJ(X1)**2)/RSQ + 1.D0) + D2V * (RIJ(X1)**2 / RSQ)
              HESS((J1-1)*3+X1,(J1-1)*3+X1) = HESS((J1-1)*3+X1,(J1-1)*3+X1) + TEMP1
              HESS((J2-1)*3+X1,(J2-1)*3+X1) = HESS((J2-1)*3+X1,(J2-1)*3+X1) + TEMP1

              HESS((J1-1)*3+X1,(J2-1)*3+X1) = HESS((J1-1)*3+X1,(J2-1)*3+X1) - TEMP1
              HESS((J2-1)*3+X1,(J1-1)*3+X1) = HESS((J2-1)*3+X1,(J1-1)*3+X1) - TEMP1

              DO X2 = X1+1,3
                TEMP2 = DV * (-(RIJ(X1)*RIJ(X2))/RSQ) + D2V * (RIJ(X1)*RIJ(X2))/(RSQ)

                HESS((J1-1)*3+X1,(J1-1)*3+X2) = HESS((J1-1)*3+X1,(J1-1)*3+X2) + TEMP2
                HESS((J1-1)*3+X2,(J1-1)*3+X1) = HESS((J1-1)*3+X2,(J1-1)*3+X1) + TEMP2

                HESS((J2-1)*3+X1,(J2-1)*3+X2) = HESS((J2-1)*3+X1,(J2-1)*3+X2) + TEMP2
                HESS((J2-1)*3+X2,(J2-1)*3+X1) = HESS((J2-1)*3+X2,(J2-1)*3+X1) + TEMP2

                HESS((J1-1)*3+X1,(J2-1)*3+X2) = HESS((J1-1)*3+X1,(J2-1)*3+X2) - TEMP2
                HESS((J2-1)*3+X2,(J1-1)*3+X1) = HESS((J2-1)*3+X2,(J1-1)*3+X1) - TEMP2

                HESS((J2-1)*3+X1,(J1-1)*3+X2) = HESS((J2-1)*3+X1,(J1-1)*3+X2) - TEMP2
                HESS((J1-1)*3+X2,(J2-1)*3+X1) = HESS((J1-1)*3+X2,(J2-1)*3+X1) - TEMP2
              END DO
            END DO
          END IF
        END IF 
      ...
      !core - ArcTanh
        ...
        IF (GTEST) THEN
          ...
          IF (SECT) THEN
            ...
            D2V = 0.5D0 * 2.D0 * AT1**2 * ( 2.D0 / ( EXP(ARG)+EXP(-ARG) ) )**2 * ((EXP(ARG)-EXP(-ARG))/(EXP(ARG)+EXP(-ARG)))
            DO X1 = 1,3
              TEMP1 = DV * (-(RIJ(X1)**2)/RSQ + 1.D0) + D2V * (RIJ(X1)**2 / RSQ)
              HESS((J1-1)*3+X1,(J1-1)*3+X1) = HESS((J1-1)*3+X1,(J1-1)*3+X1) + TEMP1
              HESS((J2-1)*3+X1,(J2-1)*3+X1) = HESS((J2-1)*3+X1,(J2-1)*3+X1) + TEMP1
      ...
      !smooth merging
        ...
        IF (GTEST) THEN
          ...
          IF (SECT) THEN
            ...
            D2V = 2.D0*VS1A2 + 6.D0*VS1A3*R + 12.D0*VS1A4*RSQ
            DO X1 = 1,3
              TEMP1 = DV * (-(RIJ(X1)**2)/RSQ + 1.D0) + D2V * (RIJ(X1)**2 / RSQ)
              HESS((J1-1)*3+X1,(J1-1)*3+X1) = HESS((J1-1)*3+X1,(J1-1)*3+X1) + TEMP1
              HESS((J2-1)*3+X1,(J2-1)*3+X1) = HESS((J2-1)*3+X1,(J2-1)*3+X1) + TEMP1
      ...
      !smooth merging
        ...
        IF (GTEST) THEN
         ...
         IF (SECT) THEN
            ...
            D2V = 2.D0*VS2A2 + 6.D0*VS2A3*R + 12.D0*VS2A4*RSQ
            DO X1 = 1,3
              TEMP1 = DV * (-(RIJ(X1)**2)/RSQ + 1.D0) + D2V * (RIJ(X1)**2 / RSQ)
              HESS((J1-1)*3+X1,(J1-1)*3+X1) = HESS((J1-1)*3+X1,(J1-1)*3+X1) + TEMP1
              HESS((J2-1)*3+X1,(J2-1)*3+X1) = HESS((J2-1)*3+X1,(J2-1)*3+X1) + TEMP1
      ...
      !Asakura-Oosawa attraction
        ...
        IF (GTEST) THEN
          ...
          G(J2*3-2:J2*3) = G(J2*3-2:J2*3) - DV * RIJ(:)
         IF (SECT) THEN
            D2V = PREFACTOR * (6.D0 * R/(2.D0*ONEPLUSQCB*SIGMAC**3))
            DO X1 = 1,3
              TEMP1 = DV * (-(RIJ(X1)**2)/RSQ + 1.D0) + D2V * (RIJ(X1)**2 / RSQ)
              HESS((J1-1)*3+X1,(J1-1)*3+X1) = HESS((J1-1)*3+X1,(J1-1)*3+X1) + TEMP1
              HESS((J2-1)*3+X1,(J2-1)*3+X1) = HESS((J2-1)*3+X1,(J2-1)*3+X1) + TEMP1
