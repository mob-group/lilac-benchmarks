...
!         IFFT1D
!***********************************************************************
USE ALIGNUTILS, ONLY : PERMGROUP, NPERMSIZE, NPERMGROUP, NATOMS, BESTPERM, NSETS, SETS
USE FFTW3
...
CASE(1)
    WRITE(*,'(A)') "  algorithm estimates that the relative error in the sum of squares is at most TOL."
CASE(2)
    WRITE(*,'(A)') "  algorithm estimates that the relative error between X and the solution is at most TOL."
CASE(3)
    WRITE(*,'(A)') "  conditions for INFO = 1 and INFO = 2 both hold."
CASE(4)
    WRITE(*,'(A)') "  FVEC is orthogonal to the columns of the jacobian to machine precision."
CASE(5)
    WRITE(*,'(A)') "  number of calls to FCN with IFLAG = 1 has reached 100*(N+1)."
CASE(6)
    WRITE(*,'(A)') "  TOL is too small.  No further reduction in the sum of squares is possible."
CASE(7)
    WRITE(*,'(A)') "  TOL is too small.  No further improvement in the approximate solution X is possible. "
END SELECT
...
DO WHILE(NFOUND.EQ.0)
    DO N=1,NPEAKS
        ...
        IF(INFO.EQ.0.OR.INFO.EQ.5) THEN
            IF (DEBUG) THEN
                WRITE(*,'(A)') "fastoverlaputils> WARNING - FINDPEAK failed with error:"
        ...
        ELSE
            IF(INFO.EQ.4.OR.INFO.EQ.6.OR.INFO.EQ.7) THEN
                IF (DEBUG) THEN
                    WRITE(*,'(A)') "fastoverlaputils> WARNING - FINDPEAK ended with message"
                    CALL PRINTLMDERERROR(INFO)
                ENDIF
            ENDIF
! Find the location of the peak and subtract this peak from the copy of the data
    ...
    IF (NFOUND.EQ.0) THEN
        ...
        IF (WIDTH.GT.(MINVAL(FSHAPE)/2)) THEN
            WRITE(*,'(A)')  "ERROR fastoverlaputils-FINDPEAKS subroutine failed to find any peaks"
            STOP
        ENDIF
    ENDIF
ENDDO
...
INTEGER, INTENT(IN) :: NX, NY, NZ
COMPLEX(KIND=REAL64), INTENT(IN) :: IN(NX, NY, NZ)
COMPLEX(KIND=REAL64), INTENT(OUT) :: OUT(NX, NY, NZ)

! INCLUDE "fftw3.f90"
INTEGER(KIND=INT64) PLAN_FORWARD
...
! INCLUDE "fftw3.f90"
INTEGER(KIND=INT64) PLAN_BACKWARD
...
COMPLEX(KIND=REAL64), INTENT(IN) :: IN(N)
COMPLEX(KIND=REAL64), INTENT(OUT) :: OUT(N)
