...
! USE COMMONS, ONLY : PERMGROUP, NPERMSIZE, NPERMGROUP, BESTPERM, &
! & NSETS, SETS, OHCELLT, PERMDIST, PERMOPT, BOXLX, BOXLY, BOXLZ
USE ALIGNUTILS, ONLY : PERMGROUP, NPERMSIZE, NPERMGROUP, BESTPERM, &
 & NSETS, SETS, DEBUG, OHCELLT, NOINVERSION, BULK_BOXVEC, &
...
SUBROUTINE BNB_ALIGN(COORDSB,COORDSA,NCOORDS,DEBUGT,NBOXLX,NBOXLY,NBOXLZ,NBULKT, &
    & DISTANCE,DIST2,RMATBEST,NSTEPS)

IMPLICIT NONE

LOGICAL, INTENT(IN) :: NBULKT, DEBUGT
INTEGER, INTENT(IN) :: NCOORDS, NSTEPS
DOUBLE PRECISION, INTENT(INOUT) :: COORDSB(3*NCOORDS), COORDSA(3*NCOORDS)
...
IF ((UPPERBOUND).LE.(BESTUPPER + MPRTOL*BESTUPPER + ATOL)) THEN
    ...
    CALL ITERATIVEALIGN(SAVECOORDSB,DUMMYA,NATOMS,DEBUG,BULK_BOXVEC(1),BULK_BOXVEC(2),BULK_BOXVEC(3),BULKT, &
    & DIST2,UPPERBOUND,DUMMYRMAT,DUMMYDISP,PERMBEST)

    ! Resetting keywords
    NQUENCH = NQUENCH + 1

    IF(DEBUG) WRITE(*, "(A,G20.5)") &
...
IF (BULKT.AND.TWOD) THEN
    ...
    DO I=1,4
        ...
        IF( ((BULK_BOXVEC(1)/2-ABS(NEWVECT(1))+WIDTH*0.25D0).GT.0.D0).AND. &
          & ((BULK_BOXVEC(2)/2-ABS(NEWVECT(2))+WIDTH*0.25D0).GT.0.D0) ) CALL ADDNODE( &
...
ELSE
    DO I=1,8
        ...
        IF(BULKT) THEN
            ! Check if displacement is within lattice cell
            IF( ((BULK_BOXVEC(1)/2-ABS(NEWVECT(1))+WIDTH*0.25D0).GT.0.D0).AND. &
              & ((BULK_BOXVEC(2)/2-ABS(NEWVECT(2))+WIDTH*0.25D0).GT.0.D0).AND. &
              & ((BULK_BOXVEC(3)/2-ABS(NEWVECT(2))+WIDTH*0.25D0).GT.0.D0) ) CALL ADDNODE( &
              & NEWVECT,WIDTH*0.5D0,IDNUM,BESTUPPER,FORCE,LOWERBOUND,UPPERBOUND)
...
IF(DEBUG) THEN
    IF(BULKT) WRITE(*, "(A,3F16.5)") &
 & "gopermdist> testing displacement vector = ", VECTOR
    IF(.NOT.BULKT) WRITE(*, "(A,3F16.5)") &
            ...
            DO I=1,NPERM
                ...
                DO J=1,M
                    ...
                    DUMMYDISPS(1,K+J,J1) = DUMMYDISPS(1,K+J,J1) - &
                     & NINT(DUMMYDISPS(1,K+J,J1)/BULK_BOXVEC(1)) * BULK_BOXVEC(1)
                    DUMMYDISPS(2,K+J,J1) = DUMMYDISPS(2,K+J,J1) - &
                     & NINT(DUMMYDISPS(2,K+J,J1)/BULK_BOXVEC(2)) * BULK_BOXVEC(2)
                    DUMMYDISPS(3,K+J,J1) = DUMMYDISPS(3,K+J,J1) - &
                     & NINT(DUMMYDISPS(3,K+J,J1)/BULK_BOXVEC(3)) * BULK_BOXVEC(3)
...
IF (DEBUG.AND.((ESTUPPER.GT.UPPERBOUND).OR.(ESTLOWER.GT.LOWERBOUND))) THEN
    WRITE(*,"(A)") "gopermdist>************WARNING*********************"
    WRITE(*,"(A)") "EST UPPER GT UPPERBOUND OR EST LOWER GT LOWERBOUND"
    WRITE(*,"(A)") "gopermdist>************WARNING*********************"
    NBAD = NBAD + 1
ENDIF
...
SUBROUTINE FINDPERMVAL(PERM, NCOORDS, MATVALS, DINVIDX, MAXNEI, NPERMGROUPS, BEST)

IMPLICIT NONE
INTEGER, INTENT(IN) :: NCOORDS, MAXNEI, NPERMGROUPS
INTEGER, INTENT(IN) :: PERM(NCOORDS), DINVIDX(NCOORDS*NCOORDS,NPERMGROUPS)
DOUBLE PRECISION, INTENT(IN) :: MATVALS(NCOORDS*MAXNEI,NPERMGROUPS)
...
SUBROUTINE INVPAIRDISTIDX(DUMMYIDX, DINVIDX, NCOORDS, MAXNEI, NPERMGROUPS)

IMPLICIT NONE
INTEGER, INTENT(IN) :: NCOORDS, MAXNEI, NPERMGROUPS
INTEGER, INTENT(IN) :: DUMMYIDX(NCOORDS*MAXNEI,NPERMGROUPS)
INTEGER, INTENT(OUT) :: DINVIDX(NCOORDS*NCOORDS,NPERMGROUPS)
...
SUBROUTINE PERMNEARESTNEIGHBOURDISTS(NDISTS,NIDX,NCOORDS,MAXNEI,NEARI,NEARD,NPERMGROUPS)

IMPLICIT NONE
INTEGER, INTENT(IN) :: NCOORDS,MAXNEI,NPERMGROUPS,NIDX(MAXNEI*NCOORDS,NPERMGROUPS)
DOUBLE PRECISION, INTENT(IN) :: NDISTS(MAXNEI*NCOORDS,NPERMGROUPS)

INTEGER, INTENT(OUT) :: NEARI(NCOORDS)
DOUBLE PRECISION, INTENT(OUT) :: NEARD(NCOORDS)
...
IF(Q%N.GT.0) THEN
    ...
    IDNUM = RES%IDNUM
ELSE IF(DEBUG) THEN
    WRITE(*,"(A)") "gopermdist> warning, trying to read empty list"
ENDIF
...
SUBROUTINE INITIALISE(COORDSB,COORDSA,NCOORDS,NBOXLX,NBOXLY,NBOXLZ,NBULKT)
...
DOUBLE PRECISION, INTENT(IN) :: COORDSB(3*NCOORDS), COORDSA(3*NCOORDS), &
...
NUMSTRUCTS = 1
IF ((.NOT.NOINVERSION)) NUMSTRUCTS = 2
...
USE KEY, ONLY : PERMGROUP, NPERMSIZE, NPERMGROUP
IMPLICIT NONE
