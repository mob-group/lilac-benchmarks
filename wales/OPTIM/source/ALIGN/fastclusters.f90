...
! Subroutines:
!
!    FOM_ALIGN_CLUSTERS(COORDSB, COORDSA, NATOMS, DEBUG, L, KWIDTH, DISTANCE, DIST2, RMATBEST, NROTATIONS)
...
USE ALIGNUTILS, ONLY : PERMGROUP, NPERMSIZE, NPERMGROUP, NATOMS, BESTPERM, &
 & SAVECOORDS, NSTORED, NOINVERSION
USE FASTOVERLAPUTILS, ONLY : DUMMYA, DUMMYB, XBESTA, XBESTASAVE
USE PREC, ONLY: INT64, REAL64

LOGICAL, SAVE :: NOINVERSIONSAVE

DOUBLE PRECISION, PARAMETER :: PI = 3.141592653589793D0

CONTAINS

SUBROUTINE FOM_ALIGN_CLUSTERS(COORDSB, COORDSA, NCOORDS, DEBUG, L, KWIDTH, DISTANCE, DIST2, RMATBEST, NROTATIONS)
...
COMPLEX(KIND=REAL64) PIMML(-L:L,-L:L,0:L)
COMPLEX(KIND=REAL64) IMML(-L:L,-L:L,0:L), YMLA(-L:L,0:L,NCOORDS), YMLB(-L:L,0:L,NCOORDS)

DOUBLE PRECISION SAVEA(3*NCOORDS),SAVEB(3*NCOORDS),COMA(3),COMB(3)
...
INTEGER SAVEPERM(NCOORDS), KEEPPERM(NCOORDS)
...
IF (.NOT.NOINVERSION) THEN
    IF (DEBUG) WRITE(*,'(A)') 'fastoverlap> inverting geometry for comparison with target'
    ...
    ENDIF
ELSE
    IF (DEBUG) WRITE(*,'(A)') 'fastoverlap> not inverting geometry for comparison with target'
    COORDSA(1:3*NATOMS) = SAVEA(1:3*NATOMS)
    DISTANCE = DISTSAVE
    DIST2 = DIST2SAVE
    RMATBEST = RMATSAVE
...
IF (DEBUG) THEN
    WRITE(*,'(A,G20.10)') 'fastoverlap> overall best distance=', distance
    WRITE(*,'(A)') 'fastoverlap> overall best rotation matrix:'
    WRITE(*, '(3F20.10)') RMATBEST(1:3,1:3)
ENDIF

END SUBROUTINE FOM_ALIGN_CLUSTERS

SUBROUTINE ALIGNHARM(COORDSB, COORDSA, NCOORDS, DEBUG, N, L, HWIDTH, KWIDTH, DISTANCE, DIST2, RMATBEST, NROTATIONS)
!  COORDSA becomes the optimal alignment of the optimal permutation(-inversion)
!  isomer. DISTANCE is the residual square distance for the best alignment with
!  respect to permutation(-inversion)s as well as orientation and centre of mass.
!  COORDSA and COORDSB are both centred on the ORIGIN
...
COMPLEX(KIND=REAL64) PIMML(-L:L,-L:L,0:L)
COMPLEX(KIND=REAL64) IMML(-L:L,-L:L,0:L), YMLA(-L:L,0:L,NCOORDS), YMLB(-L:L,0:L,NCOORDS)
COMPLEX(KIND=REAL64) COEFFSA(0:N,-L:L,0:L,NPERMGROUP), COEFFSB(0:N,-L:L,0:L,NPERMGROUP)

DOUBLE PRECISION SAVEA(3*NCOORDS),SAVEB(3*NCOORDS)
DOUBLE PRECISION ANGLES(NROTATIONS,3), DISTSAVE, RMATSAVE(3,3), WORSTRAD, DIST2SAVE
INTEGER J,J1,J2,M1,M2,IND2,NROT,NDUMMY,INVERT,PATOMS
INTEGER SAVEPERM(NCOORDS), KEEPPERM(NCOORDS)
...
SUBROUTINE ALIGNCOEFFS(COORDSB,COORDSA,NCOORDS,IMML,L,DEBUG,DISTANCE,DIST2,RMATBEST,NROTATIONS,ANGLES)
...
DOUBLE PRECISION, INTENT(OUT) :: DISTANCE, DIST2, RMATBEST(3,3)
COMPLEX(KIND=REAL64), INTENT(IN) :: IMML(-L:L,-L:L,0:L)

COMPLEX(KIND=REAL64) ILMM(0:L,0:2*L,0:2*L)
...
IF (DEBUG) WRITE(*,'(A,I3,A)') 'fastoverlap> found ', NROTATIONS, ' candidate rotations'
...
DO J=1,NROTATIONS
    ...
    IF (DEBUG) THEN
        WRITE(*,'(A,I3,A)') 'fastoverlap> testing rotation', J, ' with Euler angles:'
        WRITE(*, '(3F20.10)') ANGLES(J,:)
        WRITE(*,'(A)') 'fastoverlap> testing rotation matrix:'
        WRITE(*, '(3F20.10)') RMATSAVE(1:3,1:3)
    ENDIF
	...
	IF (DISTANCE.LT.BESTDIST) THEN
        ...
        IF (DEBUG) THEN
            WRITE(*,'(A,G20.10)') 'fastoverlap> new best alignment distance=', BESTDIST
            WRITE(*,'(A)') 'fastoverlap> new best rotation matrix:'
            WRITE(*, '(3F20.10)') RMATBEST(1:3,1:3)
        END IF

    ELSE IF (DEBUG) THEN
        WRITE(*,'(A,G20.10)') 'fastoverlap> best aligment distance found=', BESTDIST
        WRITE(*,'(A)') 'fastoverlap> best rotation matrix found:'
        WRITE(*, '(3F20.10)') RMATBEST(1:3,1:3)
    ENDIF
ENDDO
...
DO M=-L,L
    EXPIM(M) = EXP(CMPLX(0.D0, M*PHI, REAL64))
ENDDO
...
SUBROUTINE HARMONICCOEFFS(COORDS, NCOORDS, CNML, N, L, HWIDTH, KWIDTH)
...
INTEGER, INTENT(IN) :: NCOORDS, N, L
DOUBLE PRECISION, INTENT(IN) :: COORDS(3*NCOORDS), HWIDTH, KWIDTH
COMPLEX(KIND=REAL64), INTENT(OUT) :: CNML(0:N,-L:L,0:L)

COMPLEX(KIND=REAL64) :: YML(-L:L,0:L)
DOUBLE PRECISION HARMCOEFFS(0:2*N+L,0:N,0:L), DNL(0:N,0:L+2*N), RJ
INTEGER I,J,K,SI,M,INDM, S

CNML = CMPLX(0.D0,0.D0,REAL64)
...
SUBROUTINE HARMONICCOEFFSPERM(COORDS, NCOORDS, CNML, N, L, HWIDTH, KWIDTH, NPERMGROUPS)
...
INTEGER, INTENT(IN) :: NCOORDS, N, L, NPERMGROUPS
DOUBLE PRECISION, INTENT(IN) :: COORDS(3*NCOORDS), HWIDTH, KWIDTH
COMPLEX(KIND=REAL64), INTENT(OUT) :: CNML(0:N,-L:L,0:L,1:NPERMGROUPS)

DOUBLE PRECISION DUMMY(3*NCOORDS)
INTEGER J1, J2, IND2, NDUMMY, PATOMS

IF (NPERMGROUP.NE.NPERMGROUPS) THEN
    WRITE(*,'(A)') 'ERROR - number of permutation arrays inconsistent, stopping'
    STOP
ENDIF
...
SUBROUTINE HARMONICCOEFFSMULTI(COORDSLIST,NCOORDS,NLIST,CNMLLIST,N,L,HWIDTH,KWIDTH,NPERMGROUPS)

IMPLICIT NONE

INTEGER, INTENT(IN) :: NCOORDS, NLIST, N, L, NPERMGROUPS
DOUBLE PRECISION, INTENT(IN) :: COORDSLIST(3*NCOORDS, NLIST), HWIDTH, KWIDTH
COMPLEX(KIND=REAL64), INTENT(OUT) :: CNMLLIST(0:N,-L:L,0:L,1:NPERMGROUPS, NLIST)

INTEGER I

!write(*,*) NCOORDS, NLIST, N, L, NPERMGROUPS
...
DO I=1,NLIST
    CALL HARMONICCOEFFSPERM(COORDSLIST(:,I),NCOORDS,CNMLLIST(:,:,:,:,I),N,L,HWIDTH,KWIDTH,NPERMGROUP)
ENDDO
...
INTEGER, INTENT(IN) :: N, L
COMPLEX(KIND=REAL64), INTENT(IN) :: C1NML(0:N,-L:L,0:L), C2NML(0:N,-L:L,0:L)
COMPLEX(KIND=REAL64), INTENT(OUT) :: IMML(-L:L,-L:L,0:L)
...
SUBROUTINE DOTHARMONICCOEFFSPERM(C1NML, C2NML, N, L, IMML, NPERMGROUPS)

IMPLICIT NONE

INTEGER, INTENT(IN) :: N, L, NPERMGROUPS
COMPLEX(KIND=REAL64), INTENT(IN) :: C1NML(0:N,-L:L,0:L,NPERMGROUPS), C2NML(0:N,-L:L,0:L,NPERMGROUPS)
COMPLEX(KIND=REAL64), INTENT(OUT) :: IMML(-L:L,-L:L,0:L)
...
SUBROUTINE CALCSIMILARITY(C1NML, C2NML, N, L, NPERMGROUPS, NORM, MAXOVER)
...
COMPLEX(KIND=REAL64) IMML(-L:L,-L:L,0:L), ILMM(0:L,0:2*L,0:2*L)
DOUBLE PRECISION OVERLAP(2*L+2,2*L+2,2*L+2)
...
SUBROUTINE CALCSIMILARITIES(C1NMLLIST,N1LIST,C2NMLLIST,N2LIST,N,L,NPERMGROUPS,NORMS,MAXOVERS,SYM)

IMPLICIT NONE
INTEGER, INTENT(IN) :: N1LIST, N2LIST, N, L, NPERMGROUPS
COMPLEX(KIND=REAL64), INTENT(IN) :: C1NMLLIST(0:N,-L:L,0:L,NPERMGROUPS,N1LIST), &
    & C2NMLLIST(0:N,-L:L,0:L,NPERMGROUPS,N2LIST)
...
SUBROUTINE CALCOVERLAPMATRICES(COORDSLIST,NCOORDS,NLIST,N,L,HWIDTH,KWIDTH,NORMS,MAXOVERS)

IMPLICIT NONE

INTEGER, INTENT(IN) :: NCOORDS, NLIST, N, L
DOUBLE PRECISION, INTENT(IN) :: COORDSLIST(3*NCOORDS, NLIST), HWIDTH, KWIDTH
DOUBLE PRECISION, INTENT(OUT) :: NORMS(NLIST,NLIST), MAXOVERS(NLIST,NLIST)

COMPLEX(KIND=REAL64) CNMLLIST(0:N,-L:L,0:L,1:NPERMGROUP, NLIST)

CALL HARMONICCOEFFSMULTI(COORDSLIST,NCOORDS,NLIST,CNMLLIST,N,L,HWIDTH,KWIDTH,NPERMGROUP)
CALL CALCSIMILARITIES(CNMLLIST,NLIST,CNMLLIST,NLIST,N,L,NPERMGROUP,NORMS,MAXOVERS,.TRUE.)

END SUBROUTINE CALCOVERLAPMATRICES

SUBROUTINE FOURIERCOEFFS(COORDSB, COORDSA, NCOORDS, L, KWIDTH, IMML, YMLB, YMLA)
!
...
IMPLICIT NONE
INTEGER, INTENT(IN) :: NCOORDS, L
DOUBLE PRECISION, INTENT(IN) :: COORDSA(3*NCOORDS), COORDSB(3*NCOORDS), KWIDTH
COMPLEX(KIND=REAL64), INTENT(OUT) :: IMML(-L:L,-L:L,0:L)

COMPLEX(KIND=REAL64), INTENT(OUT) ::  YMLA(-L:L,0:L,NCOORDS), YMLB(-L:L,0:L,NCOORDS)
DOUBLE PRECISION RA(NCOORDS), RB(NCOORDS), IL(0:L), R1R2, EXPRA(NCOORDS), EXPRB(NCOORDS), FACT, TMP

INTEGER IA,IB,I,J,K,M1,M2,INDM1,INDM2

YMLA = CMPLX(0.D0,0.D0,REAL64)
YMLB = CMPLX(0.D0,0.D0,REAL64)
...
COMPLEX(KIND=REAL64), INTENT(OUT) :: ILMM(0:L,0:2*L,0:2*L)
COMPLEX(KIND=REAL64) FROT(2*L+2,2*L+2,2*L+2)
INTEGER I,J,M1,M2, NJ
INTEGER(KIND=INT64) BW

! Convert array into format usable by DSOFT:
BW = INT(L+1,8)
NJ = 2*L + 1

ILMM = CMPLX(0.D0, 0.D0, REAL64)
DO J=0,L
...
SUBROUTINE CHOOSE_KWIDTH(NCOORDS, COORDSA, COORDSB, KWIDTH, DEBUG)
...
DO J1 = 1, NCOORDS  ! Find the nearest-neighbour distance of atom J1
   MIN_DIST = 1.0D10
   DO J2 = 1, NCOORDS  ! Check all the neighbours of J1
      IF (J1.EQ.J2) CYCLE
...
KWIDTH = SUM_MINDISTS/(3*2*NCOORDS) ! 2*NCOORDS is the number of pairs over which we have averaged.
...
USE KEY, ONLY : NFREEZE,GEOMDIFFTOL,ORBITTOL,FREEZE,PULLT,TWOD,  &
    &   EFIELDT,AMBERT,QCIAMBERT,AMBER12T,STOCKT,PERMDIST,      &
    &   LOCALPERMDIST,LPERMDIST,OHCELLT,QCIPERMCHECK,    &
    &   NOINVERSION,GTHOMSONT,MKTRAPT,RIGIDBODY,OHCELLT

IMPLICIT NONE

IF ((.NOT.ALLOCATED(PERMGROUP)).OR.(.NOT.ALLOCATED(NPERMSIZE))) THEN
    WRITE(*,'(A)') 'ERROR - permutation arrays not set, use PERMDIST keyword'
    STOP
ENDIF
...
IF(RIGIDBODY) THEN
    WRITE(*,'(A)') 'ERROR - fastoverlap not compatible with RIGIDBODY keyword'
    STOP
ENDIF
