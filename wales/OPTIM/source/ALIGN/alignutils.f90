...
USE KEY, ONLY : PERMGROUP, NPERMSIZE, NPERMGROUP, BESTPERM, &
 & NSETS, SETS, NOINVERSION, BULK_BOXVEC, OHCELLT, TWOD!, PERMDIST, PERMOPT
...
!DOUBLE PRECISION, SAVE :: BOXLX=BULK_BOXVEC(1), BOXLY=BULK_BOXVEC(2), BOXLZ=BULK_BOXVEC(3)
...
DO WHILE(NPERM.GT.0)
    ...
    IF (DEBUG) WRITE(*,'(A,I2)') 'alignutils> beginning iteration ', NTRIES+1
    ...
    IF (((NEWDIST2-PDIST2)/NEWDIST2).GT.(SQRT(1.D0*NCOORDS)/PSCALE)) THEN
        IF (DEBUG) WRITE(*, '(A)') 'alignutils> WARNING - distance increased with nonzero permutations'
        EXIT
    ENDIF
    IF (NTRIES.GT.MAXIMUMTRIES) THEN
        IF (DEBUG) WRITE(*, '(A)') 'alignutils> WARNING - number of tries exceeded'
        EXIT
    ENDIF
ENDDO
...
IF (DEBUG) THEN
    WRITE(*, '(A,G20.10,A,I2,A)') 'alignutils> best distance found=', NEWDIST2, ' after ', NTRIES, ' iterations'
    IF (BULKT) THEN
        WRITE(*, '(A)') 'alignutils> best displacement found:'
        WRITE(*, '(3G20.10)') DISPBEST(1:3)
    ELSE
        WRITE(*, '(A)') 'alignutils> best rotation found:'
        WRITE(*, '(3G20.10)') RMATBEST(1:3,1:3)
    ENDIF
ENDIF
...
IF (INFO.NE.0) WRITE(*,'(A,I6,A)') 'alignutils> FINDROTATION WARNING - INFO=',INFO,' in DSYEV'
...
IF (MINV.LT.0.0D0) THEN
    IF (ABS(MINV).LT.1.0D-6) THEN
        MINV=0.0D0
    ELSE
        WRITE(*,'(A,G20.10,A)') 'alignutils> FINDROTATION WARNING MINV is ',MINV,' change to absolute value'
        MINV=-MINV
    ENDIF
ENDIF
DIST=SQRT(MINV)

!IF (DEBUG) WRITE(*,'(A,G20.10,A,I6)') 'alignutils> minimum residual is ',DIAG(JMIN),' for eigenvector ',JMIN
...
DO J1=1,NCOORDS
    ...
    DISP(1) = DISP(1) + XM - BULK_BOXVEC(1)*NINT(XM/BULK_BOXVEC(1))
    DISP(2) = DISP(2) + YM - BULK_BOXVEC(2)*NINT(YM/BULK_BOXVEC(2))
ENDDO
...
ELSE
    DO J1=1,NCOORDS
        ...
        DISP(3) = DISP(3) + ZM - BULK_BOXVEC(3)*NINT(ZM/BULK_BOXVEC(3))
    ENDDO
END IF
...
IF ((INFO.GT.0).AND.DEBUG) WRITE(*, "(A,I3)") &
...
NATOMS = NCOORDS
NATOMS = SUM(NPERMSIZE(1:NPERMGROUP))
CALL REALLOCATEARRAYS()
...
DO J1=1,NPERMGROUP
    ...
    ENDDO
    CALL PAIRDISTS(NPERM,PDUMMYB(1:3*NPERM),PDUMMYA(1:3*NPERM),BULK_BOXVEC(1),BULK_BOXVEC(2), &
 & BULK_BOXVEC(3),BULKT,NDISTS(1:MAXNEI*NPERM,J1),NIDX(1:MAXNEI*NPERM,J1),MAXNEI)
    NDUMMY = NDUMMY + NPERM
ENDDO
...
INTEGER(KIND=INT64) :: KK(NCOORDS*MAXNEI), CC(NCOORDS*MAXNEI)
INTEGER(KIND=INT64) :: FIRST(NCOORDS+1), X(NCOORDS), Y(NCOORDS)
INTEGER(KIND=INT64) :: U(NCOORDS), V(NCOORDS), N8, SZ8, H
...
!      INTEGER(KIND=INT64) u(n), v(n)
      integer   m, i, j, k, l, l2, t, a
      INTEGER(KIND=INT64) n8, sz8
      integer J1
...
IF (SIZE(DUMMYDISTS).NE.(PMAXNEI*NATOMS*NPERMGROUP)) THEN
    IF (DEBUG) WRITE(*,"(A)") 'alignutils> reallocating distance arrays'
...
IF (SIZE(LPERM).NE.NATOMS) THEN
    IF (DEBUG) WRITE(*,"(A)") 'alignutils> reallocating coordinate arrays'
...
IF (SAVECOORDS.AND.(SIZE(BESTCOORDS).NE.(NSAVE*NATOMS*3))) THEN
    IF (DEBUG) WRITE(*, "(A,I3,A)") "alignutils> reallocating arrays to save ", NSAVE, " coordinates"
    NSTORED = 0
...
SUBROUTINE JOVOSAP(N,SZ,CC,KK,FIRST,X,Y,U,V,H)
      IMPLICIT NONE
      INTEGER(KIND=INT64), INTENT(IN)  :: N, SZ
      INTEGER(KIND=INT64), INTENT(IN)  :: CC(SZ),KK(SZ),FIRST(N+1)
      INTEGER(KIND=INT64), INTENT(OUT) :: X(N),Y(N),U(N),V(N),H
      INTEGER(KIND=INT64) CNT,L0,T,T0,TD,V0,VJ,DJ
...
!     I don't know how to make g77 read INTEGER(KIND=INT64) constants/parameters.
...
WRITE(*, "(A,I3,A)") "alignutils> found", NSTORED, " candidate alignments with distances:"
DO J=1,NSTORED
    WRITE(*, "(G20.10)") BESTDISTS(J)
END DO
