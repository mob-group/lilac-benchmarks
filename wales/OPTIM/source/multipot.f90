...
!   e) Check that you've made equivalent changes in GMIN so that the two systems stay in sync.
!   f) Enjoy!
MODULE MULTIPOT
    ...
    DOUBLE PRECISION, ALLOCATABLE :: CHRG(:)        ! Charges for individual atoms. Used by ISO_COULOMB.
...
!   Determine the names of the potentials and the number of atoms using each potential
    ...
    DO
        ...
        ELSE IF (DUMMYCHAR(1:3).EQ.'POT') THEN ! Read in information and parameters associated with this potential
            ...
            IF(NATOM_BY_POT(COUNTER).GT.NATOMS) WRITE(*,*) "WARNING: NATOM_BY_POT for potential ", POTTYPES(COUNTER), &
    ...
    DO J1 = 1, NPOTTYPES
        ...
        IF (DUMMYCHAR(1:1).EQ.'#') THEN  ! This potential is commented out: skip it.
            IF(DEBUG) WRITE(*,*) "Skipping commented block in multipotconfig"
            DO
                READ(22,*) DUMMYCHAR
        ...
        CASE('HSPR', 'PWCA', 'PLJ')
            ...
            IF(DEBUG) THEN
                WRITE(*,*) "Potential:", POTTYPES(J1)
                WRITE(*,*) "N_ATOM_PARTNERS:"
                WRITE(*,*) N_ATOM_PARTNERS(J1,:NATOM_BY_POT(J1))
                WRITE(*,*) "POTLISTS:"
                DO J2 = 1,NATOM_BY_POT(J1)
                    WRITE(*,*) "Atom number", J2, "in this potential"
                    WRITE(*,*) POTLISTS(J1,J2,:N_ATOM_PARTNERS(J1,J2))
                ENDDO
            ENDIF
        ...
        ! There is currently no facility to exclude the interactions within a rigid body. Use EWCA/ELJ if you need that.
            ...
            IF(DEBUG) THEN
                write(*,*) "Potential:", POTTYPES(J1)
                write(*,*) "DEGS_BY_POT:"
                write(*,*) DEGS_BY_POT(J1,:NATOM_BY_POT(J1)*3)
            ENDIF

        CASE('ICOU')
        ! Coulomb potential with no truncation or smoothing.
        ! Input format: list each charged atom on its own line, followed by its charge.
        ! If RIGIDINIT is set, interactions within a rigid body are not calculated.
           
           IF(BULKT) THEN
              WRITE(*,*) "multipot> ERROR: Isotropic Coulomb potential has been specified with periodic boundaries."
              WRITE(*,*) "PBCs are not implemented for this potential."
              STOP
           ENDIF

           ! Initialise charge array - start by assuming all atoms have 0 charge.
           ALLOCATE(CHRG(NATOMS))
           CHRG(:) = 0.0D0
           N_ATOM_PARTNERS(J1,:)=0

           DO J2=1,NATOM_BY_POT(J1)
              READ(22,*) POTLISTS(J1,J2,1), CHRG(POTLISTS(J1,J2,1))
           ENDDO

           IF (RIGIDINIT) THEN  ! Need to avoid adding interactions within a rigid body.
              ...
              DO J2=1,NATOM_BY_POT(J1)
                 ATOM1 = POTLISTS(J1,J2,1)
                 DO J3=J2+1,NATOM_BY_POT(J1) ! The lower limit here ensures we don't add a pair twice, and no atom 
                                             ! interacts with itself
                    ATOM2 = POTLISTS(J1,J3,1)
                    IF(RB_BY_ATOM(ATOM1) .NE. RB_BY_ATOM(ATOM2)) THEN ! The atoms are in different rigid bodies
                       N_ATOM_PARTNERS(J1,J2) = N_ATOM_PARTNERS(J1,J2) + 1
                       POTLISTS(J1,J2,1+N_ATOM_PARTNERS(J1,J2)) = ATOM2
                    ENDIF
                 ENDDO
              ENDDO

           ELSE ! Then every atom partners with every other atom in this potential exactly once.
              DO J2=1,NATOM_BY_POT(J1)
                 DO J3=J2+1,NATOM_BY_POT(J1) ! The lower limit here ensures we don't add a pair twice.
                    N_ATOM_PARTNERS(J1,J2) = N_ATOM_PARTNERS(J1,J2) + 1
                    POTLISTS(J1,J2,1+N_ATOM_PARTNERS(J1,J2)) = POTLISTS(J1,J3,1)
                 ENDDO
              ENDDO
           ENDIF
           ...
           IF(DEBUG) THEN
              WRITE(*,*) "Potential:", POTTYPES(J1)
              WRITE(*,*) "Number of atoms:", NATOM_BY_POT(J1)
              WRITE(*,*) "N_ATOM_PARTNERS:"
              WRITE(*,*) N_ATOM_PARTNERS(J1,:NATOM_BY_POT(J1))
              WRITE(*,*) "POTLISTS:"
              DO J2 = 1,NATOM_BY_POT(J1)
                 WRITE(*,*) "Atom number", J2, "in this potential"
                 WRITE(*,*) POTLISTS(J1,J2,:1+N_ATOM_PARTNERS(J1,J2))
              ENDDO
           ENDIF
        ...
        ! If this potential type is the last one in your MULTIPOTCONFIG file, then you must include a final line 
            ...
            ELSE
                WRITE(*,*) "multipot> Warning. An exclusion potential is being used with no EXCLUDE lists specified."
                WRITE(*,*) "multipot> It's probably more efficient to use the ISO- version of this potential instead."
        ...
        CASE DEFAULT
           WRITE(*,*) "multipot> Error: no rule to read input for this type of potential."
           STOP
        END SELECT
    ENDDO
    CLOSE(22)
...
SUBROUTINE MULTIPOT_CALL (X, G, ENERGY, GTEST, STEST)
    ...
    DO J1 = 1, NPOTTYPES
        ...
        SELECT CASE(POTTYPES(J1))
            ...
            CASE('ICOU')
                CALL ISO_COULOMB(X, POTLISTS(J1,:THISN,:), N_ATOM_PARTNERS(J1,:THISN), CHRG, POTSCALES(J1), THESEPARAMS, ENERGY, G, GTEST, STEST)
            CASE DEFAULT
                ...
                WRITE(*,*) "multipot> Error: unspecified potential"
                STOP
        END SELECT
    ENDDO
...
!    IF(DEBUG) THEN
!        WRITE(*,*) "Calling potential", POTTYPES(J1)
!        WRITE(*,*) "Degrees of freedom being used:"
!        WRITE(*,*) DEGS_BY_POT(POTID,:NDEGS)
!    ENDIF
