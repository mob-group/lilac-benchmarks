...
!     DISTANCE returns the squared distance
      ...
      USE KEY,ONLY : NPERMGROUP, NPERMSIZE, PERMGROUP, NSETS, SETS, GEOMDIFFTOL, EFIELDT, NRBGROUP, PAPT, PTSTSTT, NOINVERSION, &
&                        MULTISITEPYT 
...
! do not move along the z axis in case of a gravity field (currently implemented for MULTISITEPY)
        IF(EFIELDT.AND.MULTISITEPYT) THEN 
         DUMMYA(J2)   = DUMMYA(J2)
        ELSE
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
         ...
         ENDIF
         IF (ABS(Q2(1)**2+Q2(4)**2-1.D0)>1.D-8)WRITE(*,*)"jdf43>your quaternion, it's not a quaternion!"
