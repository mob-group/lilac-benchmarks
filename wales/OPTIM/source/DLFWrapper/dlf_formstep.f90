...
!!
!! DATA
!! $Date: 2012-04-20 12:26:24 +0100 (Fri, 20 Apr 2012) $
!! $Rev: 486 $
!! $Author: twk $
!! $URL: http://ccpforge.cse.rl.ac.uk/svn/dl-find/branches/release_chemsh3.5/dlf_formstep.f90 $
!! $Id: dlf_formstep.f90 486 2012-04-20 11:26:24Z twk $
!!
!! COPYRIGHT
!!
...
! ======================================================================
  CASE (3)
    ...
    CALL DLF_newlbfgs_STEP(GLOB%ICOORDS,GLOB%IGRADIENT,GLOB%STEP)
...
! L-BFGS
  case (3)
    call dlf_newlbfgs_init(glob%nivar,glob%newlbfgs_mem)
  case (9)
! fd-test
    needhessian=.true.
  case (10)
...
! L-BFGS
  CASE (3)
    ...
    CALL DLF_checkpoint_newlbfgs_write
...
! ======================================================================
  CASE (3)
    ...
    CALL DLF_checkpoint_newlbfgs_read(tok)
    return
...
! **********************************************************************
  ...
  call dlf_hessian_update(glob%nivar, glob%icoords, oldc, glob%igradient, &
       oldgrad, glob%ihessian, glob%havehessian, fracrec,was_updated)
  ...
  if(.not.glob%havehessian) then
    ...
    if(glob%havehessian) then
      ...
      if (printl>=6 .or. (glob%iopt==11 .and. printl>=2)) then
        ...
        call dlf_matrix_print(glob%nivar, glob%nivar, glob%ihessian)
      end if
...
! **********************************************************************
  ...
  else !(iivar<nivar.or. (twopoint.and.direction==1) )
  ...
  end if ! (iivar<nivar.or. (twopoint.and.direction==1) )
...
! **********************************************************************
  ...
  if(.not.fd_hess_running) then
    ...
    else ! (iivar<nivar.or. direction==1 )
      ! final step: calculate Hessian and deallocate
      !set back coordinates
      dder(iivar)=(dder(iivar)-glob%igradient(1))/(2.D0*delta(iivar))
      glob%icoords(1)=glob%icoords(1)+delta(iivar)
    ...
    end if ! (iivar<nivar.or. direction==1 )
...
! **********************************************************************
  ...
  if(abs(eigval(1))>soft_local.and.eigval(1)<0.D0) then
    !
    ...
    if(printf>=4) then
      ...
      svar=glob%distort
      call dlf_cartesian_xtoi(glob%nat,glob%nivar,glob%massweight,glob%xcoords, &
      ...
      !glob%icoords=glob%icoords+svar*reshape(eigvec(:,1),(/3,glob%nat/))
      if(abs(svar)<1.D-7) svar=20.D0
...
! **********************************************************************
  ...
  call dlf_cartesian_itox(glob%nat, glob%nivar, glob%massweight, glob%icoords, comcoords)
