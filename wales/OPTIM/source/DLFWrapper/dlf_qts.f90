...
!!  Judith B. Rommel (rommel@theochem.uni-stuttgart.de) now in Cambrigde, jbr36@cam.ac.uk
!!
...
!!$  print*,"JK test needhessian",qts%needhessian
  ...
  call dlf_newlbfgs_select("dimer rotation",.true.)
  call dlf_newlbfgs_init(glob%nivar,min(glob%maxrot,glob%nivar))
  call dlf_newlbfgs_deselect
  qts%status=1
...
!!$      end if
      ...
      do iimage=1,neb%nimage
        call dlf_direct_xtoi(glob%nvar,neb%varperimage,neb%xcoords(:,iimage), &
            glob%xgradient,glob%icoords(neb%cstart(iimage):neb%cend(iimage)), &
...
subroutine dlf_qts_destroy()
 ...
 use dlf_allocate, only: allocate, deallocate
  ...
  call deallocate(qts%tau_prime)
  call deallocate(qts%tau_qts)
  call deallocate(qts%theta)
  call deallocate(qts%coords_midp)
  call deallocate(qts%d_actionS_alt)
  call deallocate(qts%d_actionS)
  call deallocate(qts%dtau)
  call deallocate(qts%dist)


  call dlf_newlbfgs_select("dimer rotation",.false.)
  call dlf_newlbfgs_destroy
  call dlf_newlbfgs_deselect

  if(qts%needhessian) then
    call deallocate (qts%vhessian)
!    call deallocate (qts%total_hessian)
    call deallocate (qts%ihessian_image)
    !call deallocate (qts%xhessian_image)
  endif

  ! deallocate storage for hessian update in case any update is requested
  if(qts%tupdate) then
    call deallocate (qts%igradient) 
!      call deallocate (qts%igradient_old)
!      call deallocate (qts%icoords_old)
    call deallocate (qts%igradhess)
    call deallocate (qts%icoordhess)
  end if
...
!!$  end do
  ...
  gamma=0.D0
  olddtau=0.D0
  call dlf_newlbfgs_init(neb%nimage+1,20)
...
!!$  olddtau=-dfdd + gamma * olddtau
    ...
    cALL DLF_newlbfgs_STEP(dtau,dfdd,oldgrad)
  ...
  do iimage=1,nimage
    ...
    if ( iimage==1) then
      grad_action(cstart:cend)= &
          ...
          -glob%icoords(neb%cstart(iimage+1):neb%cstart(iimage+1))/qts%dtau(iimage+1))
    else if (iimage>1 .AND. iimage<nimage) then
      ! general image
      grad_action(cstart:cend)= &
          glob%igradient(cstart:cend)*(qts%dtau(iimage)+qts%dtau(iimage+1)) + &
          2.D0 * ( &
          -glob%icoords(neb%cstart(iimage-1):neb%cstart(iimage-1))/qts%dtau(iimage) &
...
!!$  print*,"Abs(grad)",sqrt(sum((qts%d_actionS(:))**2))
  ...
  !Compute theta
  call dlf_newlbfgs_select("dimer rotation",.false.)
  call dlf_newlbfgs_step(qts%tau_qts,-F_rot,qts%theta)
  call dlf_newlbfgs_deselect
  !normalise theta
...
!  qts%total_hessian=qts%total_hessian/beta_hbar**2*dble(2*nimage)**2
  ...
  if(printl>=2) then
    if(abs(evals_hess(1)/evals_hess(2)) < 10.D0) then
      ...
      write(stdout,*) &
          '  Factor by which they differ',evals_hess(1)/evals_hess(2)
    end if
    if(abs(evals_hess(glob%nzero+3)/evals_hess(glob%nzero+2)) < 10.D0) then
      write(stdout,"('Warning: first positive eigenvalue and highest zero &
          &eigenvalue differ by less than a factor of 10')")
      write(stdout,*) &
          '  Factor by which they differ', evals_hess(glob%nzero+3)/evals_hess(glob%nzero+2)
    end if
  end if
  ...
  if(printf>=4 .and. glob%iam == 0 ) then
    ...
    do ival=1,9
      ...
      do iimage=1,2*neb%nimage
        call dlf_cartesian_itox(glob%nat,neb%varperimage,glob%massweight,&
...
!!
!! Take into account the possibility for bimolecular rates here
!! (calculate the translational partition function of the incoming
!! atom (only implemented for a single atom for the time being)
!!
!! SYNOPSIS
subroutine qts_reactant(qrs,ers,qrot,tbimol,tok)
  ...
  call read_qts_reactant(glob%nat,neb%varperimage,&
      ers,varperimage_read,xcoords,eigvals,mass_bimol,"",tok)
  !if(.not.tok) return
  if(.not.tok) then
  ...
  call rotational_partition_function(glob%nat,glob%nzero,xcoords,qrot)
  qrot=log(qrot)
...
!! SOURCE
  ...
  do iimage=1,neb%nimage
    ...
    call dlf_cartesian_gradient_itox(glob%nat,neb%varperimage,glob%massweight,&
        glob%igradient(neb%cstart(iimage):neb%cend(iimage)),&
...
subroutine read_qts_reactant(nat,varperimage,&
    ene,varperimage_read,xcoords,eigvals,mass_bimol,label,tok)
!! SOURCE
  use dlf_parameter_module, only: rk
  use dlf_neb, only: neb ! cstart and cend
  use dlf_global, only: glob,printl,stdout
  ...
  ! allow for one atom less (bimolecular reactions of a molecule with a single atom)
  if(nat/=nat_ .and. nat_/=nat-1 ) then
    if(printl>=2) write(*,*) "Error reading qts_reactant.txt file: Number of &
        &atoms not consistent"
    return
  end if
  if(varperimage<varperimage_) then
...
!  use dlf_qts
  ...
  logical   :: bimol  ! bimolecular or not
  ...
  real(rk)  :: tfact,tempe1,beta_hbar,KBOLTZ_AU,svar
  real(rk)  :: tdel
  real(rk)  :: wigner,wigner_simple
  real(rk)  :: tcross
  integer   :: itemp,ivar
  integer   :: ibound,varperimage_read,varperimage_readr
...
! line 4: bimolecular (T/F)
  bimol=.false.
  INQUIRE(FILE='class.in',EXIST=chkfilepresent)
  IF (chkfilepresent) THEN
    OPEN(28,FILE='class.in',STATUS='old',ACTION='read')
    READ (28,'(a)') sline 
    !READ (28,*), natr, nat
!    varperimager=natr*3
!   varperimage=nat*3
...
201 continue
  ...
  call read_qts_reactant(natr,varperimager,&
      ene_rs,varperimage_readr,xcoords,eigvals_rs,mu_bim,"",tok)

  if(.not.tok) then ! the same is done for the TS below ...
    ! now we have to try and read qts_hessian_rs.txt
    if(printl>=6) write(stdout,*) "Attempting to read from file qts_hessian_rs.txt"
  ...
  if(bimol.and.mu_bim<0.D0) then
    if(printl>0) then
      write(stdout,*) "Please include the reduced mass of the incoming atom in &
          &qts_reactant.txt after the energy (third line)"
      write(stdout,*) "The energy of the incoming atom should be added to the &
          &energy given as the first entry in line 3."
    end if
    call dlf_fail("Error reading reactant for bimolecular rates")
  end if
  if(bimol) then
  ...
  call read_qts_reactant(nat,varperimage,&
      ene_ts,ivar,xcoords,eigvals_ts,svar,"ts",tok)
  ...
  !write(stdout,*) tstart,'tstart',tend,tsteps
  !call dlf_fail("stop in qtshtst")

  !jbr36 adapted for an equidistant temperature spacing
  !tfact=(tstart-tend)/dble(tsteps-1)
  ...
  if(printl>=3) then
    ...
    svar=KBOLTZ_AU*tempe1*log(qrot_red)
  ...
  inquire(file="rate_H",exist=tkie)
  if(tkie) then
    open(unit=12,file="rate_H")
    do
...
1003 continue
  ...
  ibound=1 ! only 1 is correct

  do itemp=1,tsteps

    !print*,"Temperature",itemp,tempe1
    beta_hbar=1.D0/(tempe1*KBOLTZ_AU)  ! beta * hbar = hbar / (kB*T)
    ...
    if(bimol) then
      ...
      twopimuktoverh2= twopimuktoverh2 * kboltz * tempe1! kboltz=1.3806503E-023
      twopimuktoverh2= twopimuktoverh2 / planck ! planck=6.626068E-034
      twopimuktoverh2= twopimuktoverh2 * SQRT(twopimuktoverh2)
      twopimuktoverh2= 1.D-6 * twopimuktoverh2
      phi_rel=log(twopimuktoverh2)
      !print *, 'log10(phi_rel)', phi_rel/log10, 'at T', tempe1
    end if

    lrate_cl=0.D0
    lrate_qvib=0.D0
    do ivar=ibound,varperimage
...
!!$    lrate_cl= 0.5D0*lrate_cl -log(2.D0*pi*beta_hbar) + ( -beta_hbar * (ene_ts-ene_rs) )
    ...
    if(printl>=2) write(stdout,'(5f20.12)') &
        1000.D0/tempe1,(lrate_cl+timeunit)/log10,(lrate_qvib+timeunit)/log10, &
        (lrate_qvib+log(wigner_simple)+timeunit)/log10, (lrate_qvib+log(kappa_eck)+timeunit)/log10
    write(15,'(5f20.12)') 1000.D0/tempe1,(lrate_cl+timeunit)/log10,(lrate_qvib+timeunit)/log10, &
        (lrate_qvib+log(wigner_simple)+timeunit)/log10, (lrate_qvib+log(kappa_eck)+timeunit)/log10
    write(16,'(2f20.12)') tempe1,-(lrate_qvib+log(beta_hbar*2.D0*pi))/beta_hbar!, &
        !-(lrate_qvib+log(wigner_simple)+log(beta_hbar*2.D0*pi))/beta_hbar

    !polywigner
    bvar=beta_hbar*sqrt(abs(eigvals_ts(1)))*0.5D0
    write(17,'(5f20.12)') 1000.D0/tempe1, &
    ...
    if(tkie) then
      ...
      write(13,'(5f20.12)') tempe1,exp(hcl*log10-(lrate_cl+timeunit)), &
      ...
      if(abs(svar-1000.D0/tempe1) > 1.D-6) then
        write(13,*) "#Error: temperatures don't match. Here: ",tempe1,", rate_H:",1000.D0/svar
        close(13)
        close(12)
        tkie=.false.
      end if
    ...
    !tempe1=tempe1-tfact
    !tempe1=tempe1*tfact
    tempe1=tempe1/(1.D0+tempe1*tdel)
  end do
  ...
  if(tend>tcross.or.tstart>tcross) then
    ...
    do itemp=1,tsteps
      if(tempe1>tcross) then
        if(printl>=2) write(stdout,'(2f20.12)') 1000.D0/tempe1,rate_wigner(itemp)
         write(15,'(2f20.12)') 1000.D0/tempe1,rate_wigner(itemp)
      end if
...
subroutine kappa_eckart(beta_hbar,Vb,alpha,kappa_eck)
  ...
  !kappa_eck=1.D0
  enemax=1000.D0 ! one should probably improve on that...
  npoint=100000
  pi=4.D0*atan(1.D0)
  ...
  do ipoint=1,npoint
    ...
    trans=1.D0/(1.D0+cosh(dpar)/cosh(2.D0*apar))
    ! weights for endpoints
    if(ipoint==1.or.ipoint==npoint) trans=trans*0.5D0
    kappa_eck=kappa_eck+trans*exp(-beta_hbar*ene)
  end do
  ! step size
  kappa_eck=kappa_eck*enemax/dble(npoint-1)
...
!  call allocate(hess_local,neb%varperimage,neb%varperimage,neb%nimage)
  if(glob%iopt /=12 ) then
    ...
    if(tok) then
      ...
      if(qts%needhessian.and.nimage_read==1) then
        ...
        cstart=neb%cstart(1)
        cend=neb%cend(1)
        call dlf_direct_xtoi(glob%nvar,neb%varperimage,neb%xcoords(:,1), &
  ...
  call allocate(xcoords,3*glob%nat)
  call read_qts_reactant(glob%nat,neb%varperimage,&
      qts%ers,varperimage_read,xcoords,eigval,mass_bimol,"",tok)
  call deallocate(xcoords)
