...
USE KEY,ONLY : NPERMGROUP, NPERMSIZE, PERMGROUP, NSETS, SETS, GEOMDIFFTOL, &
  &            NFREEZE, RBAAT, ANGLEAXIS2, BESTPERM, LOCALPERMDIST, NTSITES, &
  &            LOCALPERMCUT, STOCKT, GMAX, CONVR, EDIFFTOL, INTCONSTRAINTT, &
  &            LOCALPERMNEIGH, LOCALPERMCUT2, ATOMACTIVE
USE MODCHARMM,ONLY : CHRMMT
IMPLICIT NONE

INTEGER, PARAMETER :: MAXIMUMTRIES=10
INTEGER NATOMS, NPERM, PATOMS, NRB, OPNUM,  NORBIT1, NORBIT2, NCHOOSE2, NCHOOSE1, NTRIES, NORBITB1, NORBITB2, NMOVE, NDMEAN
...
DOUBLE PRECISION DIST2, COORDSA(3*NATOMS), COORDSB(3*NATOMS), DISTANCE, DUMMYA(3*NATOMS), &
  &              BESTA(3*NATOMS), DUMMYB(3*NATOMS), DUMMY(3*NATOMS), DIST, DSUM
...
DOUBLE PRECISION ROTA(3,3), ROTINVA(3,3), ROTB(3,3), ROTINVB(3,3), RMATBEST(3,3), TMAT(3,3)
DOUBLE PRECISION PVEC(3), RTEMP1(3,3), RTEMP2(3,3)
LOGICAL DEBUG, TWOD, RIGID, BULKT, PITEST, AOK, BOK, ADDED, PERMUTABLE(NATOMS), USEATOM
DOUBLE PRECISION PDUMMYA(3*NATOMS), PDUMMYB(3*NATOMS), LDISTANCE, DUMMYC(3*NATOMS), XDUMMY, DUMMYD(3*NATOMS), &
   &             LDBEST(NPERMGROUP), LDBESTATOM
DOUBLE PRECISION SPDUMMYA(3*NATOMS), SPDUMMYB(3*NATOMS), AINIT, BINIT
...
INTEGER TRIED(NATOMS), DLIST(NATOMS), SORTLIST(NATOMS), NDUMMY2, INGROUP(NATOMS), NADDED, DOGROUP

IF (DEBUG.AND.(DOGROUP.EQ.0)) THEN
   IF (CHRMMT) CALL UPDATENBONDS(COORDSA)
   CALL POTENTIAL(COORDSA,AINIT,VNEW,.TRUE.,.FALSE.,RMS,.FALSE.,.FALSE.)
   WRITE(*,'(2(A,G25.15))') ' initial energy for structure A=             ',AINIT,' RMS=',RMS
   IF (RMS-MAX(GMAX,CONVR).GT.1.0D-6) THEN
      WRITE(*,'(A)') ' lopermdist> WARNING *** RMS for structure A is outside tolerance'
   ENDIF
   IF (CHRMMT) CALL UPDATENBONDS(COORDSB)
   CALL POTENTIAL(COORDSB,BINIT,VNEW,.TRUE.,.FALSE.,RMS,.FALSE.,.FALSE.)
   PRINT '(2(A,G25.15))',' initial energy for structure B=             ',BINIT,' RMS=',RMS
   IF (RMS-MAX(GMAX,CONVR).GT.1.0D-6) THEN
      PRINT '(A)',' lopermdist> WARNING *** RMS for structure B is outside tolerance'
   ENDIF
ENDIF

!IF (DOGROUP.GT.0) THEN
!   WRITE(*,'(A,I6,A)') ' lopermdist> Checking only group ',DOGROUP,' and using only QCI active atoms'
!ENDIF
...
!     WRITE(*,'(A,I6,2G20.10)') 'lopermdist> J2,dmean,localpermcut2=',J2,DMEAN(J2),LOCALPERMCUT2
...
!  PRINT '(A,3I6,3G20.10)','J1,PATOMS,NOTHER,root LDBEST(J1),root LDISTANCE=',J1,PATOMS,NOTHER,SQRT(LDBEST(J1)),SQRT(LDISTANCE)
...
!
   ...
   ENDIF
!  PRINT '(A,2G20.10)','root LDISTANCE,root LDBESTATOM=',SQRT(LDISTANCE),SQRT(LDBESTATOM)

!  PRINT '(A,4I6,2G20.10)','NORBIT1,NORBIT2,NCHOOSE1,NCHOOSE2,root LDISTANCE,root LDBEST=', &
! &                         NORBIT1,NORBIT2,NCHOOSE1,NCHOOSE2,SQRT(LDISTANCE),SQRT(LDBEST(J1))
...
!  PRINT '(A,2G20.10)','root LDBESTATOM,LOCALPERMCUT=',SQRT(LDBESTATOM),LOCALPERMCUT 
   IF (SQRT(LDBESTATOM).GT.LOCALPERMCUT) THEN
!     IF (DEBUG) THEN
!        PRINT '(A,G15.5,A,I6,A,G15.5)',' lopermdist> Best distance ',SQRT(LDBESTATOM), &
! &                                     ' is too large for atom ',DLIST(NOTHER),' cutoff=',LOCALPERMCUT
!     ENDIF
...
!           IF (DOGROUP.GT.0) PRINT '(A)',' lopermdist> atomactive values:'
!           IF (DOGROUP.GT.0) PRINT '(20L5)',ATOMACTIVE(NOTHER-NADDED+1:NOTHER-1)
!        ENDIF
...
!        ENDIF
      ...
      ENDIF
!     IF (DEBUG) THEN
!        PRINT '(A)',' all other atoms:'
!        PRINT '(20I5)',DLIST(1:NOTHER)
!        IF (DOGROUP.GT.0) PRINT '(20L5)',ATOMACTIVE(DLIST(1:NOTHER))
!     ENDIF
...
!     PRINT '(2(A,I6))',' lopermdist> Atom ',NEWPERM(PERMGROUP(NDUMMY+J2-1)), &
...
!        WRITE(*,'(A,I6,A,I6)') ' lopermdist> need to move atom ',NEWPERM(J2),' to position ',J2
         NMOVE=NMOVE+1
      ENDIF
   ENDDO
!  PRINT '(A,I6,A,I6)',' lopermdist> Total permutations (not applied!) for optimal alignment of group ',DOGROUP,' is ',NMOVE
   RETURN
ENDIF
NMOVE=0
! DO J2=1,NATOMS
...
IF (DEBUG.AND.(DOGROUP.EQ.0)) THEN
   ...
   PRINT '(2(A,G25.15))',' lopermdist> final   energy for structure A=             ',ENERGY,' RMS=',RMS
   IF (ABS(ENERGY-AINIT).GT.EDIFFTOL) THEN
      PRINT '(A)',' minpermdist> WARNING *** energy change for structure A is outside tolerance'
   ENDIF
   IF (CHRMMT) CALL UPDATENBONDS(COORDSB)
   CALL POTENTIAL(COORDSB,ENERGY,VNEW,.TRUE.,.FALSE.,RMS,.FALSE.,.FALSE.)
   PRINT '(2(A,G25.15))',' lopermdist> final   energy for structure B=             ',ENERGY,' RMS=',RMS
   IF (ABS(ENERGY-BINIT).GT.EDIFFTOL) THEN
      PRINT '(A)',' minpermdist> WARNING *** energy change for structure B is outside tolerance'
   ENDIF
ENDIF
...
SUBROUTINE DEPTHSEARCH(PATOMS,MAXSEP,NDUMMY,NPG,NDIST1)
USE KEY, ONLY : CONIFIX, CONJFIX, NCONSTRAINTFIX, CONCUTFIX, NPERMSIZE, PERMGROUP
USE COMMONS, ONLY: NATOMS, DEBUG
IMPLICIT NONE
INTEGER NDIST1(NATOMS), NCYCLE, DMAX1, NUNCON1, MAXSEP, NDUMMY, NPG, PATOMS, J1, J2, DMIN1, NDPARAM
LOGICAL CHANGED

NDPARAM=999
NDIST1(1:NATOMS)=NDPARAM
!
! Set atoms in the permutable group NPG to depth 0.
! We only need to set distances up to a number of steps MAXSEP.
!
DO J1=1,NATOMS
   DO J2=1,NPERMSIZE(NPG)
      IF (J1.EQ.PERMGROUP(NDUMMY+J2-1)) THEN
         NDIST1(J1)=0
      ENDIF
   ENDDO
ENDDO
...
IF (MAXSEP.EQ.0) THEN
   PRINT *,' depthsearch ERROR MAXSEP=',MAXSEP
   STOP
ENDIF
...
DMAX1=0
NUNCON1=0
IF (NCONSTRAINTFIX.EQ.0) THEN
   PRINT '(A)',' lopermdist> ERROR *** no constraints specified - depth search impossible'
   STOP
ENDIF
DO J1=1,NATOMS
   IF (NDIST1(J1).EQ.0) CYCLE ! permutable atom
   DO J2=1,NCONSTRAINTFIX
      IF (CONIFIX(J2).EQ.J1) THEN
         IF (NDIST1(CONJFIX(J2))+1.LT.NDIST1(J1)) THEN
            CHANGED=.TRUE.
            NDIST1(J1)=NDIST1(CONJFIX(J2))+1
         ENDIF
      ELSE IF (CONJFIX(J2).EQ.J1) THEN
         IF (NDIST1(CONIFIX(J2))+1.LT.NDIST1(J1)) THEN
            CHANGED=.TRUE.
            NDIST1(J1)=NDIST1(CONIFIX(J2))+1
         ENDIF
      ENDIF
   ENDDO
   IF ((NDIST1(J1).GT.DMAX1).AND.(NDIST1(J1).NE.NDPARAM)) DMAX1=NDIST1(J1)
   IF (NDIST1(J1).LT.DMIN1) DMIN1=NDIST1(J1)
   IF (NDIST1(J1).EQ.NDPARAM) NUNCON1=NUNCON1+1
ENDDO
! PRINT '(A,L5,3I6)','CHANGED,NCYCLE,MAXSEP,NCONSTRAINTFIX=',CHANGED,NCYCLE,MAXSEP,NCONSTRAINTFIX
IF (CHANGED.AND.(.NOT.(NCYCLE.GE.MAXSEP))) GOTO 5
IF (DEBUG) THEN
!  PRINT '(3(A,I8))',' depthsearch> steps to permutable atoms converged in ',NCYCLE, &
!   &                        ' cycles; maximum=',DMAX1,' disconnected=',NUNCON1
!  PRINT '(A,I6,A)',' depthsearch> Atoms within ',MAXSEP,' steps of the permutable set:'
!  DO J1=1,NATOMS
!     IF (NDIST1(J1).LE.MAXSEP) PRINT '(2I6)',J1,NDIST1(J1)
!  ENDDO
ENDIF

END SUBROUTINE DEPTHSEARCH
