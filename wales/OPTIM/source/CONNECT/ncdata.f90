...
MODULE CONNECTDATA
     ...
     LOGICAL :: FINISHED
!--------------------------------------------------------
     ! NTSGOOD discounts duplicate TS, those with chirality/cis-trans problems, 
     ! those where the path run has failed, and those where the connecting barriers are too high
     INTEGER :: NMIN=0,NTS=0, NTSGOOD=0 !NUMBER OF TS/MIN FOUND SO FAR
     INTEGER :: NTSOLD=0
...
!--------------------------------------------------------
     TYPE MINDATA
          ...
          INTEGER,POINTER :: INTNTRIES(:) ! Holds number of intconstraint tries for this minimum
     ...
     TYPE TSDATA
          ...
          INTEGER         :: HITS ! Counts the number of times this ts has been found
     END TYPE TSDATA
