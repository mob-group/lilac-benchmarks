...
SUBROUTINE INITIALISE(NA,EI,Q,EF,FIN,ENDPOINTSEP)
     USE KEY, ONLY: READSP, INTIMAGE, INTLJT, INTCONSTRAINTT, FREEZENODEST, ATOMACTIVE, MLP3T, MLPVB3T,ORBITALS
     IMPLICIT NONE
...
!    LOGICAL EDGEINT(INTIMAGE+1,NA,NA)
     ...
     NATOMS=NA
     NOPT=3*NATOMS
     IF (MLP3T.OR.MLPVB3T.OR.ORBITALS) NOPT=NATOMS
     ...
     MI(1)%DATA%X => Q
     MI(2)%DATA%X => FIN
     ALLOCATE(MI(2)%DATA%D(1),MI(2)%DATA%NTRIES(1),MI(2)%DATA%INTNTRIES(1))
...
!            MI(2)%DATA%INTERP(1)=MI(2)%DATA%INTERP(1)/1.0D3+ENDPOINTSEP ! new formulation
...
131     CLOSE(UNIT=82)
        ...
        DO
          ...
          NTS=NTS+1
          TS(NTS)%DATA%E => ETEMP
          PRINT *,'ido> A NOPT=',NOPT
          PRINT *,'ido> A SIZE ts=',SIZE(TS(NTS)%DATA%X(:))
...
SUBROUTINE OUTPUT
     ...
     USE KEYCONNECT
     USE CONNECTUTILS
     USE BESTPATHDIJKSTRA
     USE KEY, ONLY: DUMPSP, MACHINE, DUMPALLPATHS, DUMPBESTPATH
     IMPLICIT NONE
     ...
     IF (FINISHED) THEN
          ...
          DO
               ...
               WRITE(*,'(I4,1X,F16.7,1X,G11.5,F16.7,1X,G11.5,1X,F15.7,F8.3,F8.3,F8.3,F8.3)') &
