...
MODULE DECIDEWHATTOCONNECT
     ...
     SUBROUTINE DECIDE
          USE PORFUNCS
          USE CONNECTDATA
          USE CONNECTUTILS,ONLY: GETDISTANCE,SETDISTANCE,GETINTERP,NREDO
          USE KEY, ONLY : INTCONSTRAINTT, INTLJT, MAXGAPT
          USE KEYDECIDE
...
INTEGER :: I, NDUMMY, BEGIN, END, NSTEP, J, ISTAT, NDMAX
TYPE MINIMUM
...
END TYPE PATHWAY
DOUBLE PRECISION :: WEIGHT, MINWEIGHT, DUMMYDJW, DIST, DMAX
INTEGER JS, JF, J1, J2
...
! Start algorithm at 'begin' minimum
   ...
   MAIN: DO
      DO J=2, NMIN    ! LOOP OVER ALL MINIMA 
         ...
         ENDIF
!
! Disfavour edges that have previously been tried by multiplying the weight by
! the number of previous attempts.
!
         JS=MAX(I,J)
         JF=MIN(I,J)
         IF (INTCONSTRAINTT.OR.INTLJT) THEN
            IF (WEIGHT.LT.1.0D300) WEIGHT=WEIGHT*(MI(JS)%DATA%INTNTRIES(JF)+1)
         ELSE
            IF (WEIGHT.LT.1.0D300) WEIGHT=WEIGHT*(MI(JS)%DATA%NTRIES(JF)+1)
         ENDIF
!        PRINT '(A,3I6)','decide> I,J,NTRIES=',I,J,MI(JS)%DATA%NTRIES(JF)
...
! If no temporary minima with total weight less than HUGE are present, we will never exit the outer do loop!
      IF (NDUMMY==I) THEN
         WRITE(*,'(A)') ' decide> There appear to be no connections left to try in DIJKSTRA'
         IMAGEMAX=MAX(1.1D0*IMAGEMAX,DBLE(IMAGEMAX+1))
         IF (IMAGEMAX.GT.10000) STOP
         WRITE(*,'(A,I6,A)') ' decide> Increasing maximum number of images to ',IMAGEMAX,' and clearing weights'
         DO J1=1,NMIN-1
            DO J2=J1+1,NMIN
...
! WRITE(*,'(I6)') BEGIN
  WRITE(*,'(A)') ' '
  IF (MAXGAPT) THEN
     PRINT '(A,G20.10,A,I8)',' decide> Largest distance is ',DMAX,' for gap ',NDMAX
     DIJPAIRDIST(1)=DMAX
     NDIJPAIRS=1
     DIJPAIR(1,1)=DIJPAIR(NDMAX,1)
     DIJPAIR(1,2)=DIJPAIR(NDMAX,2)
  ENDIF
