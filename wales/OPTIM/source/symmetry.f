...
! jwrm2> Need to use Cartesian coordinates for Generalised Thomson, but we don't want to alter the coordinates,
!        so we'll restore them to QSAVE at the end.
      IF (GTHOMSONT) CALL GTHOMSONANGTOC(Q, QSAVE, NATOMS)
...
!
! Here GMIN uses a different routine, testsymop, which
! may be more robust. GMIN forbids exchange of atoms with
! different masses, whereas this test allows it.
!
...
C jwrm2> If the structure is 2D and the highest order axis is 2, we have
C        accidental degeneracy, so skip on to the Abelian block.
            IF ((IHIGH.EQ.2).AND.TWOD) THEN
              IF (IPRNT.GE.3) WRITE(*,*)'Accidental degeneracy detected'
              IDEGEN = 0
              GOTO 630
            ENDIF
...
390            FORMAT(' Horizontal plane of symmetry found.')
               ...
               ELSE
                  ...
                  DO J = 1,3
                     ...
                     CALL REFLECT(NEWQ,SCRATCH,NATOMS,J)
                     CALL COMPARE2(SCRATCH,QSORT,NORD,ICOMP2,TOLD)
                     IF (TWOD .AND. J.EQ.3) ICOMP2 = 1
400                  IF (ICOMP2.EQ.0) FPGRP = 'CNv'
                  ENDDO
               ENDIF
            ENDIF
         ENDIF
...
776      FORMAT(A1,I2,A1)
      ...
      IF (IDEGEN.GT.0) THEN
         IF (TWOD) THEN
            IF (PTEST) WRITE(*,'(A,A4,A)') ' symmetry> The 2D subgroup is ',FPGRP,'.'
            WRITE(GPSTRING,'(A,A4,A)') ' symmetry> The 2D subgroup is ',FPGRP,'.'
         ELSE
...
750      FORMAT(' symmetry> The full molecular point group is ',A4,'.')
      ...
      IF (IDEGEN.EQ.0) THEN
         IF (TWOD) THEN
            IF (PTEST) WRITE(*,'(A,A4,A)') ' symmetry> The 2D subgroup is ',STRING(BPGRP,IHIGH),'.'
            WRITE(GPSTRING,'(A,A4,A)') ' symmetry> The 2D subgroup is ',STRING(BPGRP,IHIGH),'.'
         ELSE
...
! jwrm2> Restore polar coordinates if using Generalised Thomson
      IF (GTHOMSONT) Q(1:3*NATOMS) = QSAVE(1:3*NATOMS)
...
! 
      IF (PTEST) THEN
         IF (TWOD) THEN
            WRITE(*,'(A,I6)') ' symmetry> Order of 2D subgroup=',HORDER
         ELSE
