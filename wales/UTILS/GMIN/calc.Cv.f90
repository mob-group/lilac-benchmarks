...
DOUBLE PRECISION, ALLOCATABLE :: GRAD(:), DGRAD(:), EVAR(:), FITA(:), FITB(:), FITC(:), REPWEIGHTS(:), PQT(:)
...
DOUBLE PRECISION, ALLOCATABLE :: XPIQBIN(:), MEANEQBIN(:), MEANORDERQBIN(:,:), PROBORDERQBIN(:), OVALUES(:), MEANAQBIN(:,:), AVALUES(:)
DOUBLE PRECISION, ALLOCATABLE :: ORDERMAX(:),ORDERMIN(:),ONORM(:),OGRID(:),OGRIDINT(:)
INTEGER, ALLOCATABLE :: VISITS2(:,:,:), VISITSALL(:,:)
...
DOUBLE PRECISION, ALLOCATABLE :: MAXVISITS(:), VTOT2(:)
DOUBLE PRECISION CHI2PLUS, CHI2MINUS, TOL, MAXBHSTEP, DPRAND, DELTA, DUMMY, &
  &              USEQBINS, PEMIN, QHISTINT, BSCALE, HSUM, BSUM, XDUMMY, RATMAX, FIRSTFAC, XXDUMMY, EREF, TEMPERATURE
INTEGER NBINS, J1, NTEMP, J2, KAPPA, NREP, NVAR, NEVAR, NCOUNT, MUPDATES, ITMAX, ITDONE, NHOPS, RNDSEED, &
  &     J3, NQBINS, J4, NVAROLD, MERGEI, MERGEQ, MGAMMA, NDUMMY2, J5, NBINSSAVE, NQBINSSAVE, JTEMP, &
  &     PEBINMIN, PEBINMAX, MINFIT, NDUMMY, NBINSAVE, NQBINSAVE, NREPMIN, NDUMMY3, &
  &     NRESMIN, LOWEST, NORDER, NORDERGRID
COMMON /PELIMITS/ PEBINMIN, PEBINMAX
CHARACTER(LEN=80) FSTRING, FNAME, FSTRING2, PTSTART, BSSTART, BSPTFINISH, REPSTRING, OFILE
CHARACTER(LEN=1) DUMMYSTRING
LOGICAL, ALLOCATABLE :: ALLZERO(:), ALLZERO2(:,:), ALLZEROQ(:)
LOGICAL CONVERGED, RESERVOIRT, FIRST, YESNO, FIXB, MAKEORDER
...
MERGEI=1
MERGEQ=1
INQUIRE(FILE='merge',EXIST=YESNO)
IF (YESNO) THEN
   OPEN(UNIT=1,FILE='merge',STATUS='OLD')
   READ(1,*,END=963) MERGEI
   READ(1,*,END=963) MERGEQ
963 PRINT '(A)','Detected data file merge in current working directory'
   CLOSE(1)
ENDIF
MAKEORDER=.FALSE.
INQUIRE(FILE='orderdata',EXIST=YESNO)
IF (YESNO) THEN
   OPEN(UNIT=1,FILE='orderdata',STATUS='OLD')
   READ(1,*,END=964) NORDER, NORDERGRID, OFILE
964 PRINT '(A,I10,A,I10)','Detected data file orderdata in current working directory - number of order parameters:',NORDER, &
   & ' file: ' // TRIM(ADJUSTL(OFILE)) //' grid entries=',NORDERGRID
   CLOSE(1)
   MAKEORDER=.TRUE.
ENDIF
FIXB=.FALSE.
...
ALLOCATE(VISITSALL(NBINSSAVE,NQBINSSAVE))
...
ALLOCATE(VTOT2(NQBINS))
ALLOCATE(MOSTVISITED(NQBINS))
!
! The quench energy bin value is always assumed to be the first order parameter, and
! is in the second column of the OFILE.
!
IF (MAKEORDER) ALLOCATE(MEANORDERQBIN(NQBINS,NORDER),PROBORDERQBIN(NQBINS),OVALUES(NORDER), &
  &                     AVALUES(NORDER-1),ORDERMAX(NORDER),ORDERMIN(NORDER),MEANAQBIN(NQBINS,NORDER-1))
...
DO J1=1,NREP ! read in data for all replicas regardless of NREPMIN
   ...
   IF (PTSTART(1:2).NE.'na') THEN
      PRINT '(A,A)','opening unit 2 file=',TRIM(ADJUSTL(FSTRING2))
   ...
   DO J2=MERGEI*NBINS,NBINSAVE-1 ! in case MERGEI is not an integer divisor of NBINSAVE
      READ(1,*) DUMMYSTRING
   ENDDO
   IF (NQBINS.EQ.0) CYCLE
   READ(1,*) DUMMYSTRING
   DO J2=1,NQBINS
      QENERGY(J2)=0.0D0
      DO J3=1,MERGEQ
         READ(1,*) DUMMY
!        IF (J3.EQ.1) QENERGY(J2)=DUMMY
!        PRINT '(A,2I8,2G20.10)','J2,J3,DUMMY,QENERGY=',J2,J3,DUMMY,QENERGY(J2)
         QENERGY(J2)=QENERGY(J2)+DUMMY
      ENDDO
      QENERGY(J2)=QENERGY(J2)/MERGEQ
   ENDDO
   CLOSE(1)
!  WRITE(*,'(A,I5,2(A,G25.10))') 'temperature for replica ',J1,' is ',REPT(J1),' total visits=',VTOTAL(J1)
...
!
   ALLOCATE(BAV(NQBINS),NMINQBIN(NQBINS),XPIQBIN(NQBINS),MEANEQBIN(NQBINS))
   REWIND(10)
   PEMIN=1.0D100
   DO J1=1,NRESMIN
   ...
   DO J1=1,NQBINS
      IF (XPIQBIN(J1).GT.0) THEN
         IF (NMINQBIN(J1).NE.0) MEANEQBIN(J1)=MEANEQBIN(J1)/NMINQBIN(J1)
         BAV(J1)=BAV(J1)/XPIQBIN(J1)
         PRINT '(A,I6,A,I6,A,F12.4,2(A,G20.10))',' # entries in quench bin ',J1,' is ', &
  ...
  &                                   ' <V>=',MEANEQBIN(J1)
   ...
   ENDDO
   IF (MAKEORDER) THEN
      MEANORDERQBIN(1:NQBINS,1:NORDER)=0.0D0
      MEANAQBIN(1:NQBINS,1:NORDER-1)=0.0D0
      ORDERMAX(1:NORDER)=-HUGE(1.0D0)
      ORDERMIN(1:NORDER)= HUGE(1.0D0)
      NMINQBIN(1:NQBINS)=0
      OPEN(UNIT=10,FILE=OFILE,STATUS='OLD')
      NDUMMY=0 ! counter for number of minima in OFILE. Could be different from min.data
      DO
         READ(10,*,END=31) OVALUES(1:NORDER),AVALUES(1:NORDER-1)
         J2=INT((OVALUES(1)-QENERGY(1))/QHISTINT)+1
         NMINQBIN(J2)=NMINQBIN(J2)+1
         MEANORDERQBIN(J2,1:NORDER)=MEANORDERQBIN(J2,1:NORDER)+OVALUES(1:NORDER)
         MEANAQBIN(J2,1:NORDER-1)=MEANAQBIN(J2,1:NORDER-1)+AVALUES(1:NORDER-1)
         NDUMMY=NDUMMY+1
         IF (ABS(OVALUES(1)-EMIN(NDUMMY)).GT.1.0D-5) THEN
            PRINT '(A,2G20.10)','ERROR *** energies from order parameter data and min.data disagree by more than 0.00001:', &
   &                             OVALUES(1),EMIN(NDUMMY)
         ENDIF
         DO J3=1,NORDER
            IF (OVALUES(J3).GT.ORDERMAX(J3)) ORDERMAX(J3)=OVALUES(J3)
            IF (OVALUES(J3).LT.ORDERMIN(J3)) ORDERMIN(J3)=OVALUES(J3)
         ENDDO
      ENDDO
31    WRITE(*,'(A,I10,A)') 'keyword> There are ',NDUMMY,' entries in ' // OFILE
      PRINT '(A)','Minimum and maximum values of the order parameters and grid spacing'
!
! Extend range for order parameter probability by 10% at either end.
! Tabulate values at 
! ORDERMIN(J3)-0.1D0*(ORDERMAX(J3)-ORDERMIN(J3)) + (counter-1) * OGRIDINT(J3)
! with counter from 1 to NORDERGRID
!
      ALLOCATE(OGRIDINT(NORDER))
      DO J3=1,NORDER
         OGRIDINT(J3)=(ORDERMAX(J3)-ORDERMIN(J3))*1.2D0/MAX(1,(NORDERGRID-1))
         PRINT '(I8,3G20.10)',J3,ORDERMIN(J3),ORDERMAX(J3),OGRIDINT(J3)
      ENDDO
      DO J1=1,NQBINS
         IF (NMINQBIN(J1).NE.0) THEN
            MEANORDERQBIN(J1,1:NORDER)=MEANORDERQBIN(J1,1:NORDER)/NMINQBIN(J1)
            MEANAQBIN(J1,1:NORDER-1)=MEANAQBIN(J1,1:NORDER-1)/NMINQBIN(J1)
            PRINT '(A,I8,A,I8,A,10G15.4)',' # order parameter entries in quench bin ',J1,' is ', &
  &                                   NMINQBIN(J1),' mean values: ',MEANORDERQBIN(J1,1:NORDER)
            PRINT '(A,I8,A,I8,A,10G15.4)',' # order parameter entries in quench bin ',J1,' is ', &
  &                                   NMINQBIN(J1),' mean A     : ',MEANAQBIN(J1,1:NORDER-1)
            DO J2=1,NORDER-1
               MEANAQBIN(J1,J2)=MAX(MEANAQBIN(J1,J2),1.0D-50) ! set a minimum value for MEANAQBIN
!              IF (MEANAQBIN(J1,J2).LT.1.0D-50) THEN
!                 PRINT '(A,2I8,G20.10)','ERROR mean A value is too small *** J1,J2,MEANAQBIN(J1,J2)=',J1,J2,MEANAQBIN(J1,J2)
!                 STOP
!              ENDIF
            ENDDO
!        ELSE
...
! IF ((PTSTART(1:2).NE.'na').AND.(BSPTFINISH(1:3).NE.'all')) CALL EQGM(NREP, NBINS, NQBINS, PTSTART, MERGEQ)
...
DO J1=NREPMIN,NREP
   ...
   ENDIF
   READ(1) VISITSALL(1:NBINSSAVE,1:NQBINSSAVE)
   CLOSE(1)
   NDUMMY2=0
   DO J2=1,NBINS    ! J2 labels the PE bin that we quenched from
      VTOT2(1:NQBINS)=0
      DO J3=1,MERGEI
         NDUMMY3=0
         NDUMMY2=NDUMMY2+1
...
!              READ(1,*) DUMMY,DUMMY
               NDUMMY3=NDUMMY3+1
               VTOT=VTOT+VISITSALL(NDUMMY2,NDUMMY3)
            ENDDO
            VTOT2(J4)=VTOT
         ENDDO
!         DO J5=MERGEQ*NQBINS,NQBINSAVE-1 ! in case MERGEQ is not an integer divisor of NQBINSAVE
!!           READ(1,*) DUMMYSTRING
!         ENDDO
      ENDDO
      VISITS2(J1,J2,1:NQBINS)=VTOT2(1:NQBINS)
   ENDDO
   IF (BSSTART(1:2).NE.'na') THEN
!     READ(2) VISITST(1:NBINS,1:NQBINS)
!     VISITS2(J1,1:NBINS,1:NQBINS)=VISITS2(J1,1:NBINS,1:NQBINS)-VISITST(1:NBINS,1:NQBINS)
      READ(2) VISITSALL(1:NBINSSAVE,1:NQBINSSAVE)
      CLOSE(2)
      NDUMMY2=0
      DO J2=1,NBINS    ! J2 labels the PE bin that we quenched from
         VTOT2(1:NQBINS)=0
...
!            ENDDO
         ENDDO
         VISITS2(J1,J2,1:NQBINS)=VISITS2(J1,J2,1:NQBINS)-VTOT2(1:NQBINS)
      ENDDO
   ENDIF
ENDDO
MAXVISITS(1:NREP)=-1.0D0
      ...
      DO J2=NREPMIN,NREP
         IF (VISITS2(J2,J3,J4).GT.0) THEN
            IF (ALLZERO2(J3,J4)) THEN
               ...
               VARMAP2(J3,J4)=NEVAR
!              IF (J4.EQ.575) THEN PRINT '(A,I8,A,I8)','pe bin=',J3,' variable=',NEVAR
               CYCLE loop2
            ENDIF
         ENDIF
      ENDDO 
...
DO J1=1,NBINS
   ...
   DO J2=1,NQBINS
      ...
      NCOUNT=NCOUNT+1
!     IF (J2.EQ.575) PRINT '(A,3I8)','J1,J2,NCOUNT=',J1,J2,NCOUNT
...
!     IF (J2.EQ.575) THEN
!        PRINT '(A,I6,A,G20.10,A,I8,A,G20.10,2(A,I8))','BESTVAR entry ',NCOUNT,' is ',BESTVAR(NCOUNT),' for pe bin ',J1, &
!  &  ' using EVAR=', EVAR(J1),' NDUMMY2=',NDUMMY2,' NDUMMY3=',NDUMMY3
!     ENDIF
...
!        IF (J4.EQ.575) PRINT '(A,3I8,2G20.10)',' pe bin, q bin, NCOUNT, BESTVAR, OLDVAR=',J1,J4,NCOUNT,BESTVAR(NCOUNT),OLDVAR(NCOUNT)
...
!
EVAR(1:NBINS)=0.0D0
OPEN(UNIT=19,FILE='weights.2D.C')
DO J2=1,NQBINS             ! J2 labels the PE bin we quench to
    IF (ALLZEROQ(J2)) THEN ! no fit for this q bin - use direct visits only
       DO J1=1,NBINS       ! J1 labels the PE bin we quenched from
          ...
          WRITE(19,'(2I8,3G20.10)') J2, J1, QENERGY(J2), ENERGY(J1), OLDVAR(NCOUNT)
       ENDDO
  ...
  &                                           *LOG(ENERGY(J1)-QENERGY(J2))+FITB(J2))
             WRITE(19,'(2I8,3G20.10)') J2, J1, QENERGY(J2), ENERGY(J1), (KFAC+(ENERGY(J1)-QENERGY(J2))*EXP(FITA(J2))) &
  &                                           *LOG(ENERGY(J1)-QENERGY(J2))+FITB(J2)
          ENDIF
       ENDDO
...
!         IF ((SUMVISITS(J1,J2).GT.0).AND.(ENERGY(J1)-QENERGY(J2).GT.0.0D0)) THEN
  ...
  &                                           *LOG(ENERGY(J1)-QENERGY(J2))+FITB(J2))
          ...
          NCOUNT=VARMAP2(J1,J2)
!
! Is this fraction one when the above if block is not used?
!
          EVAR(J1)=EVAR(J1)+FRACTION*EXP(OLDVAR(NCOUNT))

          IF (ENERGY(J1)-QENERGY(J2).GT.0.0D0) THEN
             WRITE(19,'(2I8,3G20.10)') J2, J1, QENERGY(J2), ENERGY(J1), &
  &                          LOG( (1.0D0-FRACTION)*EXP((KFAC+(ENERGY(J1)-QENERGY(J2))*EXP(FITA(J2))) &
  &                                           *LOG(ENERGY(J1)-QENERGY(J2))+FITB(J2)) &
  &                               +FRACTION*EXP(OLDVAR(NCOUNT)) )
          ELSE                    
             WRITE(19,'(2I8,3G20.10)') J2, J1, QENERGY(J2), ENERGY(J1), &
  &                          LOG( FRACTION*EXP(OLDVAR(NCOUNT)) )
          ENDIF
...
DO J2=1,NQBINS
   ...
   IF ((QE.LT.USEQBINS).AND.(NMINQBIN(J2).NE.0)) THEN
      PRINT '(A,I8,2(A,G20.10))','resetting q bin energy ',J2,' from ',QENERGY(J2),' to ',MEANEQBIN(J2)
      QENERGY(J2)=MEANEQBIN(J2)
   ENDIF
ENDDO
HSUM=0.0D0
BSUM=0.0D0
...
!
EVAR(1:NBINS)=0.0D0
OPEN(UNIT=19,FILE='weights.2D.D')
QHISTINT=QENERGY(2)-QENERGY(1) ! in case we didn't set this before
OPEN(UNIT=1,FILE='anharmonic.fit',STATUS='UNKNOWN')
DO J2=1,NQBINS             ! J2 labels the PE bin we quench to
   IF (FITA(J2).NE.0.0D0) THEN
      WRITE(1,'(5G20.10)') QENERGY(J2),QENERGY(J2)-QHISTINT/2.0D0,QENERGY(J2)+QHISTINT/2.0D0,FITA(J2),FITB(J2)
   ENDIF
!
! Next block was causing artefacts for LJ31, but seems to be needed for LJ75.
...
!        value from direct visits seems to work.
!
   ...
   &                              *LOG(ENERGY(J1)-QENERGY(J2))+FITB(J2))
         IF (DUMMY.LT.RATMAX) WRITE(19,'(2I8,3G20.10)') J2, J1, QENERGY(J2), ENERGY(J1), &
   &                             (KFAC+(ENERGY(J1)-QENERGY(J2))*EXP(FITA(J2))) &
   &                              *LOG(ENERGY(J1)-QENERGY(J2))+FITB(J2)
      ENDDO
...
IF (MAKEORDER) THEN
   OPEN(UNIT=1,FILE='order.out',STATUS='UNKNOWN')
   OPEN(UNIT=2,FILE='Landau.out',STATUS='UNKNOWN')
!
...
!
   ...
   ENDDO
!
! Need to repeat the loops above where we caluclate the final omega(V^I_i).
! Now we need a Boltzmann weight to make P(q,T) for each q bin and temperature.
! For each order parameter grid point we sum over q bin, multiplying P(q,T) by a Gaussian containing the mean order parameter value
! and sum to get P(x,T), the probability of observing order parameter value x at temperature T.
!
! For each T we calculate P(q,T) over the q values first.
!
   ALLOCATE(PQT(NQBINS),ONORM(NORDER),OGRID(NORDER))
   DO JTEMP=1,NTEMP
      PQT(1:NQBINS)=0.0D0
      TEMPERATURE=TMIN+(JTEMP-1)*TINT
         ...
         IF ((QENERGY(J2).LT.USEQBINS).AND.(NMINQBIN(J2).NE.0)) THEN ! don't try to use direct visits even if we have some
            DO J1=1,NBINS ! J1 labels the PE bin
               ...
               ELSE ! we are above BESTBINS and there were no direct visits
                  DUMMY=HUGE(1.0D0)
               ENDIF
               IF (DUMMY.LT.RATMAX) PQT(J2)=PQT(J2)+EXP((KFAC+(ENERGY(J1)-QENERGY(J2))*EXP(FITA(J2))) &
   &                                    *LOG(ENERGY(J1)-QENERGY(J2))+FITB(J2)-(ENERGY(J1)-EREF)/TEMPERATURE)
            ENDDO
         ELSEIF (ALLZEROQ(J2)) THEN ! no quenches or fit for this q bin - use direct visits only
            DO J1=1,NBINS ! J1 labels the PE bin we quenched from
               IF (ALLZERO2(J1,J2)) CYCLE
               NCOUNT=VARMAP2(J1,J2)
               PQT(J2)=PQT(J2)+EXP(OLDVAR(NCOUNT)-(ENERGY(J1)-EREF)/TEMPERATURE)
            ENDDO
         ELSE ! use a combination of extrapolated or direct visit results
            DO J1=1,LOWESTDIRECT(J2)-1 ! interpolated values only available from fit to quenches from
                                       ! higher energy PE bins
               IF (ENERGY(J1)-QENERGY(J2).GT.0.0D0) THEN
                  PQT(J2)=PQT(J2)+EXP((KFAC+(ENERGY(J1)-QENERGY(J2))*EXP(FITA(J2))) &
  &                                             *LOG(ENERGY(J1)-QENERGY(J2))+FITB(J2)-(ENERGY(J1)-EREF)/TEMPERATURE)
               ENDIF
            ENDDO
            DO J1=LOWESTDIRECT(J2),BESTBINS(J2) ! range used for fit
               FRACTION=( (J1*1.0D0-LOWESTDIRECT(J2)*1.0D0)/(BESTBINS(J2)*1.0D0-LOWESTDIRECT(J2)*1.0D0) )
               IF (ENERGY(J1)-QENERGY(J2).GT.0.0D0) THEN 
                  PQT(J2)=PQT(J2)+(1.0D0-FRACTION)*EXP((KFAC+(ENERGY(J1)-QENERGY(J2))*EXP(FITA(J2))) &
  &                                              *LOG(ENERGY(J1)-QENERGY(J2))+FITB(J2)-(ENERGY(J1)-EREF)/TEMPERATURE)
               ENDIF
               NCOUNT=VARMAP2(J1,J2)
               PQT(J2)=PQT(J2)+FRACTION*EXP(OLDVAR(NCOUNT)-(ENERGY(J1)-EREF)/TEMPERATURE)
            ENDDO
            DO J1=BESTBINS(J2)+1,NBINS ! use direct visits above range of fit if available
               IF (ALLZERO2(J1,J2)) CYCLE
               NCOUNT=VARMAP2(J1,J2)
         ...
         ENDIF
      ENDDO
      PRINT '(A,F12.4)','Probabilities > 0.00001 of seeing a configuration that quenches to quench bin q at temperature ',TEMPERATURE
!
! Normalise P(q,T) for temperature TEMPERATURE
!
      DUMMY=0.0D0
      DO J2=1,NQBINS
         DUMMY=DUMMY+PQT(J2)
      ENDDO
      PQT(1:NQBINS)=PQT(1:NQBINS)/DUMMY
      DO J2=1,NQBINS
         IF (PQT(J2).GT.1.0D-5) PRINT '(I6,G20.10)', J2,PQT(J2)
      ENDDO
!
! In the normalisation we need to multiply by the spacing OGRIDINT(J3) for a rough quadrature.
!
      ONORM(1:NORDER)=0.0D0
      DO J1=1,NORDERGRID
         OVALUES(1:NORDER)=0.0D0
!
! For V^Q_q we already have the probability - it is just P_q(T) obtained by summing the
! 2D density of states over i bins for a given q (J4 index here).
!
         DO J3=1,NORDER
            OGRID(J3)=ORDERMIN(J3)-0.1D0*(ORDERMAX(J3)-ORDERMIN(J3)) + (J1-1) * OGRIDINT(J3)
!
! Note temperature dependence in Gaussians from CPL Frontiers paper.
!
            qbinloop: DO J4=1,NQBINS
               IF (NMINQBIN(J4).EQ.0) CYCLE qbinloop
               NDUMMY=0
               IF (J3.EQ.1) THEN
                  IF (ABS(OGRID(J3)-QENERGY(J4)).LT.OGRIDINT(J3)/2.0D0) THEN
                     OVALUES(J3)=OVALUES(J3)+PQT(J4)
                     NDUMMY=NDUMMY+1
                  ENDIF
               ELSE
!                 IF (MEANAQBIN(J4,J3-1).EQ.0.0D0) THEN ! exponential -> unity at X = X_alpha
!                    IF (ABS(OGRID(J3)-MEANORDERQBIN(J4,J3)).LT.OGRIDINT(J3)/2.0D0) THEN
!                       OVALUES(J3)=OVALUES(J3)+PQT(J4)*2.506628275D0*SQRT(TEMPERATURE)/OGRIDINT(J3)
!                    ENDIF
!                 ELSE 
!                    PRINT '(A,3I8,3G20.10)','J1,J3,J4,NMINQBIN,MEANORDERQBIN,MEANAQBIN=', &
! &                                           J1,J3,J4,NMINQBIN(J4),MEANORDERQBIN(J4,J3),MEANAQBIN(J4,J3-1)
                     OVALUES(J3)=OVALUES(J3)+ &
  &                    EXP(-(OGRID(J3)-MEANORDERQBIN(J4,J3))**2/(2*TEMPERATURE*MEANAQBIN(J4,J3-1)))*PQT(J4)/SQRT(MEANAQBIN(J4,J3-1))
                  ENDIF
!              ENDIF
            ENDDO qbinloop
            IF (J3.EQ.1) THEN
               IF (NDUMMY.GT.0) OVALUES(1)=OVALUES(1)/NDUMMY
            ELSE
               OVALUES(J3)=OVALUES(J3)*OGRIDINT(J3)/(2.506628275D0*SQRT(TEMPERATURE))
            ENDIF
! to avoid numbers with negative exponents that Mathematica can't read!
            IF (ABS(OVALUES(J3)).LT.1.0D-50) OVALUES(J3)=0.0D0 
            ONORM(J3)=ONORM(J3)+OVALUES(J3)
         ENDDO
         WRITE(1,'(20G18.8)') TEMPERATURE,(OGRID(J3),OVALUES(J3),J3=1,NORDER)
         WRITE(2,'(20G18.8)') TEMPERATURE,(OGRID(J3),-TEMPERATURE*LOG(MAX(1.0D-100,OVALUES(J3))),J3=1,NORDER)
      ENDDO
      PRINT '(A,F12.4)','Normalisation for order parameter distribution P(x,T) over quench bins at temperature ',TEMPERATURE
      PRINT '(10G20.10)',ONORM(1:NORDER)
   ENDDO ! end loop over temperatures
   DEALLOCATE(PQT,ONORM)
   CLOSE(1)
   CLOSE(2)
ENDIF
...
!
SUBROUTINE EQGM(NREP, NBINS, NQBINS, PTSTART, MERGEQ)
IMPLICIT NONE
INTEGER NREP, NBINS, NQBINS, NINC, NAVAIL, NDUMMY, J1, NGM, J2, J3, NGM2, J4, MERGEQ
CHARACTER(LEN=80) PTSTART, FSTRING2, DSTRING, DUMMYSTRING
DOUBLE PRECISION, ALLOCATABLE :: QENERGY(:)
DOUBLE PRECISION NORM, NORM2, DUMMY
...
!  NDUMMY=NINC+10000*(J2-1)
   ...
   DO J1=1,NREP
      ...
      DO J3=1,NQBINS
         ...
         DO J4=1,MERGEQ
            READ(1,*) DUMMY,NDUMMY
            IF (J4.EQ.1) QENERGY(J3)=DUMMY
            QVISITS(J3,J1,J2)=QVISITS(J3,J1,J2)+NDUMMY
         ENDDO
      ENDDO
      CLOSE(1)
   ENDDO
...
INTEGER VARIABLE2,NAVAIL,J2,MOSTVISITED(NQBINS)
...
DOUBLE PRECISION FITA(NQBINS),FITB(NQBINS),FITC(NQBINS),RMS,DIFF,CHI2PLUS,CHI2MINUS, KFAC
...
IF (NAVAIL.LT.MAX(MINFIT,3)) THEN
   PRINT '(A,I8,A,I8)','Insufficient data points for fit: ',NAVAIL,' total visits=',NDUMMY
   ALLZEROQ(J1)=.TRUE.
   PRINT '(A)',' '
   RETURN
ENDIF
...
PRINT '(A,I8,A,2I8,A,I8)','quench bin ',J1,' lowest and highest pe bins quenching here: ', &
  &   LOWESTDIRECT(J1),HIGHESTDIRECT(J1),' total visits=',NDUMMY
...
!  IF ((J1.GT.570).AND.(J1.LT.580)) THEN
!     WRITE(FNAME,'(I8)') NAVAIL
!     WRITE(FNAME2,'(I8)') J1
!     FNAME='fit.' // TRIM(ADJUSTL(FNAME2)) // '.' // TRIM(ADJUSTL(FNAME))
!     OPEN(UNIT=1,FILE=FNAME,STATUS='UNKNOWN')
!     DO J2=LOWESTDIRECT(J1),MAXBIN
!        IF (SUMVISITS(J2,J1).GT.0) THEN
!           NCOUNT=VARMAP2(J2,J1)
!           PRINT '(A,4I8,2G20.10)',' pe bin, q bin, NCOUNT, OLDVAR=',J2,J1,NCOUNT,VARMAP2(J2,J1),OLDVAR(NCOUNT)
...
ELSE
  ...
  &    BESTAVAIL, '/', BESTBINS(J1)-LOWESTDIRECT(J1)+1,' bins'
   PRINT '(A,I8,A,F20.1)','most visited pe bin for this q bin=',MOSTVISITED(J1),' visits=',SUMVISITS(MOSTVISITED(J1),J1)
ENDIF
PRINT '(A)',' '
FITA(J1)=BESTA
FITB(J1)=BESTB
...
! & J2,ENERGY(J2),ENERGY(1),TEMPERATURE-(ENERGY(J2)-EREF)/TEMPERATURE
  ...
  &                      - (Z1/(Z0*TEMPERATURE))**2 + Z2/(Z0*TEMPERATURE**2)
   ELSE
      PRINT '(A,3G20.10,A)','WARNING *** Z0,Z1,Z2=',Z0,Z1,Z2,' is RATMAX parameter too small?'
   ENDIF
ENDDO
