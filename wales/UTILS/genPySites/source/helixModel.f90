      SUBROUTINE HELIXMODEL(iErr,nP,nC,nE,lPARAMS,lESIZE,lEPOS,lERMAT,lCPOS,lCRMAT)
!This subroutine populates the data arrays according to a helical model.
!There are nE ellipsoids in the cluster arranged in nS sheets with nL layers per cluster. 
...
!The positions and orientations of a sequence of clusters is then computed such that the clusters are spread uniformly over a helix and rotated about the z-axis so that each ellipsoid in the cluster points towards the central helical axis
...
!margin defined by the variable spacer. avoids overlaps and excessive forces.
      tMax=DBLE(nC*nL-1)*spacer/SQRT(R*R+C*C)+tMin
...
!Design the cluster. The ellipsoids position and orientation is given relative to the lab frame origin and basis.
      ...
      DO curC=1,nC,1
         ...
         !Only need to output the z-coordinate because cluster is defined relative to the helix axis along which x=y=0
         lCPOS(curC,1)=0
         lCPOS(curC,2)=0
