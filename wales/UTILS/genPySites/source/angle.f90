SUBROUTINE ANGLE(iErr,nP,nC,nE,lPARAMS,lESIZE,lEPOS,lERMAT,lCPOS,lCRMAT)
     ...
     END DO
      ...
      DO curC=1,nC,1
        ...
        !Set up a sequence of the three euler angles over the parameter curC - defines the orientation of the cluster at each point on the space curve
         ca=0!DBLE(0)!(-pi/DBLE(2.0)) !(DBLE(curC)- DBLE(1))*DBLE(2.0)*pi/(DBLE(nC) - DBLE(1.0))
         cb=0!DBLE(0)!(DBLE(curC)- DBLE(1))*DBLE(2.0)*pi/(DBLE(nC) - DBLE(1.0))
         cg=0!DBLE(0)!-cb !DBLE(pi/DBLE(2.0)) !(DBLE(curC)- DBLE(1))*DBLE(2.0)*pi/(DBLE(nC) - DBLE(1.0))
