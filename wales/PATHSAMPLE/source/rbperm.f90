...
!     ----------------------------------------------------------------------------------------------
      ...
      SUBROUTINE RBMINPERMDIST(COORDSB,COORDSA,DISTANCE,DIST2,QBEST,RMATBEST,DEBUG,BOXLX,BOXLY,BOXLZ,BULKT)

!     returns DISTANCE as the actual distance, rather than the squared distance

      USE COMMONS, ONLY : NATOMS, NRBSITES, NTSITES, NRBGROUP, PAPT, RBSITE, RBOPS, EFIELDT, RBSYMT
      USE COMMONS,ONLY : NPERMGROUP, NPERMSIZE, PERMGROUP, NSETS, SETS, GEOMDIFFTOL, MULTISITEPYT
...
!     do not call minpermdistrbCOM for gravity field      
      IF(EFIELDT.AND.MULTISITEPYT) GOTO 100
      ...
      IF (SQRT(DISTANCE) <= GEOMDIFFTOL) THEN
         ...
         IF (DEBUG) PRINT '(A)',' psrbpermdist> minpermdistrbcom suggests identical'
...
!     Site-site distance
      ...
      IF (ABS(SQRT(XDUMMY)-SQRT(DISTANCE)).GT.GEOMDIFFTOL) THEN
          PRINT '(2(A,F20.10))','psrbminpermdist> ERROR *** distance between transformed XBEST and COORDSB=',  &
...
!      IF (DISTANCE <= GEOMDIFFTOL .AND. PAPT) THEN
         CMAX = 0.0D0; CMAY = 0.0D0; CMAZ = 0.0D0
         CMBX = 0.0D0; CMBY = 0.0D0; CMBZ = 0.0D0
         DO J1 = 1, NATOMS/2
...
!     GENERATE SITE POSITIONS FROM THE RIGID-BODY COORDINATES
      ...
      USE COMMONS, ONLY: NATOMS, NTSITES, NRBSITES, RBSITE, DBPTDT
...
!     IF (DBPTDT) CALL DEFTD(RBSITETD,SIGTD,SIGDB)
      IF (DBPTDT) CALL DEFTD(RBSITETD) ! bug fix DJW 5/5/2013
...
!     ----------------------------------------------------------------------------------------------
      ...
      USE COMMONS, ONLY: NRBSITES, NTSITES, EFIELDT, MULTISITEPYT      
!                        
...
!     about the origin
      USE COMMONS, ONLY : EFIELDT, MULTISITEPYT
