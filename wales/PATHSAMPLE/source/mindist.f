...
C
      ...
      USE COMMONS,ONLY : FREEZE, PULLT, GTHOMSONT, GTHOMMET, NOTRANSROTT ! hk286
      IMPLICIT NONE
...
C
! hk286
      IF ((.NOT.FREEZE).AND.(.NOT.GTHOMSONT) .AND. (.NOT. NOTRANSROTT)) THEN
         do ig=1,3
            rg=0.d0
            rg0=0.d0
            do i=1,nsize
...
C           WRITE(*,'(A,2F15.5,A,I6,A,I6)') 'Initial and final distances:',DIST0,DIST,' iterations=',ITER,' NCOUNT=',NCOUNT
            ...
            ENDDO
            ! hk286
            IF (.NOT.(NOTRANSROTT.OR.TWOD.OR.PULLT.OR.(GTHOMSONT.AND.(GTHOMMET<5)))) P(1)=2*(DPRAND()-0.5D0)/10.0D0
            IF (.NOT.(NOTRANSROTT.OR.TWOD.OR.PULLT.OR.(GTHOMSONT.AND.(GTHOMMET<5)))) P(2)=2*(DPRAND()-0.5D0)/10.0D0
...
C    1              RA(3*(NATOMS/2+J1-1)+1),RA(3*(NATOMS/2+J1-1)+2),RA(3*(NATOMS/2+J1-1)+3)
      ...
      IF (PRESERVET .OR. NOTRANSROTT) RB(1:3*NATOMS)=RBSAVE(1:3*NATOMS)
...
C
      ...
      USE COMMONS, ONLY : PULLT, TWOD, GTHOMSONT, GTHOMMET, NOTRANSROTT ! hk286
      use porfuncs
      IMPLICIT NONE
...
C
      ...
      ! hk286
      IF (NOTRANSROTT.OR.TWOD.OR.PULLT.OR.(GTHOMSONT.AND.(GTHOMMET<5))) GSAVE(1)=0.0D0
      IF (NOTRANSROTT.OR.TWOD.OR.PULLT.OR.(GTHOMSONT.AND.(GTHOMMET<5))) GSAVE(2)=0.0D0
...
C
      ...
      ENDIF
      ! hk286
      IF (NOTRANSROTT.OR.TWOD.OR.PULLT.OR.(GTHOMSONT.AND.(GTHOMMET<5))) W(ISPT+POINT*N+1)=0.0D0
      IF (NOTRANSROTT.OR.TWOD.OR.PULLT.OR.(GTHOMSONT.AND.(GTHOMMET<5))) W(ISPT+POINT*N+2)=0.0D0
