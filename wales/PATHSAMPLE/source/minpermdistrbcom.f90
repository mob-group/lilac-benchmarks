...
!     DISTANCE returns the squared distance
      ...
      USE COMMONS,ONLY : NATOMS, NPERMGROUP, NPERMSIZE, PERMGROUP, NSETS, SETS, GEOMDIFFTOL, EFIELDT, &
     &               NRBGROUP, NOINVERSION
...
!      DOUBLE PRECISION   :: XA(3*NATOMS*NRBSITES/2),  XB(3*NATOMS*NRBSITES/2), XBS(3*NATOMS*NRBSITES/2)
      ...
      DOUBLE PRECISION   :: COORDSAS(3*NATOMS), COORDSBS(3*NATOMS), T(3*NATOMS), ITX, ITY, ITZ, PREVDIST
...
!     ----------------------------------------------------------------------------------------------
      ...
      DO J1 = 1, 3*NATOMS
         DISTANCE = DISTANCE + (DUMMYA(J1) - DUMMYB(J1))**2
      ENDDO
      IF(ABS(PREVDIST-DISTANCE)<1.0D-6) THEN
!     distance did not improve, stop looping
        NCHOOSE1=NORBIT1
        NCHOOSE2=NORBIT2     
      END IF
      PREVDIST=DISTANCE
...
!      IF (DEBUG) PRINT '(A,G20.10)',' minpermdistrbcom> after initial call to RBSITESORIENT distance=',SQRT(DISTANCE),NCHOOSE1,NCHOOSE2,DBEST
      IF (DEBUG) WRITE(*,*) ' minpermdistrbcom> after initial call to RBSITESORIENT distance=',SQRT(DISTANCE),NCHOOSE1,NCHOOSE2,DBEST,NATOMS
!
...
!
      ...
      IF (EFIELDT.OR.NOINVERSION) GOTO 50
