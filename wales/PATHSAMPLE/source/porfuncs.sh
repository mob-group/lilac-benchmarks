...
# C bindings for gfortran
if test "x${FC}" == "xgfortran"; then :
  echo " "
  echo "     interface"
  echo "       function gfortran_fork() bind(C, name=\"fork\")"
  echo "         use iso_c_binding, only: c_int"
  echo "         integer(c_int) :: gfortran_fork"
  echo "       end function gfortran_fork"
  echo " "
  echo "       function gfortran_wait(ExitStatus) bind(C, name=\"wait\")"
  echo "         use iso_c_binding, only: c_int"
  echo "         integer(c_int) :: gfortran_wait"
  echo "         integer(c_int), intent(out) :: ExitStatus"
  echo "       end function gfortran_wait"
  echo "     end interface"
  echo " "
fi
...
if test "x${FC}" == "xifort"; then :
   ...
   echo "               pid=gfortran_wait(ExitStatus)"
   echo "               ExitStatus=ishft(ExitStatus,-8)"
elif test "x${FC}" == "xnag"; then :
