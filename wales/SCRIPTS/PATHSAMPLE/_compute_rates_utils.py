...
#            v, u = uv
    ...
    def compute_committors(self):
        ...
        if self.right_side.size == 1:
            ...
            #committors, res = scipy.sparse.linalg.minres(self.matrix, self.right_side)
            committors, res = scipy.sparse.linalg.lgmres(self.matrix, self.right_side)
...
class MfptLinalgSparse(object):
    ...
    def compute_mfpt(self, use_umfpack=True):
        ...
        #times = scipy.sparse.linalg.factorized(self.matrix)(-np.ones(self.matrix.shape[0]))
        #times = scipy.sparse.linalg.bicgstab(self.matrix, -np.ones(self.matrix.shape[0]))
        #times, res = scipy.sparse.linalg.minres(self.matrix, -np.ones(self.matrix.shape[0]))
        times, res = scipy.sparse.linalg.lgmres(self.matrix, -np.ones(self.matrix.shape[0]))
