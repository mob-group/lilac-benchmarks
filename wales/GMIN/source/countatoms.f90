...
!
! grand canonical basin-hopping or GBH:
!
    IF (WORD.EQ.'GCBH') THEN
       ...
       !GOTO 191 ! removed by ds656 to allow complete scan of file
    ELSE IF (WORD.EQ.'GBH') THEN
       GBHT=.TRUE.
       WRITE(MYUNIT,'(A)') 'countatoms> Detected keyword GBH.'
    ENDIF
...
191 CONTINUE
    CLOSE(LUNIT)
    !
    ! ds656> Now start the atom counting...
    !
    YESNO=.FALSE.
    YESNOA=.FALSE.
    YESNOAMH=.FALSE.
    ...
    INQUIRE(FILE='conf_initiale.pdb',EXIST=YESNOOPEP)
    ...
    IF (YESNO) THEN
       ...
       IF ( NPAR .GT. 1 .AND. .NOT. GBHT) THEN 
...
! -------------------------
  ...
  SUBROUTINE COUNT_SPECIES_FROM_DATAFILE()
    ...
    DATALOOP: DO ! Loop through lines in data file
       ...
       IF(WORD.EQ.'BLJCLUSTER_NOCUT'.OR.WORD.EQ.'BGUPTAT'.OR.&
            WORD.EQ.'GTHOMSONBIN') THEN
          J=2
          EXIT DATALOOP
