...
USE COMMONS,ONLY : MYUNIT, MULLERBROWNT, BOXLX, BOXLY, BOXLZ, STOCKT, CSMT, MKTRAPT, FASTOVERLAPT, BNB_ALIGNT, NFREEZE
IMPLICIT NONE
...
IF (RIGIDBODY) THEN
   ...
   RETURN
ELSEIF (MKTRAPT.OR.(NFREEZE.GT.0)) THEN
   DIST=0.0D0
   DO J1=1,3*NATOMS
...
IF ((FASTOVERLAPT.OR.BNB_ALIGNT) .AND. BULKT) THEN   ! Make this default for BULKT?
! mg542 superimposing COM means that adding relative displacement is meaningless - so don't do it
! Just put the coordinates back in the box?
   XA(1:3*NATOMS)=RA(1:3*NATOMS)
   XB(1:3*NATOMS)=RB(1:3*NATOMS)
   DO J1=1,NSIZE
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!mg542!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   IF (FASTOVERLAPT.OR.BNB_ALIGNT) THEN  ! Make this the default?
      DO J1=1,NSIZE
