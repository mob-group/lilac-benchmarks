...
!
! We never want to move atoms if we are doing PERMOPT. Bug fix 4/10/17 DJW
!
      IF (PERMOPT.OR.PERMINVOPT.OR.DISTOPT.OR.ALIGNT) RETURN 
      IF ((PERMOPT.OR.PERMINVOPT.OR.DISTOPT.OR.ALIGNT).AND.PERIODIC) RETURN 
      IF ((PERMOPT.OR.PERMINVOPT.OR.ALIGNT).AND.MKTRAPT) RETURN 
      ...
      IF (MLQT.OR.MLPVB3T.OR.MLP3T.OR.ORBITALS) THEN
         DO J1=1,NATOMS
            IF (FROZEN(J1)) CYCLE
            RANDOM=(DPRAND()-0.5D0)*2.0D0
            COORDS(J1,NP)=COORDS(J1,NP)+STEP(NP)*RANDOM
         ENDDO
         RETURN
      ENDIF
!
...
!     IF (CENT.AND.(.NOT.SEEDT)) CALL CENTRE2(COORDS(1:3*NATOMS,NP)) ! COORDS might have been shifted by symmetry
     ...
     &   .AND.(.NOT.PERMINVOPT).AND.(.NOT.QCIPOTT).AND.(.NOT.MLP3T).AND.(.NOT.MLQT).AND.(.NOT.MKTRAPT).AND.(.NOT.MLPB3T)
     &   .AND.(.NOT.GAUSST).AND.(.NOT.(CSMT.AND.(.NOT.SYMMETRIZECSM))).AND.(.NOT.PERCOLATET).AND.(.NOT.ORBITALS)
     &   .AND.(.NOT.MLPVB3T)) THEN
!
!	csw34> CHECK NOTHING HAS MOVED OUTSIDE THE CONTAINER RADIUS 
!
         DO J1=1,NATOMS
            IF ((.NOT.(RIGID.OR.RIGIDINIT)).OR.(J1.LE.NATOMS/2)) THEN
               J2=3*J1
...
12       RANDOM=(DPRAND()-0.5D0)*2.0D0
         ...
         RETURN
      ELSE IF (PERMOPT.OR.PERMINVOPT.OR.ALIGNT.OR.(CSMT.AND.(.NOT.SYMMETRIZECSM))) THEN
...
!    &                                    ASTEP(NP),VMIN,ASTEP(NP)*VMIN
     ...
     &         (.NOT.DIFFRACTT).AND.(.NOT.GAUSST).AND.(.NOT.PERCOLATET).AND.(.NOT.MLPVB3T).AND.(.NOT.ORBITALS)
     &        .AND.(.NOT.NORESET).AND.(.NOT.PERIODIC).AND.(.NOT.THOMSONT).AND.(.NOT.ONEDAPBCT).AND.(.NOT.ONEDPBCT)
     &      .AND.(.NOT.TWODPBCT).AND.(.NOT.THREEDAPBCT).AND.(.NOT.THREEDPBCT).AND.(.NOT.QCIPOTT).AND.(.NOT.MLP3T)
     &      .AND.(.NOT.MLPB3T)
     &       .AND.(.NOT.MLQT).AND.(.NOT.TWODAPBCT).AND.(.NOT.((NCORE(NP).GT.0).AND.(J1.GT.NATOMS-NCORE(NP))))
     &       .AND.(.NOT.RIGIDINIT)) THEN
...
!
           IF ((.NOT.PERIODIC).AND.(.NOT.AMBERT).AND.(.NOT.(RIGID.AND.((J1.GT.NATOMS/2)))).AND.(.NOT.BLNT).AND.(.NOT.MLP3T)
     1     .AND.(.NOT.PERCOLATET).AND.(.NOT.DIFFRACTT).AND.(.NOT.THOMSONT).AND.(.NOT.GAUSST).AND.(.NOT.QCIPOTT).AND.(.NOT.ORBITALS)
     2     .AND.(.NOT.MKTRAPT).AND.(.NOT.MLPB3T).AND.(.NOT.MLQT).AND.(.NOT.MLPVB3T)) THEN
