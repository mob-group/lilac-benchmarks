...
USE COMMONS, ONLY : RMS, DEBUG, CUDATIMET, MAXBFGS, MAXERISE, CUDAPOT, NPCALL, COLDFUSION, COLDFUSIONLIMIT, MYUNIT, DGUESS, &
                    ...
                    AMBER12T, LJADD3T, NADDTARGET, LJADDREP, LJADDATT
...
INTERFACE
    SUBROUTINE CUDA_LBFGS(N, C_XCOORDS, EPS, C_MFLAG, C_ENERGY, ITMAX, C_ITDONE, C_MAXBFGS, C_MAXERISE, C_RMS, C_CUDAPOT, & 
                         ...
                         PROJECT, C_FREEZE, C_FROZEN, C_NFREEZE, POTENTIALTIME, C_AACONVERGENCET, C_NADDTARGET, C_LJADDREP, & 
                         C_LJADDATT) BIND(C,NAME="setup_lbfgs")
...
INTERFACE
    SUBROUTINE CUDA_ENEGRAD_CPUTOGPU(NATOMS, COORDS, C_TOTENERGY, C_GRADIENTS, C_NADDTARGET, C_LJADDREP, C_LJADDATT, C_CUDAPOT, & 
        ...
        ! Need Fortran compiler support for Technical Specification 29113 to pass allocatable arrays across ISO_C_BINDING interface
        ! Currently only available for very newest compilers, but this should be done in future for LJADDREP/LJADATT so that large 
        ! arrays reside on the heap rather than the stack
        REAL(KIND=C_DOUBLE), DIMENSION(C_NADDTARGET, C_NADDTARGET), INTENT(IN) ::C_LJADDREP, C_LJADDATT ! Repulsive/attractive epsilon matrix
...
CONTAINS
    ...
    SUBROUTINE CUDA_LBFGS_WRAPPER(N, MUPDATE, XCOORDS, EPS, MFLAG, ENERGY, ITMAX, ITDONE, RESET)
...
#ifndef DUMMY_CUDA
        INTEGER(KIND=C_INT) :: N, MUPDATE, ITMAX, C_ITDONE, ECALLS, C_DEGFREEDOMS, C_NRIGIDBODY, C_MAXSITE, C_NFREEZE, C_NADDTARGET
        ...
        REAL(KIND=C_DOUBLE), DIMENSION(1) :: C_LJADDREP, C_LJADDATT
        ...
        IF ((SEEDT.AND.FREEZECORE) .OR. STEEREDMINT .OR. QUENCHDOS .OR. CALCQT .OR. INTMINT .OR. COMPRESSRIGIDT) THEN
            WRITE(MYUNIT,'(A)') "modcudalbfgs> Keyword SEED with FREEZECORE is not yet supported. SEED can be used with NOFREEZE. &
                                 Keywords STEEREDMIN, QUENCHDOS, CALCQ, INTMIN, COMPRESSRIGID are not yet supported. & 
    ...
    SUBROUTINE CUDA_ENEGRAD_WRAPPER(NATOMS, COORDS, TOTENERGY, GRADIENTS)
        ...
        REAL(KIND=C_DOUBLE), DIMENSION(NADDTARGET*NADDTARGET) :: C_LJADDREP, C_LJADDATT
#else /* ifdef DUMMY_CUDA */
        REAL(KIND=C_DOUBLE), DIMENSION(1) :: COORDS, C_GRADIENTS
        REAL(KIND=C_DOUBLE), DIMENSION(1, 1) :: C_LJADDREP, C_LJADDATT
#endif
    ...
    SUBROUTINE CUDA_NUMERICAL_HESS(NATOMS, COORDS, HESSIAN, DELTA)
        ...
        INTEGER(KIND=C_INT) :: NATOMS, C_NADDTARGET
        REAL(KIND=C_DOUBLE) :: C_ENERGY, POTENTIALTIME
        ...
        DOUBLE PRECISION  :: MYTSTART, MYTFINISH
        ...
        CALL CPU_TIME(MYTSTART)
        DO I = 1, 3*NATOMS
            ! Plus
            COORDS(I) = COORDS(I) + DELTA
            CALL CUDA_ENEGRAD_CPUTOGPU(3*NATOMS, COORDS, C_ENERGY, C_GRADIENTS, C_NADDTARGET, C_LJADDREP, C_LJADDATT, C_CUDAPOT, & 
                                      C_CUDATIMET, POTENTIALTIME)
            NPCALL = NPCALL + 1
        ...
        END DO
        CALL MYCPU_TIME(MYTFINISH,.FALSE.)
        MYTFINISH=MYTFINISH-MYTSTART
        WRITE(MYUNIT,'(A,F15.2)') ' Numerical hessian elapsed time = ',MYTFINISH
#endif
