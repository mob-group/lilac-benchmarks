...
C
!      WRITE(MYUNIT,'(A,12E15.7)') 'EREAL,ECONV,QMIN',EREAL,ECONV,(QMIN(J1),J1=1,NSAVE)
      DO J1=1,NSAVE
...
20          CONTINUE
            ...
            IF (NSAVE.GT.1) THEN
               ...
               ENDDO
               ! dj337: update saved box parameters
               if (boxderivt) boxq(j2,1:6) = boxq(j2-1,1:6)
      ...
      IF (TARGET) THEN ! beh35
         DO J1=1,NTARGETS
            IF (EREAL-TARGETS(J1).LT.ECONV) THEN
               IF (NPAR.LT.2) THEN
                  WRITE(MYUNIT,'(2(A,I15),A)') 'saveit> Target hit after ',NQ(NP),' quenches ',NPCALL,' function calls.'
