...
14       CONTINUE
      ...
      ELSEIF (.NOT.(AMBER12T.OR.AMBERT.OR.AMBER.OR.CPMD.OR.CHRMMT.OR.DMACRYST.OR.USERPOTT.OR.OPEPT)) THEN
...
!        ENDIF
!        NATOMS=NATOMS/NPAR
         IF (PERMOPT.OR.PERMINVOPT.OR.ALIGNT) THEN
...
!           REWIND(7) ! this seems to be needed now?
            DO JP=1,NPAR
               ...
               ENDDO
            ENDDO
         ELSEIF (MLP3T.OR.MLPVB3T) THEN ! for this neural net it is one variable per line
            REWIND(COORDS_UNIT)
            DO JP=1,NPAR
               DO J1=1,NATOMS
                   READ(COORDS_UNIT,*) COORDS(J1,JP)
               ENDDO
            ENDDO
         ELSEIF (MLQT) THEN ! for this ML it is one variable per line
            REWIND(COORDS_UNIT)
            DO JP=1,NPAR
               DO J1=1,NATOMS
                   READ(COORDS_UNIT,*) COORDS(J1,JP)
               ENDDO
            ENDDO
         ELSEIF (ORBITALS) THEN ! for this orbital landscape it is one variable per line
            REWIND(COORDS_UNIT)
            DO JP=1,NPAR
               DO J1=1,NATOMS
                   READ(COORDS_UNIT,*) COORDS(J1,JP)
...
20       FORMAT('Initial coordinates:')
         ...
         ELSEIF (MLP3T.OR.MLPVB3T) THEN 
            WRITE(MYUNIT,'(G20.10)') (COORDS(J1,MYNODE+1),J1=1,NATOMS)
         ELSEIF (MLQT) THEN 
            WRITE(MYUNIT,'(G20.10)') (COORDS(J1,MYNODE+1),J1=1,NATOMS)
...
!<gd351|
      ...
      ELSE IF (PYGPERIODICT) THEN
         ...
         IF  (PYADDT.OR.PYADD2T) THEN
            IF (SORTT) THEN
               WRITE(MYUNIT,'(A)') 'Turning off SORT option for PYADD'
               SORTT=.FALSE.
            ENDIF
            WRITE(MYUNIT,'(I4,A)') NATOMS/2,' addressable ellipsoids'
         ELSE
...
!bs360         WRITE(MYUNIT, '(A,2F12.4)') 'Maximum and minimum number of twists are ',CHNMAX,CHNMIN
      ...
      ELSEIF (MKTRAPT) THEN
         WRITE(MYUNIT,'(I4,A)') NATOMS,' MK trapped ions'
      ELSEIF (DJWRBT) THEN
         IF (DJWRBID.EQ.1) THEN
            WRITE(MYUNIT,'(3(I4,A))') NATOMS,' sites for ',NRIGIDBODY,' rigid bodies - DJW potential ',DJWRBID
            WRITE(MYUNIT,'(2(I4,A))') NRIGIDBODY-NHEXAMERS,' pentamers and ',NHEXAMERS,' hexamers'
            WRITE(MYUNIT,'(A,4G20.10)') 'rho, eps, sigma and pentamer radius=',CAPSIDRHO,CAPSIDEPS,SIGMAPENT,RADPENT
            WRITE(MYUNIT,'(A,4G20.10)') 'hexamer sigma, radius and hex/pent sigma=',SIGMAHEX,RADHEX,SIGMAPH
         ENDIF
      ELSEIF (MLPVB3T) THEN
         IF (MLPVB3NNT) THEN
            DO J1=1,NATOMS
               IF (FROZEN(J1)) COORDS(J1,1)=0.0D0 ! for frozen zero nearest-neighbour setup
            ENDDO
         ENDIF
         WRITE(MYUNIT,'(I4,A)') NATOMS,' link weights for MLPVB3'
      ELSEIF (MLP3T) THEN
         WRITE(MYUNIT,'(I4,A)') NATOMS,' link weights for MLP3'
      ELSEIF (MLQT) THEN
         WRITE(MYUNIT,'(I4,A)') NATOMS,' variables for ML quadratic'
      ELSEIF (ORBITALS) THEN
         WRITE(MYUNIT,'(I4,A)') NATOMS,' rotations for orbital landscape'
      ELSEIF  (LJADDT) THEN
         IF (SORTT) THEN
            WRITE(MYUNIT,'(A)') 'Turning off SORT option for LJADD'
            SORTT=.FALSE.
         ENDIF
         WRITE(MYUNIT,'(I4,A)') NATOMS,' LJ addressable atoms'
      ELSE
     ...
     &                        SHELLPROB
      ...
      IF (FREEZE) THEN
         IF (MLQT.OR.MLPVB3T.OR.MLP3T.OR.ORBITALS) THEN
            WRITE(MYUNIT,'(A,I6,A)') 'io1> ', NFREEZE,' variables will be frozen:'
         ELSE
...
!    &                    0.677441D0-0.0037582*NATOMS+9.40318D-6*NATOMS**2-6.21931D-9*NATOMS**3,' to give ',STEP(1)
     ...
     &                    .AND.(.NOT.QCIPOTT).AND.(.NOT.INTCONSTRAINTT).AND.(.NOT.MLP3T).AND.(.NOT.MKTRAPT).AND.(.NOT.MLQT)
     &                    .AND.(.NOT.MLPVB3T).AND.(.NOT.ORBITALS)) 
