...
contains
    ...
    subroutine pairwise_py(sitei, sitej, grad_contrib, energy_contrib, gtest, t_rep, t_att)
        ...
        double precision :: dU_dF, dU_drmod, torquei(6), torquej(6), t_att(6), t_rep(6)
        ...
        call compute_py_values(elli, ellj, gtest, g, g_pm1, g_pm2, g_pm6, F_pm1_2, dg_dF, dg_dr, dg_drmod, torquei, torquej)
        if(paramonovcutoff .and. above_cutoff) then
            ...
            if(gtest) then
                ...
                t_rep(1:3)=24*pyepsnot*sitei%rep%strength*torquei(1:3)
                t_rep(4:6)=24*pyepsnot*sitej%rep%strength*torquej(1:3)
            end if
        end if
        ...
        if(paramonovcutoff .and. above_cutoff) then
            ...
            if(gtest) then
                ...
                g_att(4: 6) = -g_att(1: 3)
                t_att(1:3)=12*pyepsnot*sitei%att%strength*torquei(4:6)
                t_att(4:6)=12*pyepsnot*sitej%att%strength*torquej(4:6)
            end if
        end if
    ...
    subroutine compute_py_values(elli, ellj, gtest, g, g_pm1, g_pm2, g_pm6, F_pm1_2, dg_dF, dg_dr, dg_drmod, torquei, torquej)
        ...
        double precision, intent(out) :: torquei(6), torquej(6)
        integer :: i
...
! torques (PY eq C11)
            torquei(1:3)=-2.d0*g_pm6*g_pm6*g_pm1*dg_dF*(cross(xc_minus_ri,elli%dF_dr))
            torquei(4:6)=g_pm6*g_pm1*dg_dF*(cross(xc_minus_ri,elli%dF_dr))
            torquej(1:3)=-2.d0*g_pm6*g_pm6*g_pm1*dg_dF*(cross(xc_minus_rj,ellj%dF_dr))
            torquej(4:6)=g_pm6*g_pm1*dg_dF*(cross(xc_minus_rj,ellj%dF_dr))
...
END FUNCTION cross
    ...
    subroutine compute_contact(elli, ellj, lambda_c, F)
        ...
        integer :: image_list_offset, xyz, o1, o2, image_num, i
        ...
        ellj%r(:) = ellj%site%r(:)
        rij(:) = elli%r(:) - ellj%r(:)
        elli%mat_dot_r(:) = matmul(elli%shapemat, elli%r) ! seems that introducing this line solves the discontinuity for PBCs!
        ellj%mat_dot_r(:) = matmul(ellj%shapemat, ellj%r)
!                    write(*,*) 'initial distances', rij(:)

        if(paramonovpbcx .or. paramonovpbcy .or. paramonovpbcz) then
            ! set up the pbc matrix
!        WRITE(*,*) 'in compute_contact, boxlx=', boxlx, pcutoff
            pbc_values(:) = 0.d0
            if(paramonovpbcx) pbc_values(1) = boxlx
            if(paramonovpbcy) pbc_values(2) = boxly
            if(paramonovpbcz) pbc_values(3) = boxlz
            do xyz = 1, 3
                if(pbc_values(xyz) /= 0.d0 .and. abs(rij(xyz)) > pbc_values(xyz) / 2.d0) then
!                    write(*,*) 'distance, box length',xyz, rij(xyz), pbc_values(xyz)
...
!        write(*,*) 'before update', r(1), nint(r(1) / boxlx)
        ...
        mol%r(:) = r(:)
!        write(*,*) 'after update', r(1), nint(r(1) / boxlx)
...
subroutine py(x, grad, energy, gtest)
    use commons, only: natoms, vt, frozen, efieldt
    use py_module
    implicit none
    logical, intent(in) :: gtest
    ...
    type(py_site), pointer :: sitei, sitej
    integer :: num_mols
    integer :: moli_index, molj_index, sitei_index, sitej_index, i1
    double precision :: energy_contrib, grad_contrib(12), energy_contrib2, grad_contrib2
    double precision :: t_rep(6), t_att(6)
        ...
        do molj_index = moli_index + 1, num_mols
            ...
            do sitei_index = 1, size(moli%sites)
                ...
                do sitej_index = 1, size(molj%sites)
                    ...
                    call pairwise_py(sitei, sitej, grad_contrib, energy_contrib, gtest, t_rep, t_att)
...
use commons , only : pygravityc1,pygravityc2
implicit none
    ...
    do moli_index = 1, num_mols
        ...
        if(.not. percolatet) then
            if(vec_len(moli%r) > radius) then
                write(myunit, *) 'py_takestep> initial coord outside container, bringing in'
