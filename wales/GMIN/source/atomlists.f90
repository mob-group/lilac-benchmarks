...
!
SUBROUTINE RESET_LABELS(IP)
  !
  USE COMMONS, ONLY : LABELS, NATOMS, NATOMSALLOC, NSPECIES, &
       SPECMASST, SPECMASS, ATMASS, MYUNIT
  !
  IMPLICIT NONE
  !
  ...
  ! Sanity check
  IF(NATOMS /= K) THEN
     WRITE(MYUNIT, '(A)') 'reset_labels> Inconsistent atom count!'
     !WRITE(MYUNIT, *) NATOMS, K, NSPECIES(0)
     STOP
  ENDIF
  !
...
!
SUBROUTINE SET_LABELS(L,IP)
  !
  USE COMMONS, ONLY : LABELS, NATOMS, NSPECIES, &
       SPECMASST, SPECMASS, ATMASS, MYUNIT, LFLIPST, QALCSMODE
  !
  IMPLICIT NONE
  !
  INTEGER, INTENT(IN) :: L(NATOMS), IP
  INTEGER :: I,TYPECOUNTS(1:NSPECIES(0))
  !
  TYPECOUNTS(:) = 0
  !
  DO I=1,NATOMS
     LABELS(I,IP) = L(I)
     TYPECOUNTS(L(I)) = TYPECOUNTS(L(I)) + 1
     IF(SPECMASST) ATMASS(I) = SPECMASS(L(I))
  ENDDO
  !
  DO I=1,NSPECIES(0)
     IF(LFLIPST.OR.(QALCSMODE.GE.6)) THEN
     ...
     ELSE IF(NSPECIES(I) /= TYPECOUNTS(I)) THEN
        WRITE(MYUNIT,'(A, I2, A, I5, I5)') &
             'set_labels> Inconsistent counts for atom-type ', &
             I,' ,these numbers should equal:', NSPECIES(I), &
             TYPECOUNTS(I)
        STOP
     ENDIF
...
!
SUBROUTINE RESET_STOICHIOMETRY(IP)
  !
  ! ds656> 31/07/2015
  ! Reset stoichiometry to the initial value, with the labels 
  ! distributed randomly.
  !
  USE COMMONS, ONLY : NATOMS, MYUNIT, NSPECIES_INI, LABELS
  !
  IMPLICIT NONE
  !
  INTEGER, INTENT(IN) :: IP
  ...
  ENDDO
  !
  CALL KNUTH_SHUFFLE(NATOMS, LABELS(1:NATOMS,IP))
  CALL SET_ATOMLISTS(LABELS(1:NATOMS,IP), 1) ! put all in group 1
  !
  RETURN
  !
