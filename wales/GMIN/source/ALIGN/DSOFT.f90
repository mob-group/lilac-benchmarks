...
IMPLICIT NONE
INTEGER(KIND=INT64), INTENT(IN) :: BANDWIDTH
...
INTEGER(KIND=INT64), INTENT(IN) :: J,M1,M2
...
INTEGER(KIND=INT64) I,J,M1,M2,IND1,IND2,MAXM
...
DO I=1,2*BANDWIDTH
    ...
    COSB2(I) = COS(BETA/2)
    SINB2(I) = SIN(BETA/2)
    SINCOSB2(I) = SINB2(I)*COSB2(I)
    SINDIVCOSB2(I) = SINB2(I)/COSB2(I)
ENDDO 
...
COMPLEX(KIND=REAL64), INTENT(OUT) :: OUTPUT(BANDWIDTH, 2*BANDWIDTH-1, 2*BANDWIDTH-1)

! INCLUDE "fftw3.f90"
COMPLEX(KIND=REAL64) IN1D(2*BANDWIDTH), OUT1D(2*BANDWIDTH), TEMP(2*BANDWIDTH, 2*BANDWIDTH, 2*BANDWIDTH)
INTEGER(KIND=INT64) PLAN, K1,K2,K3,M1,M2,I1,I2,IND1,IND2,J,MAXM
...
DO K1=1,2*BANDWIDTH
    DO K2=1,2*BANDWIDTH
        DO K3=1,2*BANDWIDTH
            IN1D(K3) = CMPLX(INPUT(K3,K2,K1),0.D0, REAL64)
        ENDDO
...
INTEGER(KIND=INT64), INTENT(IN) :: BANDWIDTH
COMPLEX(KIND=REAL64), INTENT(IN) :: INPUT(BANDWIDTH, 2*BANDWIDTH-1, 2*BANDWIDTH-1)
COMPLEX(KIND=REAL64), INTENT(OUT) :: OUTPUT(2*BANDWIDTH,2*BANDWIDTH,2*BANDWIDTH)
