...
SUBROUTINE FINALIO
  ...
  USE AMBER12_INTERFACE_MOD, ONLY : AMBER12_FINISH, AMBER12_WRITE_RESTART, AMBER12_WRITE_PDB, &
       AMBER12_WRITE_XYZ
  USE AMBER12_MUTATIONS, ONLY : FINISH_AMBERMUT
  USE OPEP_INTERFACE_MOD, ONLY: OPEP_FINISH, OPEP_WRITE_PDB
  USE QMODULE
  USE MODCHARMM
  USE AMHGLOBALS, ONLY:NMRES,IRES
  USE BGUPMOD
  USE PERMU
  USE MODHESS, ONLY : HESS, MASSWT
  USE CONVEX_POLYHEDRA_MODULE, ONLY: VIEW_POLYHEDRA
  USE LJ_GAUSS_MOD, ONLY: VIEW_LJ_GAUSS
  USE OPP_MOD, ONLY: VIEW_OPP
  USE ORBITALS_MOD, ONLY: ORBITALS_FINISH

  IMPLICIT NONE

  !   MCP
  INTEGER III, I3,  GLY_COUNT, ID, NUMCRD, NUMPRO, NCPHST, GETUNIT, AMHUNIT1, LUNIT, NSYMOPS, NCLOSE
  INTEGER J1, J2, J3, J4, J5, MYUNIT2, I1, NDUMMY, MYUNIT3, NRBS1, NRBS2, LJGOUNIT, MJ2, REORDERUNIT
  DOUBLE PRECISION RBCOORDS(NRBSITES*3), DCOORDS(3*NATOMS), EDUMMY, ITDET, DIST
  !
  !ds656> Symmetry detection for clusters on a substrate...
  INTEGER :: ISITES(MIEF_NSITES), NSITES, COMBO_N
  DOUBLE PRECISION :: DISTS(MIEF_NSITES)
  INTEGER, ALLOCATABLE :: COMBO_LABELS(:)
  DOUBLE PRECISION, ALLOCATABLE :: COMBO_COORDS(:)
  !<ds656
    !
  DOUBLE PRECISION P(3), DU(3), RMI(3,3), DRMI(3,3), PI, PHI, THT
...
!  CHARACTER(LEN=2)  DUMMYSTR
  ...
  DOUBLE PRECISION TEND, DMIN
!  INTEGER QDONE, NP, ITERATIONS, CLOSEST(NADDTARGET), BRUN, MYUNIT4
!  DOUBLE PRECISION SCREENC(3*NATOMSALLOC), TIME
  CHARACTER(LEN=2) :: ALPHABET(26)=(/'ja','jb','jc','jd','je','jf','jg','jh','ji','jj','jk','jl', &
  &                                  'jm','jn','jo','jp','jq','jr','js','jt','ju','jv','jw','jx','jy','jz'/)

  ! ds656> Extra stuff for HSA in OPTIM's min.data format
  INTEGER :: NUM_ZERO_EVS, INFO
  DOUBLE PRECISION :: LOG_PROD, EVALS(3*NATOMS), WORK(9*NATOMS)
...
!
! This block gave nonesense in the lowest file. Removed 4/10/17 DJW
!
! IF (PERMOPT) THEN
!    NSAVE=2
!    QMIN(2)=0.0D0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  IF (LJADD3T) THEN
     LJGOUNIT=GETUNIT()
     OPEN(LJGOUNIT,FILE='ljadd3.xyz',STATUS='UNKNOWN')
  ENDIF
  IF (REORDERADDT) THEN
     REORDERUNIT=GETUNIT()
     OPEN(REORDERUNIT,FILE='reorder.xyz',STATUS='UNKNOWN')
  ENDIF
  ...
  savemin: DO J1=1,NSAVE
     ...
     ELSE IF (.NOT. PRINT_MINDATA) THEN
        WRITE(MYUNIT2,*) NATOMS
        !ds656> With PRINT_MINDATA we want to discard "minima"
        ! with negative eigenvalues, and so we print the number
        ! of atoms after checking the Hessian.
     ENDIF
     !        IF (CSMT.AND.DEBUG) WRITE(MYUNIT,'(A,I6,2G20.10)') 'finalio> J1,QMIN,QMINAV=',J1,QMIN(J1),QMINAV(J1)
     !
     ! ds656> Either append point group or print stuff for HSA
     IF(PRINT_MINDATA.OR.PRINT_PTGRP) THEN
        !
        WRITE(MYUNIT,'(A,I5,A)') &
             'finalio> ---| Post-processing minimum ',J1,' |-----------'
        !
        ...
        !
        IF(MIEFT.AND.NSYMOPS>1) THEN
           !
           ! Sort field sites by distance from the cluster's
           ! system of symmetry (CM).
           DO J4=1,MIEF_NSITES
              ISITES(J4) = J4
              DISTS(J4) = 0.0
              DO J5=1,3
                 DISTS(J4) = DISTS(J4) + &
                      (MIEF_SITES(J4,J5)-CM(J5))**2
              ENDDO
              DISTS(J4) = DSQRT(DISTS(J4))
              J5=J4
              sort_sites: DO WHILE(J5>1)
                 IF(DISTS(J5) < DISTS(J5-1)) THEN
                    NDUMMY = ISITES(J5)
                    ISITES(J5) = ISITES(J5-1)
                    ISITES(J5-1) = NDUMMY
                    EDUMMY = DISTS(J5)
                    DISTS(J5) = DISTS(J5-1)
                    DISTS(J5-1) = EDUMMY
           ...
           ENDDO
           !
           !ds656> Extract a small subset of field sites.
           ! While coding this I had graphene-like lattice in
           ! in mind, in which case the fragment can be small.
           ! Complications may arise for other substrates...
           J5=3
           count_sites: DO J4=4,MIEF_NSITES
              IF(ABS(DISTS(J4)-DISTS(J4-1)) < PGSYMTOLS(2)) THEN
                 J5=J5+1
              ELSE
                 EXIT count_sites
              ENDIF
           ...
           NSITES=J5
           !
           COMBO_N=NATOMS+NSITES
           IF(ALLOCATED(COMBO_COORDS)) DEALLOCATE(COMBO_COORDS)
           ALLOCATE(COMBO_COORDS(1:3*COMBO_N))
           IF(ALLOCATED(COMBO_LABELS)) DEALLOCATE(COMBO_LABELS)
           ALLOCATE(COMBO_LABELS(1:COMBO_N))
           !
           ! Set substrate labels to 0 and store coordinates
           COMBO_LABELS(1:NSITES) = 0
           DO J4=1,NSITES
              DO J5=1,3
                 COMBO_COORDS(3*(J4-1)+J5) = &
                      MIEF_SITES(ISITES(J4),J5)
              ENDDO
           ENDDO
           !
           COMBO_LABELS(NSITES+1:COMBO_N) = QMINT(J1,1:NATOMS)
           COMBO_COORDS(3*NSITES+1:3*COMBO_N) = &
                QMINP(J1,1:3*NATOMS)
           !
           WRITE(MYUNIT,'(A,I4,A)') &
                'finalio> Diagnosing point group including ', &
                NSITES,' MIEF sites.'
           !
           CALL PGSYM( COMBO_N, COMBO_COORDS(1:3*COMBO_N), &
                COMBO_LABELS(1:COMBO_N), PGSYMTOLS, 2, J3,&
                CM, NSYMOPS, SYMOPS, FPGRP )
           !
           DEALLOCATE(COMBO_COORDS, COMBO_LABELS)
           !
        ENDIF
        !
        IF(PRINT_PTGRP) THEN ! Just append point group
           WRITE(MYUNIT2,99) J1, QMIN(J1), FF(J1), &
                NPCALL_QMIN(J1), FPGRP, NSYMOPS
99         FORMAT('Energy of minimum ',I6,'=',G20.10, &
                ' first found at step ',I8,' after ', &
                I20,' function calls. Point group ',A4, &
                ' of order ', I3,'.')
        ELSE ! Print stuff for HSA in OPTIM's min.data format
           !
           NUM_ZERO_EVS=6
           IF (NATOMS.EQ.2) NUM_ZERO_EVS=5
           IF (MIEFT) NUM_ZERO_EVS=0
           !
           IF (ALLOCATED(HESS)) DEALLOCATE(HESS)
           ALLOCATE(HESS(3*NATOMS,3*NATOMS))
           IF(NSPECIES(0)>1) THEN ! ds656> this also sets ATMASS
              CALL SET_ATOMLISTS(QMINT(J1,1:NATOMS),1)
              CALL SET_LABELS(QMINT(J1,1:NATOMS),1)
           ENDIF
           CALL POTENTIAL(QMINP(J1,1:3*NATOMS), EVALS, EDUMMY, &
                .TRUE., .TRUE.) ! EVALS is dummy for gradient here
           IF(ABS(EDUMMY-QMIN(J1)) > ECONV) THEN
              WRITE(MYUNIT,'(A)') &
                   'finalio> WARNING: Unexpected energy value! Configuration changed on final requench?'
           ENDIF
           CALL MASSWT() ! Weight the Hessian by mass.
           !
           WRITE(MYUNIT, '(A)') &
                'finalio> Using DSYEV to calculate Hessian eigenvalues'
           CALL DSYEV('N','U',3*NATOMS,HESS,3*NATOMS,EVALS,&
                WORK,9*NATOMS,INFO)
           IF(INFO /= 0) THEN
              WRITE(MYUNIT,'(A,I2)') &
                   'finalio> ***WARNING*** DSYEV error status', INFO
           ENDIF
           J3=0
           WRITE(MYUNIT,'(A,I6,A)',ADVANCE='NO') &
                'finalio> ',3*NATOMS,' Hessian eigenvalues:'
           DO J2=1,3*NATOMS
              WRITE(MYUNIT,'(1X, E12.5E2)',ADVANCE='NO') EVALS(J2)
              IF(EVALS(J2) < - ABS(HESS_EIGEN_TOL)) THEN
                 WRITE(MYUNIT,*)
                 WRITE(MYUNIT,'(A)') &
                      'finalio> WARNING: -ve eigenvalue => not writing to ''lowest''!'
                 CYCLE savemin
              ELSEIF(ABS(EVALS(J2)) < ABS(HESS_EIGEN_TOL)) THEN
                 J3=J3+1
              ENDIF
           ENDDO
           WRITE(MYUNIT,*)
           IF(J3 /= NUM_ZERO_EVS) THEN
              WRITE(MYUNIT,'(A,I6,A)') &
                   'finalio> WARNING: Unexpected number of zero eigenvalues: ',J3,' => not writing to ''lowest''!'
           ELSE
              WRITE(MYUNIT,'(A,I6,A)') &
                   'finalio> Detected ', J3,' zero eigenvalues.'
           ENDIF
           ! Log prod. of frequencies is log prod. of sqrt of evals
           LOG_PROD = &
                SUM(DLOG(EVALS(NUM_ZERO_EVS+1:3*NATOMS)))
           WRITE(MYUNIT,'(A,I6,A,G20.10)') &
                'finalio> Log product of ', &
                3*NATOMS-NUM_ZERO_EVS,&
                ' non-zero eigenvalues: ', &
                LOG_PROD
           CALL MYINERTIA(QMINP(J1,1:3*NATOMS),ITDET)
           WRITE(MYUNIT,'(A,G20.10)') &
                'finalio> Determinant of inertia tensor: ', ITDET
           WRITE(MYUNIT2,*) NATOMS
           WRITE(MYUNIT2, '(2F20.10,I6,1X,E15.8E2)') &
                EDUMMY, LOG_PROD, NSYMOPS, ITDET
           !
        ENDIF
        !
     ELSE ! <ds656
...
10      FORMAT('Energy of minimum ',I6,'=',G20.10, &
        ...
        ! dj337: write cell parameters
        if (boxderivt) then
           write(myunit2, *) 'Box lengths: ', boxq(j1,1:3)
           if (.not.ortho) write(myunit2, *) 'Box angles: ', boxq(j1,4:6)
        endif
     ENDIF
     !
...
65      FORMAT('AR ',3F20.10)
     ...
     ELSE IF (GTHOMSONT) THEN
        ...
        IF (GTHOMPOT.EQ.7) THEN
           DO J2=1,NATOMS
              IF (J2.LE.GTHOMSONBINN) THEN
                 WRITE(MYUNIT2,'(A,3F20.10)') 'LA  ',(DCOORDS(3*(J2-1)+J3),J3=1,3)
              ELSE
                 WRITE(MYUNIT2,'(A,3F20.10)') 'LB  ',(DCOORDS(3*(J2-1)+J3),J3=1,3)
              ENDIF
           ENDDO
        ELSE
           DO J2=1,NATOMS
     ...
     ELSE IF (GLJT) THEN
        ...
        ENDDO
        !
     ELSE IF (BGUPTAT .AND. .NOT. QALCST .AND. .NOT. MULTIPERMT) THEN
        DO J2=1,NATOMS
           IF (J2.LE.NTYPEA) THEN
     ...
     ELSE IF (AMBER12T) THEN
        !kr366> We use MYNODE + 1 throughout, need to use the same here!
     ...
     ELSE IF (AMBERT) THEN
        ...
        IF (DUMPSTRUCTURES) THEN
           WRITE(J1CHAR2,'(I3)') J1
           IF (MPIT.OR.GBHT) THEN
              CALL AMBERFINALIO(j1,MYUNIT2,MYNODE+1,tempstring,1,COORDS1(1:3*NATOMS))
              WRITE (ISTR, '(I10)') MYNODE+1
              MYUNIT3=GETUNIT()
           ...
           ELSE
              CALL AMBERFINALIO(j1,MYUNIT2,MYNODE+1,tempstring,0,COORDS1(1:3*NATOMS))
              MYUNIT3=GETUNIT()
     ...
     ELSE IF (CAPBINT.OR.DBPT.OR.DBPTDT.OR.DMBLMT.OR.DMBLPYT.OR.LINRODT.OR.LWOTPT.OR.MSTBINT.OR.MSSTOCKT.OR.NCAPT.OR.NPAHT &
          ...
          .OR. PAPBINT .OR. PAPJANT .OR. POLYT .OR. PTSTSTT .OR. MORSEDPT) THEN
        DO J2 = 1, NATOMS/2
     ...
     ELSE IF (MLP3T.OR.MLPB3T.OR.MLQT.OR.MLPVB3T.OR.ORBITALS) THEN
        DO J2 = 1, NATOMS
           WRITE(MYUNIT2,'(3G20.10)') QMINP(J1,J2)
        ENDDO
           ...
           DO J3=1,ATOMLISTS(J2,1,0)
              ...
              IF(STRESST) THEN ! Tack on local stresses
                 ...
                 ELSEIF(STRESS_MODE==1) THEN
                    WRITE(MYUNIT2,'(3(1X,F10.5))',ADVANCE='YES') &
...
!
! Reorder the addressable particles into clusters.
!
     IF (REORDERADDT) THEN
        WRITE(REORDERUNIT,'(I10)') NATOMS
        WRITE(REORDERUNIT,10) J1, QMIN(J1), FF(J1), NPCALL_QMIN(J1)
        ADDDONE(1:NATOMS)=.FALSE.
        DO J2=1,NATOMS/NADDTARGET
           MJ2=(J2-1)*NADDTARGET+1
!           CLOSEST(1)=MJ2
           WRITE(REORDERUNIT,'(A2,1X,3G20.10)') 'LA ',QMINP(J1,3*(MJ2-1)+1),QMINP(J1,3*(MJ2-1)+2),QMINP(J1,3*(MJ2-1)+3)
           ADDDONE(MJ2)=.TRUE.
           DO J3=2,NADDTARGET
              DMIN=1.0D100
              DO J4=1,NATOMS/NADDTARGET
                 IF (ADDDONE((J4-1)*NADDTARGET+J3)) CYCLE
                 DIST=(QMINP(J1,3*(MJ2-1)+1)-QMINP(J1,3*((J4-1)*NADDTARGET+J3-1)+1))**2+ &
  &                   (QMINP(J1,3*(MJ2-1)+2)-QMINP(J1,3*((J4-1)*NADDTARGET+J3-1)+2))**2+ &
  &                   (QMINP(J1,3*(MJ2-1)+3)-QMINP(J1,3*((J4-1)*NADDTARGET+J3-1)+3))**2
                 IF (DIST.LT.DMIN) THEN
                    DMIN=DIST
                    NCLOSE=(J4-1)*NADDTARGET+J3
                 ENDIF
                 WRITE(MYUNIT,'(A,4I6,2G20.10)') 'J2,MJ2,J3,NCLOSE,DIST,DMIN=',J2,MJ2,J3,NCLOSE,DIST,DMIN
              ENDDO
!              CLOSEST(J3)=NCLOSE
              ADDDONE(NCLOSE)=.TRUE.
              WRITE(MYUNIT,'(A,4I6)') 'J2,MJ2,J3, final closest=',J2,MJ2,J3,NCLOSE
              WRITE(REORDERUNIT,'(A2,1X,3G20.10)') 'LA ',QMINP(J1,3*(NCLOSE-1)+1),QMINP(J1,3*(NCLOSE-1)+2),QMINP(J1,3*(NCLOSE-1)+3)
           ENDDO
        ENDDO
     ENDIF
     IF (LJADD3T.AND.(NADDTARGET.LE.26)) THEN
        WRITE(LJGOUNIT,'(I10)') NATOMS
        WRITE(LJGOUNIT,10) J1, QMIN(J1), FF(J1), NPCALL_QMIN(J1)
        DO J2=1,NATOMS
           MJ2=MOD(J2-1,NADDTARGET)+1
           WRITE(LJGOUNIT,'(A2,1X,3G20.10)') ALPHABET(MJ2),QMINP(J1,3*(J2-1)+1),QMINP(J1,3*(J2-1)+2),QMINP(J1,3*(J2-1)+3)
        ENDDO
     ENDIF
     !
  ENDDO savemin
  ...
  ELSE IF (DJWRBT) THEN
     IF (DJWRBID.EQ.1) THEN
        LUNIT=GETUNIT()
        OPEN(UNIT=LUNIT,FILE='GRlowest.xyz',STATUS='UNKNOWN')
        DO J1=1,NSAVE
           NDUMMY=0
           WRITE(LUNIT,'(I6)') NATOMS
           WRITE(LUNIT,10) J1, QMIN(J1), FF(J1)
           DO J2=1,NRIGIDBODY
              IF (J2.LE.NRIGIDBODY-NHEXAMERS) THEN ! pentamer
                 DO J3=1,NSITEPERBODY(J2)
                    J4=NDUMMY+J3
                    IF (J3.EQ.1) THEN
                       WRITE (LUNIT,'(A4,3F18.10,A12,3F18.10)') 'LC ',QMINP(J1,3*(J4-1)+1),QMINP(J1,3*(J4-1)+2),QMINP(J1,3*(J4-1)+3)
                    ELSEIF (J3.EQ.2) THEN
                       WRITE (LUNIT,'(A4,3F18.10,A12,3F18.10)') 'LD ',QMINP(J1,3*(J4-1)+1),QMINP(J1,3*(J4-1)+2),QMINP(J1,3*(J4-1)+3)
                    ELSE
                       WRITE (LUNIT,'(A4,3F18.10,A12,3F18.10)') 'LA ',QMINP(J1,3*(J4-1)+1),QMINP(J1,3*(J4-1)+2),QMINP(J1,3*(J4-1)+3)
                    ENDIF
                 ENDDO
              ELSE
                 DO J3=1,NSITEPERBODY(J2)
                    ...
                    ELSEIF (J3.EQ.2) THEN
                       WRITE (LUNIT,'(A4,3F18.10,A12,3F18.10)') 'LD ',QMINP(J1,3*(J4-1)+1),QMINP(J1,3*(J4-1)+2),QMINP(J1,3*(J4-1)+3)
                    ELSE
                       WRITE (LUNIT,'(A4,3F18.10,A12,3F18.10)') 'LB ',QMINP(J1,3*(J4-1)+1),QMINP(J1,3*(J4-1)+2),QMINP(J1,3*(J4-1)+3)
                    ENDIF
                 ENDDO
              ENDIF
              NDUMMY=NDUMMY+NSITEPERBODY(J2)
           ENDDO
        ENDDO
        CLOSE(LUNIT)
     ENDIF
  ...
  IF (AMBER12T.AND.(.NOT.(AMBERMUTATIONT))) THEN
     CALL AMBER12_FINISH()
  END IF

  IF (OPEPT) CALL OPEP_FINISH()

  IF (ORBITALS) CALL ORBITALS_FINISH()
