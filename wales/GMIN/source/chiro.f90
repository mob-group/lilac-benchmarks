...
! hk286
        double precision :: dli_dri(3), dli_drj(3), dlj_dri(3), dlj_drj(3)
        double precision :: dli_dpi(3), dli_dpj(3), dlj_dpi(3), dlj_dpj(3)
        ...
        if(gtest) then
            ...
            if(inti) then
                ...
                if (intj) then
                   ...
                   dli_dri = (mu_hati - mu_dot_mu * mu_hatj) / cc
                   do k = 1, 3
                      dli_dpi(k) = (2.d0 * mu_dot_mu * dot_product(mui_deriv(k, :), mu_hatj) * &
                           & (r_dot_mui - mu_dot_mu * r_dot_muj) / (cc * cc) + (dot_product(rij, mui_deriv(k, :)) - &
                           & dot_product(mui_deriv(k, :), mu_hatj) * r_dot_muj) / cc )
                      dli_dpj(k) = (2.d0 * mu_dot_mu * dot_product(mu_hati, muj_deriv(k, :)) * &
                   ...
                   dli_dri = mu_hati
                   do k = 1, 3
                      dli_dpi(k) = lambdaj * dot_product(mui_deriv(k, :), mu_hatj) + dot_product(rij, mui_deriv(k, :))
                      dli_dpj(k) = lambdaj * dot_product(mu_hati, muj_deriv(k, :))
                   enddo
                endif
                
                dd2_dri(:) = dd2_dri(:) + dd2_dl * dli_dri
                do k = 1, 3
                   dd2_dpi(k) = dd2_dpi(k) + dd2_dl * dli_dpi(k)
                   dd2_dpj(k) = dd2_dpj(k) + dd2_dl * dli_dpj(k)
                end do
             endif
            ...
            if(intj) then
                ...
                if (inti) then
                   dlj_dri = (-mu_hatj + mu_dot_mu * mu_hati) / cc
                   do k = 1, 3
                      dlj_dpi(k) = (2.d0 * mu_dot_mu * dot_product(mui_deriv(k, :), mu_hatj) * &
                        & (-r_dot_muj + mu_dot_mu * r_dot_mui) / (cc * cc) + (dot_product(mui_deriv(k, :) , mu_hatj) * &
                        & r_dot_mui + mu_dot_mu * dot_product(rij, mui_deriv(k, :))) / cc )
                      dlj_dpj(k) = (2.d0 * mu_dot_mu * dot_product(mu_hati, muj_deriv(k, :)) * &
                   ...
                   dlj_dri = - mu_hatj
                   do k = 1, 3
                      dlj_dpi(k) = lambdai * dot_product(mui_deriv(k, :), mu_hatj)
                      dlj_dpj(k) = lambdai * dot_product(mu_hati, muj_deriv(k, :)) - dot_product(rij, muj_deriv(k, :))
                   enddo
                endif
                
                dd2_dri(:) = dd2_dri(:) + dd2_dl * dlj_dri 
                do k = 1, 3
                    dd2_dpi(k) = dd2_dpi(k) + dd2_dl * dlj_dpi(k)
                    dd2_dpj(k) = dd2_dpj(k) + dd2_dl * dlj_dpj(k)
                 end do
              end if
...
subroutine chiro(x, grad, energy, gtest)
    ...
    double precision :: energy_jar, grad_jar(6)
...
! hk286
    double precision :: epsilon_penalty, delta_penalty
    integer :: power_penalty


    epsilon_penalty = 1.0d20
    delta_penalty = 1.0d-3
    power_penalty = 100
    ...
    do moli_ind = 1, nmols - 1
        ...
        do molj_ind = moli_ind + 1, nmols
            ...
            ! compute angular parts only once per set of images
                ...
                if ( 1 - mu_dot**2 <  delta_penalty ) then
                   energy_contrib = energy_contrib &
                        & + epsilon_penalty * ((1 - mu_dot**2)/delta_penalty-1)**power_penalty 

                   if(gtest) then
                      grad_pair(:) = 0.0
                      do k = 1, 3
                         grad_pair(6 + k) = -2.0d0*power_penalty*epsilon_penalty*mu_dot* &
                              & ((1 - mu_dot**2)/delta_penalty-1)**(power_penalty-1) / delta_penalty * &
                              & dot_product(moli%mu_hat_deriv(k, :), molj%mu_hat)
                         
                         grad_pair(9 + k) = -2.0d0*power_penalty*epsilon_penalty*mu_dot* &
                              & ((1 - mu_dot**2)/delta_penalty-1)**(power_penalty-1) / delta_penalty * &
                              & dot_product(moli%mu_hat, molj%mu_hat_deriv(k, :))
                      end do
                      grad_contrib(:) = grad_contrib(:) + grad_pair(:)
                   end if
                endif
