...
USE COMMONS,ONLY : NPERMGROUP, NPERMSIZE, PERMGROUP, NSETS, SETS, GEOMDIFFTOL, &
  &            NFREEZE, RBAAT, BESTPERM, LOCALPERMDIST, NTSITES, &
  &            LOCALPERMCUT, STOCKT, GMAX, INTCONSTRAINTT, &
  &            LOCALPERMNEIGH, LOCALPERMCUT2, CHRMMT, MYUNIT
IMPLICIT NONE

INTEGER, PARAMETER :: MAXIMUMTRIES=10
INTEGER NATOMS, NPERM, PATOMS, NRB, OPNUM,  NORBIT1, NORBIT2, NCHOOSE2, NCHOOSE1, NTRIES, NORBITB1, NORBITB2, NDMEAN
...
LOGICAL DEBUG, TWOD, RIGID, BULKT, PITEST, AOK, BOK, ADDED, PERMUTABLE(NATOMS)
...
!        WRITE(MYUNIT,'(A)') ' lopermdist> No more atoms within cutoff'
         GOTO 91
      ENDIF
...
!     IF (DEBUG) WRITE(MYUNIT,'(2(A,I6))') ' lopermdist> Atom ',DLIST(NOTHER),' belongs to permutable set ', &
...
!           IF (DEBUG) WRITE(MYUNIT,'(A,I6)') ' lopermdist> Adding partner atom ',DLIST(NOTHER)
         ELSE
            WRITE(MYUNIT,'(A,I6,A)') ' lopermdist> ERROR *** Partner atom ',DLIST(NOTHER),' has already been tried'
            STOP
         ENDIF
      ENDDO
   ENDIF
...
!  WRITE(MYUNIT,'(A,I6,3G20.10)') 'J1,LDBEST(J1),LDISTANCE=',J1,LDBEST(J1),LDISTANCE

   LDISTANCE=LDISTANCE
   DO J2=1,PATOMS
      IF (LPERM(J2).GT.PATOMS) THEN
         LDISTANCE=1.0D300
!        IF (DEBUG) WRITE(MYUINT,'(A,I6,A,I6,A)') ' lopermdist> For group ',J1,' with ',NOTHER,' neighbours - neighbours mix in' 
         EXIT
      ENDIF
   ENDDO

!  IF (J1.EQ.16) THEN
!  PRINT '(I6)',PATOMS+NOTHER
!  PRINT '(A,8I6,G20.10)',' PDUMMYB for NO1,NO2,NC1,NC2,NOB1,NOB2,NCB1,NCB2,distance ', &
! &                                     NORBIT1,NORBIT2,NCHOOSE1,NCHOOSE2,NORBITB1,NORBITB2,NCHOOSEB1,NCHOOSEB2,SQRT(LDISTANCE)
!  PRINT '(A,3G20.10)',('LA ',PDUMMYB(3*(J2-1)+1:3*(J2-1)+3),J2=1,PATOMS+NOTHER)
!  PRINT '(I6)',PATOMS+NOTHER
!  PRINT '(A,8I6,G20.10)',' PDUMMYA for NO1,NO2,NC1,NC2,NOB1,NOB2,NCB1,NCB2,distance ', &
! &                                     NORBIT1,NORBIT2,NCHOOSE1,NCHOOSE2,NORBITB1,NORBITB2,NCHOOSEB1,NCHOOSEB2,SQRT(LDISTANCE)
!  PRINT '(A,3G20.10)',('LA ',PDUMMYA(3*(J2-1)+1:3*(J2-1)+3),J2=1,PATOMS+NOTHER)
!  ENDIF

   DO J2=1,NOTHER
      IF (LPERM(PATOMS+J2).NE.PATOMS+J2) THEN
!        IF (DEBUG) PRINT '(A,I6,A)',' lopermdist> Atom ',DLIST(J2),' also needs to permute'
...
!        ENDIF
      ...
      GOTO 71
   ELSE
!     IF (DEBUG) PRINT '(A,F12.2,3(A,I6))',' lopermdist> Best distance ',SQRT(LDBESTATOM), &
...
91 CONTINUE ! jump here when there are no atoms left to try.
   ...
   IF (DEBUG) WRITE(MYUNIT,'(2(A,I6),A,G15.5)') ' lopermdist> For group ',J1,' maximum neighbours=', &
...
!     WRITE(MYUNIT,'(2(A,I6))') ' lopermdist> Atom ',NEWPERM(PERMGROUP(NDUMMY+LPERMBEST(J2)-1)), &
...
!  PRINT '(A,I6,2(A,F20.10))',' lopermdist> For group ',J1,' after myorient distance=',SQRT(LDBEST(J1)),' total=',DSUM
...
DISTANCE=DSUM
IF (DEBUG) WRITE(MYUNIT,'(A,G20.10)') ' lopermdist> After myorient block sum of distances=',DISTANCE
!
...
!  IF (DEBUG) WRITE(MYUNIT,'(A,2G20.10)') & 
...
IF (DEBUG) WRITE(MYUNIT,'(A,G20.10)') ' lopermdist> after overall alignment distance=',DISTANCE
