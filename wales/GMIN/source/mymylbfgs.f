...
!hk286 - introduce XRIGIDCOORDS
     ...
     1                 X1, Y1, Z1, X2, Y2, Z2, TRY(3*NATOMS), D1, D2, RBCOORDS(18), DUMMY2, DIST, DIST1
      ...
      ! dj337: add variables for box derivatives
      DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: BOXDIAG, BOXW
      DOUBLE PRECISION :: YSBOX, YYBOX, SQBOX, YRBOX, BETABOX
      INTEGER :: NBOX, ISPTBOX, IYPTBOX, INMCBOX, IYCNBOX, ISCNBOX, NPTBOX
      DOUBLE PRECISION :: WTEMPBOX(3*6), DUMMYBOX, SLENGTHBOX
      DOUBLE PRECISION :: BOX_PARAMSSAVE(6), BOX_GRADSAVE(6), BOX_GRADNEW(6)
!     DOUBLE PRECISION DIAG(N), W(N*(2*M+1)+2*M)
      ...
      double precision xfracgrad(3*natoms)
      ...
      LOGICAL FTEST
      COMMON /FAIL/ FTEST
      SAVE W, DIAG, ITER, POINT, ISPT, IYPT, NPT, BOXW, BOXDIAG, ISPTBOX, IYPTBOX, NPTBOX
      LOGICAL RESETLBFGS
...
!
      ...
      ! dj337: allocate box parameter variables
      IF (BOXDERIVT) THEN
         ! optimize 3 lengths if orthorhombic
         IF (ORTHO) THEN
            NBOX = 3
         ! optimize 3 lengths + 3 angles if triclinic
         ELSE
            NBOX = 6
         ENDIF
         IF (.NOT.ALLOCATED(BOXDIAG)) ALLOCATE(BOXDIAG(NBOX))
         IF (.NOT.ALLOCATED(BOXW)) ALLOCATE(BOXW(NBOX*(2*M+1)+2*M))
      ENDIF
...
!
         ...
         ELSE
            ...
            IF (BOXDERIVT) THEN
               DO J1 = 1, NBOX
                  DUMMY = -BOX_PARAMSGRAD(J1)*BOXDIAG(J1)
                  BOXW(ISPTBOX+J1) = DUMMY
                  BOXW(J1) = DUMMY
               ENDDO
               GNORM = DSQRT(DDOT(N,GRAD,1,GRAD,1)+DDOT(NBOX,BOX_PARAMSGRAD(1:NBOX),1,BOX_PARAMSGRAD(1:NBOX),1))
            ELSE
...
!
      ...
      ELSE 
         ...
         IF (BOXDERIVT) THEN
            YSBOX = DDOT(NBOX,BOXW(IYPTBOX+NPTBOX+1),1,BOXW(ISPTBOX+NPTBOX+1),1)
         ENDIF
...
!              DIAG(J1)= ABS(YS/YY) ! messes up after step reversals!
            ...
            ENDDO
            IF (BOXDERIVT) THEN
               YYBOX = DDOT(NBOX,BOXW(IYPTBOX+NPTBOX+1),1,BOXW(IYPTBOX+NPTBOX+1),1)
               IF (ABS(YYBOX).LT.1.0D-50) THEN
                  YYBOX=SIGN(1.0D-50,YYBOX)
               ENDIF
               IF (ABS(YSBOX).LT.1.0D-50) THEN
                  YSBOX=SIGN(1.0D-50,YSBOX)
               ENDIF
               DO J1=1,NBOX
                  BOXDIAG(J1) = YSBOX/YYBOX
               ENDDO
            ENDIF
         ELSE
            WRITE(MYUNIT,'(A)') 'using estimate of the inverse diagonal elements'
...
!
         ...
         IF (POINT.EQ.0) CP=M
         W(N+CP)= 1.0D0/YS
         IF (BOXDERIVT) BOXW(NBOX+CP)=1.0D0/YSBOX
         IF (CHRMMT.AND.INTMINT) THEN
            DO I=1,N
               W(I)= -GINT(I)
            ENDDO
         ...
         DO J1= 1,BOUND
            ...
            IF (BOXDERIVT) THEN
               SQBOX=DDOT(NBOX,BOXW(ISPTBOX+CP*NBOX+1),1,BOXW,1)
               INMCBOX=NBOX+M+CP+1
               IYCNBOX=IYPTBOX+CP*NBOX
               BOXW(INMCBOX)=BOXW(NBOX+CP+1)*SQBOX
               CALL DAXPY(NBOX,-BOXW(INMCBOX),BOXW(IYCNBOX+1),1,BOXW,1)
            ENDIF
         ENDDO
         ...
         IF (BOXDERIVT) THEN
            DO J1=1,NBOX
               BOXW(J1) = BOXDIAG(J1)*BOXW(J1)
               IF (ABS(BOXW(J1)).GT.1.0D2) BOXW(J1)=SIGN(1.0D2,BOXW(J1))
            ENDDO
         ENDIF
         ...
         DO J1=1,BOUND
            ...
            IF (BOXDERIVT) THEN
               YRBOX=DDOT(NBOX,BOXW(IYPTBOX+CP*NBOX+1),1,BOXW,1)
               BETABOX=BOXW(NBOX+CP+1)*YRBOX
               INMCBOX=NBOX+M+CP+1
               BETABOX=BOXW(INMCBOX)-BETABOX
               ISCNBOX=ISPTBOX+CP*NBOX
               CALL DAXPY(NBOX,BETABOX,BOXW(ISCNBOX+1),1,BOXW,1)
            ENDIF
            CP=CP+1
            IF (CP.EQ.M) CP=0
         ENDDO
...
!
      IF (ITER.GT.0) THEN
         ...
         IF (BOXDERIVT) THEN
            DO J1=1,NBOX
               BOXW(ISPTBOX+POINT*NBOX+J1)=BOXW(J1)
            ENDDO
         ENDIF
      ENDIF
      ...
      ELSE
         IF (BOXDERIVT) THEN
            DOT1=SQRT(DDOT(N,GRAD,1,GRAD,1) + DDOT(NBOX,BOX_PARAMSGRAD(1:NBOX),1,BOX_PARAMSGRAD(1:NBOX),1))
         ELSE
...
!
      ...
      IF (BOXDERIVT) THEN
         DUMMYBOX = 1.0D0
         DO J1=1,NBOX
            IF (ABS(BOXW(J1)).GT.DUMMYBOX) DUMMYBOX=ABS(BOXW(J1))
         ENDDO
         DO J1=1,NBOX
            WTEMPBOX(J1)=BOXW(J1)/DUMMYBOX
         ENDDO
         DOT2=SQRT(DDOT(N,WTEMP,1,WTEMP,1)+DDOT(NBOX,WTEMPBOX,1,WTEMPBOX,1))
      ENDIF
      IF (.NOT.BOXDERIVT) DOT2=SQRT(DDOT(N,WTEMP,1,WTEMP,1))
      OVERLAP=0.0D0
      IF (DOT1*DOT2.NE.0.0D0) THEN
         ...
         ELSE
            ...
            IF (BOXDERIVT) THEN
               OVERLAP=OVERLAP+DDOT(NBOX,BOX_PARAMSGRAD(1:NBOX),1,WTEMPBOX,1)/(DOT1*DOT2)
            ENDIF
        ENDIF
      ENDIF
...
!        SLENGTH=SLENGTH+W(ISPT+POINT*N+J1)**2
      ...
      IF (BOXDERIVT) THEN
         SLENGTHBOX = 0.0D0
         DO J1=1,NBOX
            SLENGTHBOX=SLENGTHBOX+BOXW(ISPTBOX+POINT*NBOX+J1)**2
            SLENGTHBOX=SLENGTHBOX+WTEMPBOX(J1)**2
         ENDDO
         SLENGTHBOX=SQRT(SLENGTHBOX)*DUMMYBOX
      ENDIF
!
! Taking out the magnitude of the largest element can prevent overflow.
!
      SLENGTH=SQRT(SLENGTH)*DUMMY
      IF (BOXDERIVT) SLENGTH=SLENGTH+SLENGTHBOX
...
!
         ...
         ENDDO
         IF (BOXDERIVT) THEN
            BOX_PARAMSSAVE(1:NBOX)=BOX_PARAMS(1:NBOX)
            DO J1=1,NBOX
               BOX_PARAMS(J1)=BOX_PARAMS(J1)+STP*BOXW(ISPTBOX+POINT*NBOX+J1)
            ENDDO
         ENDIF
!
...
! a bit of useful debug printing         
      ...
      ! dj337: save current box grad
      IF (BOXDERIVT) BOX_GRADSAVE(1:NBOX)=BOX_PARAMSGRAD(1:NBOX)
      CALL POTENTIAL(XCOORDS,GNEW,ENEW,.TRUE.,.FALSE.)
      ! dj337: store new box grad and restore previous box grad
      IF (BOXDERIVT) THEN
         BOX_GRADNEW(1:NBOX) = BOX_PARAMSGRAD(1:NBOX)
         BOX_PARAMSGRAD(1:NBOX) = BOX_GRADSAVE(1:NBOX)
      ENDIF
      IF(RESETLBFGS) THEN
         RESETLBFGS=.FALSE.
...
!
      ...
      IF (((ENEW-ENERGY.LE.MAXERISE).OR.EVAP.OR.GUIDECHANGET.OR.SMINKCHANGET).AND.(ENEW-ENERGY.GT.MAXEFALL)) THEN
         ...
         IF (BOXDERIVT) THEN
            DO J1=1,NBOX
               BOX_PARAMSGRAD(J1) = BOX_GRADNEW(J1)
            ENDDO
         ENDIF
!fh301>{{{
...
!
!  Energy increased too much - try again with a smaller step size
!
...
!
               ...
               IF (BOXDERIVT) THEN
                  BOX_PARAMS(1:NBOX) = BOX_PARAMSSAVE(1:NBOX)
                  BOX_PARAMSGRAD(1:NBOX) = BOX_GRADNEW(1:NBOX)
               ENDIF
...
!
            ...
            IF (BOXDERIVT) THEN
               BOX_PARAMS(1:NBOX)=BOX_PARAMSSAVE(1:NBOX)
               DO J1=1,NBOX
                  BOX_PARAMS(J1)=BOX_PARAMS(J1)+0.5*STP*BOXW(ISPTBOX+POINT*NBOX+J1)
               ENDDO
            ENDIF
!           DO J1=1,N
!              XCOORDS(J1)=XCOORDS(J1)-0.5*STP*W(ISPT+POINT*N+J1)
...
!        
     ...
     1                      ' energy increased too much from ',ENERGY,' to ',ENEW,' decreasing step to ',STP*SLENGTH
...
!
            ...
            IF (BOXDERIVT) THEN
               BOX_PARAMS(1:NBOX)=BOX_PARAMSSAVE(1:NBOX)
               DO J1=1,NBOX
                  BOX_PARAMS(J1)=BOX_PARAMS(J1)+0.1D0*STP*BOXW(ISPTBOX+POINT*NBOX+J1)
               ENDDO
            ENDIF
...
!
         ...
         IF (BOXDERIVT) THEN
            DO J1=1,NBOX
               BOXW(ISPTBOX+NPTBOX+J1)=STP*BOXW(ISPTBOX+NPTBOX+J1)
               BOXW(IYPTBOX+NPTBOX+J1)=BOX_PARAMSGRAD(J1)-BOXW(J1)
               IF (ABS(BOXW(IYPTBOX+NPTBOX+J1)).GT.1.0D10) BOXW(IYPTBOX+NPTBOX+J1)=SIGN(1.0D10,BOXW(IYPTBOX+NPTBOX+J1))
            ENDDO
         ENDIF
      ENDIF
      POINT=POINT+1
