      SUBROUTINE PAHAGENRIGID (X, G, ENERGY, GTEST)

      USE COMMONS, ONLY: NATOMS, NCARBON, RBSTLA, STCHRG, RHOCC0, RHOCC10, RHOCC20, &
     &                   RHOHH0, RHOHH10, RHOHH20, RHOCH0, RHOC10H, RHOCH10, RHOC20H, RHOCH20,      &
     &                   ALPHACC, ALPHAHH, ALPHACH, DC6CC, DC6HH, DC6CH, KKJ, CCKJ

      ! dj337: PAHA adapted to the genrigid framework
      USE GENRIGID, ONLY: NRIGIDBODY, ATOMRIGIDCOORDT, TRANSFORMCTORIGID, NSITEPERBODY, &
     &                    MAXSITE, SITESRIGIDBODY, TRANSFORMRIGIDTOC

      IMPLICIT NONE

      INTEGER          :: I, J, K, J1, J2, J3, J4, J5, J6, J7, J8, OFFSET, FCT(6) 
      DOUBLE PRECISION :: X(3*NATOMS), G(3*NATOMS), XR(3*NATOMS)
      DOUBLE PRECISION :: ENERGY, R2, R6, ABSRIJ, DVDR, ENERGY1, ENERGY2, ENERGY3
      DOUBLE PRECISION :: RI(3), RSS(3), NR(3), P(3), EI(3), EJ(3), FRIJ(3), TIJ(3), TJI(3) 
      DOUBLE PRECISION :: R(MAXSITE*NRIGIDBODY,3), E(3*MAXSITE*NRIGIDBODY,3)
      ...
      !print *, 'cart: ', xr(:3*natoms)
      
      ! calculate gradient if GTEST true
      IF (GTEST) G(:) = 0.D0
...
!     CALCULATE THE DISPERSION DAMPING FACTOR
                  ...
                  ! initialize sum for the damping function
                  DMPFCT = 1.D0
...
!     NOW CALCULATE RHOAB
                  ...
                  ! ENERGY3 is energy due to electrostatic interactions
                  ENERGY3   = ENERGY3 + CCKJ*STCHRG(I)*STCHRG(J)/ABSRIJ
 
                  IF (GTEST) THEN

                     ! now DVDR is derivative of ENERGY1 + ENERGY2 wrt r_ij with factor of 1/Rab
...
!      RBSTLA(12,:) = SITE(12,:) - SITE(3,:)                 ! Z FROM C3 TO H6
      ...
      !print *, 'defined benzrigid'
      !print *, stchrg(:12)
      !print *, site(:12, :3)

      END SUBROUTINE DEFBENZENERIGID
