...
! Work out the number of chiral centres by reading the .chirality_list file 
   ...
   if (allocated(sr_atoms)) deallocate(sr_atoms)
   allocate(sr_atoms(num_chiral_centres, 5))
...
! Now calculate the chirality of the centres and save it in sr_states_initial
   if (allocated(sr_states_initial)) deallocate(sr_states_initial)
   allocate(sr_states_initial(num_chiral_centres))
   do i = 1, num_chiral_centres
...
! Work out the number of peptide bonds by reading the .cis_trans_list file 
   ...
   if (allocated(cis_trans_atoms)) deallocate(cis_trans_atoms)
   allocate(cis_trans_atoms(num_peptide_bonds, 4))
...
! Now calculate the isomerism of the peptide bonds and save it in cis_trans_states_initial
   if (allocated(cis_trans_states_initial)) deallocate(cis_trans_states_initial)
   allocate(cis_trans_states_initial(num_peptide_bonds))
   do i = 1, num_peptide_bonds
      do j = 1, 4
...
subroutine dealloc_states_mutation()
   if (allocated(sr_states)) deallocate(sr_states)
end subroutine dealloc_states_mutation
