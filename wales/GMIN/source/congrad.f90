...
!  DO J1=1,INTIMAGE+2 ! can change when zero energies are confirmed for end images
      ...
      IF (INTFROZEN(NREPI(J2)).AND.INTFROZEN(NREPJ(J2))) THEN
         WRITE(MYUNIT, '(A,I6,A,2I6)') ' congrad> ERROR *** repulsion ',J2,' between frozen atoms ',NREPI(J2),NREPJ(J2)
         STOP
      ENDIF
!     WRITE(MYUNIT,'(A,2I8,6G20.10)') 'congrad> B J1,J2,GGG(1:6)=',J1,J2,GGG(1:6)
...
!     IF (r2ax**2+r2ay**2+r2az**2+r2bx**2+r2by**2+r2bz**2-2*(r2ax*r2bx+r2ay*r2by+r2az*r2bz).EQ.0.0D0) THEN
      IF ((r1ax-r1bx-r2ax+r2bx)**2+(r1ay-r1by-r2ay+r2by)**2+(r1az-r1bz-r2az+r2bz)**2.LT.1.0D-10) THEN
!        WRITE(MYUNIT, '(A,I6,A,2I6)') 'B repulsion number ',J2, ' between ',NREPI(J2),NREPJ(J2)
...
!        WRITE(MYUNIT,'(A,7I10)') 'congrad> J2,NI1,NJ1,NI2,NJ2,NREPI,NREPJ=',J2,NI1,NJ1,NI2,NJ2,NREPI(J2),NREPJ(J2)
!        WRITE(MYUNIT,'(A,7I10)') 'frames ',J1-1,J1
      ELSE
...
! WRITE(MYUNIT,'(A,6F15.10)') 'R1AX,R1AY,R1AZ,R2AX,R2AY,R2AZ=',R1AX,R1AY,R1AZ,R2AX,R2AY,R2AZ
! WRITE(MYUNIT,'(A,6F15.10)') 'R1BX,R1BY,R1BZ,R2BX,R2BY,R2BZ=',R1BX,R1BY,R1BZ,R2BX,R2BY,R2BZ
! WRITE(MYUNIT,'(A,6F15.10)') 'DSQ2,DSQ1=',DSQ2,DSQ1
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!! DEBUG
   IF (INTFROZEN(CONI(J2)).AND.INTFROZEN(CONJ(J2))) THEN
      WRITE(MYUNIT, '(A,I6,A,2I6)') ' congrad2> ERROR *** constraint ',J2,' between frozen atoms ',CONI(J2),CONJ(J2)
      STOP
   ENDIF
!!!!!!!!!!!!!!!!!!!!!!!!!! DEBUG
   DO J1=2,INTIMAGE+2
...
!  INTCONST=NREPCUT(J2)**13
   ...
   DO J1=2,INTIMAGE+2
      ...
      ENDIF
      IF (INTFROZEN(NREPI(J2)).AND.INTFROZEN(NREPJ(J2))) THEN
         WRITE(MYUNIT, '(A,I6,A,2I6)') ' congrad2> ERROR *** repulsion ',J2,' between frozen atoms ',NREPI(J2),NREPJ(J2)
         STOP
      ENDIF
!     WRITE(MYUNIT,'(A,2I8,6G20.10)') 'congrad2> B J1,J2,GGG(1:6)=',J1,J2,GGG(1:6)
      ...
      R2BX=XYZ(NJ2+1); R2BY=XYZ(NJ2+2); R2BZ=XYZ(NJ2+3)
!     IF (r2ax**2+r2ay**2+r2az**2+r2bx**2+r2by**2+r2bz**2-2*(r2ax*r2bx+r2ay*r2by+r2az*r2bz).EQ.0.0D0) THEN
      IF ((r1ax-r1bx-r2ax+r2bx)**2+(r1ay-r1by-r2ay+r2by)**2+(r1az-r1bz-r2az+r2bz)**2.LT.1.0D-50) THEN
!        WRITE(MYUNIT, '(A,I6,A,2I6)') 'A repulsion number ',J2, ' between ',NREPI(J2),NREPJ(J2)
!        WRITE(MYUNIT, '(A,6F15.10)') 'R1AX,R1AY,R1AZ,R2AX,R2AY,R2AZ=',R1AX,R1AY,R1AZ,R2AX,R2AY,R2AZ
!        WRITE(MYUNIT, '(A,6F15.10)') 'R1BX,R1BY,R1BZ,R2BX,R2BY,R2BZ=',R1BX,R1BY,R1BZ,R2BX,R2BY,R2BZ
!        WRITE(MYUNIT,'(A,7I10)') 'congrad2> J2,NI1,NJ1,NI2,NJ2,NREPI,NREPJ=',J2,NI1,NJ1,NI2,NJ2,NREPI(J2),NREPJ(J2)
!        WRITE(MYUNIT,'(A,7I10)') 'frames ',J1-1,J1
         D1=1.0D100; D2=1.0D100; NOINT=.TRUE.  ! to skip the next blocks
      ELSE
         CALL MINMAXD2R(R1AX,R1AY,R1AZ,R2AX,R2AY,R2AZ,R1BX,R1BY,R1BZ,R2BX,R2BY,R2BZ, &
  &                    D2,D1,DINT,DSQ2,DSQ1,DSQI,G1,G2,G1INT,G2INT,NOINT,.FALSE.,NREPCUT(J2))
      ENDIF
...
USE COMMONS, ONLY: FROZEN, FREEZE, NREPI, NREPJ, NNREPULSIVE, &
  ...
  &            INTFREEZET, INTFROZEN, CONCUTLOCAL, CONCUT, CONCUTABST, CONCUTABS, CONCUTFRACT, CONCUTFRAC, &
  &  NATOMS, DEBUG, MYUNIT, INTMINFAC, FREEZENODEST, INTSPRINGACTIVET, QCIADDREP, QCIADDREPCUT, QCIADDREPEPS, QCIBONDS
USE PORFUNCS
IMPLICIT NONE
           
INTEGER :: J1,J2,NI2,NI1,NJ2,NJ1,NMAXINT,NMININT,NREPINT(INTIMAGE+2),ISTAT,J3,J4,J5,NINTMIN,NINTMIN2
DOUBLE PRECISION :: ECON, EREP, ETOTAL, RMS
DOUBLE PRECISION R1AX,R1AY,R1AZ,R2AX,R2AY,R2AZ,R1BX,R1BY,R1BZ,R2BX,R2BY,R2BZ,D2,D1
DOUBLE PRECISION G1(3),G2(3),DINT,G1INT(3),G2INT(3)
DOUBLE PRECISION DUMMY, REPGRAD(3), INTCONST, D12, DSQ2, DSQ1, DSQI
...
DOUBLE PRECISION XYZ((3*NATOMS)*(INTIMAGE+2)), GGG((3*NATOMS)*(INTIMAGE+2)), EEE(INTIMAGE+2), CCLOCAL
LOGICAL IMGFREEZE(INTIMAGE)
DOUBLE PRECISION DPLUS, ESPRING, SPGRAD(3), X1, X2, Y1, Y2, Z1, Z2
...
IF (QCIADDREP.LT.1) THEN
   WRITE(MYUNIT,'(A,I6)') 'congrad3> ERROR congrad3 called with no QCIADDREP=',QCIADDREP
   STOP
ENDIF
!
!  Constraint energy and forces.
         ...
         DO J4=1,QCIADDREP
            ...
            DO J5=1,QCIADDREP
               ...
               IF (D2.LT.QCIADDREPCUT) THEN 
                  ...
                  REPE(J1)=REPE(J1)+DUMMY
                  EREP=EREP+DUMMY
!                 WRITE(MYUNIT,'(A,4I6,A,2I6,A,2G20.10)') 'congrad3> atoms: ',CONI(J2),CONJ(J2),CONI(J3),CONJ(J3), &
...
!  DO J1=1,INTIMAGE+2 ! can change when zero energies are confirmed for end images
      ...
      ENDIF
      IF (INTFROZEN(NREPI(J2)).AND.INTFROZEN(NREPJ(J2))) THEN
         WRITE(MYUNIT, '(A,I6,A,2I6)') ' congrad3> ERROR *** repulsion ',J2,' between frozen atoms ',NREPI(J2),NREPJ(J2)
         STOP
      ENDIF
!     WRITE(MYUNIT,'(A,2I8,6G20.10)') 'congrad3> B J1,J2,GGG(1:6)=',J1,J2,GGG(1:6)
      NI2=(3*NATOMS)*(J1-1)+3*(NREPI(J2)-1)
      NJ2=(3*NATOMS)*(J1-1)+3*(NREPJ(J2)-1)
      R2AX=XYZ(NI2+1); R2AY=XYZ(NI2+2); R2AZ=XYZ(NI2+3)
      R2BX=XYZ(NJ2+1); R2BY=XYZ(NJ2+2); R2BZ=XYZ(NJ2+3)
...
!        WRITE(MYUNIT, '(A,6F15.10)') 'R1AX,R1AY,R1AZ,R2AX,R2AY,R2AZ=',R1AX,R1AY,R1AZ,R2AX,R2AY,R2AZ
!        WRITE(MYUNIT, '(A,6F15.10)') 'R1BX,R1BY,R1BZ,R2BX,R2BY,R2BZ=',R1BX,R1BY,R1BZ,R2BX,R2BY,R2BZ
!        WRITE(MYUNIT,'(A,7I10)') 'congrad3> J2,NI1,NJ1,NI2,NJ2,NREPI,NREPJ=',J2,NI1,NJ1,NI2,NJ2,NREPI(J2),NREPJ(J2)
!        WRITE(MYUNIT,'(A,7I10)') 'frames ',J1-1,J1
         NOINT=.TRUE.
      ELSE
         CALL MINMAXD2R(R1AX,R1AY,R1AZ,R2AX,R2AY,R2AZ,R1BX,R1BY,R1BZ,R2BX,R2BY,R2BZ, &
  &                 D2,D1,DINT,DSQ2,DSQ1,DSQI,G1,G2,G1INT,G2INT,NOINT,.FALSE.,NREPCUT(J2))
         IF (.NOT.NOINT) THEN
!           WRITE(MYUNIT,'(A,I6,A,I6,A,2I6,A,2G20.10)') 'congrad3> internal minimum images ',J1-1,' and ',J1,' atoms: ',NREPI(J2),NREPJ(J2), &
! &                        ' distance,cutoff=',DINT,NREPCUT(J2)
            NINTMIN=NINTMIN+1
         ENDIF
      ENDIF
...
DO J1=2,INTIMAGE+1
   ETOTAL=ETOTAL+EEE(J1)
!  WRITE(MYUNIT, '(A,I6,A,3G20.10)') ' congrad3> con/rep/RMS image ',J1,' ',CONE(J1),REPE(J1),RMSIM(J1)
   IF (REPEINT(J1).LT.MININT) THEN
      MININT=REPEINT(J1)
      NMININT=J1
   ENDIF
