...
c      return
c      end
      subroutine hmatd(x,n,h0,h1,igrad)
      USE commons
      implicit DOUBLE PRECISION (a-h,o-z)
      IMPLICIT INTEGER(I-N)
      INTEGER N
...
cc      Ex1= energy(rkl,ix,1)*(x(ir,l)-x(ir,k))/rkl
        IF (NEON) THEN
        Ex1= utn(rkl/RauA,1)/RauA*(x(ir,l)-x(ir,k))/rkl
        ELSE
           CALL ASAR1(rkl,Ex,Ex1)
...
cc      Exij1= energy(rij,ix,1)*drijdx
        IF (NEON) THEN
        Exij1= utn(rij/RauA,1)/RauA*drijdx
        ELSE
           CALL ASAR1(rij,Exij,Exij1)
        Exij1= Exij1*drijdx
        ENDIF
...
cc      print *, rx(ir5), Ex(ir5)
   ...
   55 continue
    ...
    5 continue
      ...
      parameter (nemax=100)
        integer iw(nemax,3), NCALL, N, I, ID, IEV, IT, J, K, N3, NE
C       INTEGER IWORK(3*5*N), IFAIL(3*N)

        DOUBLE PRECISION x(3,n),h0(n*3,n*3)
     &        ,h1(n*3,n*3)
     &        ,grad(n*3),ee(n*3),ev(n*3,n*3)
C     &       ,w(n*3,n*3),WORK(3*8*N)
      logical gradt
      data iev/1/, ncall/1/, shift/10.0d0/
     &  , nstep/1000/,iflag/-2/
C     &  , Elow/-1.d3/,eps/1.d-6/,message/6/,nwork/777/
      save
...
c      end
   ...
   17 CONTINUE
      ...
      subroutine hmat(x,n,n3,h0,h1,igrad)
      USE commons
      implicit DOUBLE PRECISION (a-h,o-z)
      IMPLICIT INTEGER(I-N)
C       common/test/itest
...
cc      if(k.eq.m) Ex1=-energy(rkl,ix,1)*(x(ir,l)-x(ir,k))/rkl
        IF (NEON) THEN
      if(l.eq.m) Ex1= utn(rkl/RauA,1)/RauA*(x(ir,l)-x(ir,k))/rkl
      if(k.eq.m) Ex1=-utn(rkl/RauA,1)/RauA*(x(ir,l)-x(ir,k))/rkl
        ELSE
           CALL ASAR1(rkl,Ex,Ex1)
...
cc                 FUNCTION ASNE1(R,I,IP)
                 DOUBLE PRECISION FUNCTION UTN(R,IP)
           implicit DOUBLE PRECISION (a-h,o-z)
      IMPLICIT INTEGER(I-N)
           save
             COMMON/DIPOL/ adip,Runit,ReX                                            
