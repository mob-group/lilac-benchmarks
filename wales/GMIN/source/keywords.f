...
!       sf344> AMBER additions
     ...
     &                      SALTCON, macroiont, nmacroions, macroiondist
      USE modamber
      USE AMBER12_MUTATIONS, ONLY : AMBERMUTATION_SETUP!, AMBERMUTRB_SETUP
      USE PORFUNCS
      USE MYGA_PARAMS
      USE BGUPMOD
      USE GLJYMOD
      USE CHIRO_MODULE, ONLY: CHIRO_SIGMA, CHIRO_MU, CHIRO_GAMMA, CHIRO_L
      USE CONVEX_POLYHEDRA_MODULE, ONLY: INITIALISE_POLYHEDRA, K_COMPRESS, K_OVERLAP
      USE LJ_GAUSS_MOD, ONLY: LJ_GAUSS_MODE, LJ_GAUSS_RCUT, LJ_GAUSS_EPS,
     &                        LJ_GAUSS_R0, LJ_GAUSS_SIGMASQ, LJ_GAUSS_PARAMS,
     &                        LJ_GAUSS_INITIALISE
      USE OPP_MOD, ONLY: OPP_MODE, OPP_RCUT, OPP_K, OPP_PHI, OPP_PARAMS,
     &                   OPP_INITIALISE, OPP_K_LOWER, OPP_K_UPPER, OPP_RON
      USE MBPOLMOD, ONLY: MBPOLINIT
      USE SWMOD, ONLY: SWINIT, MWINIT
      USE AMBER12_INTERFACE_MOD, ONLY : AMBER12_GET_COORDS
!     &                                 AMBER12_ATOMSS, AMBER12_SETUP,
!     &                                 AMBER12_RESIDUES,
!     &                                 POPULATE_ATOM_DATA
     ...
     &     PARSE_MLJ_PARAMS
      ...
      USE HINGE_MOVES, ONLY: HINGE_INITIALISE
      USE MOLECULAR_DYNAMICS, ONLY : MDT, MD_TSTEP, MD_GAMMA, MD_NWAIT, MD_NFREQ, MD_NSTEPS
      ...
      INTEGER K1, I1
      INTEGER ITEM, NITEMS, LOC, LINE, NCR, NERROR, LAST, IX, J1, JP, NPCOUNT, NDUMMY, J2, J3
     ...
     &                NERROR, ECHO, LAST, CAT
      ...
      LOGICAL MLPDONE, MLPNORM, MLQDONE, MLQNORM
...
!     ds656>    COORDS and LABELS allocated here
      ALLOCATE( FIXSTEP(1),FIXTEMP(1),FIXBOTH(1),TEMP(1),ACCRAT(1),STEP(1),ASTEP(1),OSTEP(1),BLOCK(1),NT(1),NQ(1),EPREV(1),
     &     JUMPMOVE(1),JUMPINT(1),JDUMP(1),COORDS(3*NATOMSALLOC,1),LABELS(NATOMSALLOC,1),COORDSO(3*NATOMSALLOC,1),
     &     LABELSO(NATOMSALLOC,1),VAT(NATOMSALLOC,1),VATO(NATOMSALLOC,1),
...
! sf344> unfreeze structures at the final quench
      ...
      FREEZESAVE=.TRUE.
! sf344> Modular global optimization defaults
      MODULART=.FALSE.
      MODULARRESTARTT=.FALSE.
      MODULARLOWEST=1
      ALLOCATE(EXCLUDED(NATOMS))
      EXCLUDED(:)=.FALSE.
...
!     ds656> Saw-tooth protocol
      SAWTOOTH=.FALSE.
      SAWTOOTH_NREJMAX=0
      SAWTOOTH_TMAX=0.3D0
      SAWTOOTH_TFAC=1.0D0
      SAWTOOTH_SFAC=1.0D0
      SAWTOOTH_SFAC2=1.0D0
...
! sn402: hinge moves for the plate potential
      ROTATEHINGET=.FALSE.
      HINGEROTATEFACTOR=1.0D0
      ROTATEHINGEFREQ=1
...
!fh301>}}}
      ...
      PERCOLATET=.FALSE.
      PERCCUT=1.0D100
      PERCGROUPCUT=1.0D100
      PERCGROUPT=.FALSE.
      PERCGROUPRESEEDT=.FALSE.
      PERCGROUPSF=1.0D0
      GENALT=.FALSE.
!
...
! sn402 > Multiple potential scheme
      MULTIPOTT = .FALSE.

! dj337: pahagenrigid for benz with Ewald
      BENZRIGIDEWALDT = .FALSE.
...
! jdf43>
      ...
      ORBITTOL=1.0D-3
      NOINVERSION=.FALSE.
!cv320> rigid body watermethane
      WATERMETHANET=.FALSE.
      CLATHRATET= .FALSE.
! ds656> Parallelised generalised basin-hopping
      GBHT=.FALSE.
...
! jk669 SQNM keywords
      SQNMT=.FALSE.
      SQNM_HISTMAX=20 !defualt
      SQNM_DEBUGT=.FALSE.
      SQNM_DEBUGRUN=0 !defaul 0 means debug all runs
      SQNM_DEBUGLEVEL=0 !how much debug data to print 
      SQNM_WRITEMAX=10 !how many lines to print out. default is 10
      SQNM_BIOT=.TRUE.
...
! cs675 ORBITALS stuff
      ORBITALS = .FALSE.
      ORBVAREXPONENT = -1!default to easily identifiable (quasi-)nonsense value.

! AMBER mutations
      AMBERMUTATIONT = .FALSE.
      MUTATIONFREQ = 1000
      MUTTESTSTEPS = 50
      NMUTATION = 0
      AMBERMUTIGB = 2
...
!
      ...
      ! Before doing any basin-hopping steps, read a second set of coordinates from file 'finish' and align the
      ! two input structures, outputting the distance between them as the initial energy. Normally use with STEPS 0:
      ! This keyword is only intended to be used for testing GMIN's structure alignment procedures.
      ELSE IF (WORD.EQ.'ALIGN') THEN
         ALIGNT = .TRUE.
...
! ds656> Quanech-Assisted Local Combinatorial Search
      ...
      ELSE IF (WORD .EQ. 'QALCS_SURF') THEN
         QALCS_SURFT = .TRUE.
         IF(NITEMS > 1) CALL READI(QALCS_SURF_MODE)
...
!
      ...
      ELSE IF (WORD.EQ.'FIXLABELS') THEN
         FIXLABELS=.TRUE.
...
! ds656> Generalised LJ with Yukawa
      ELSE IF (WORD .EQ. 'GLJY') THEN
         ...
         CALL READF(YUK_XI)
! ds656> Saw-tooth protocol
      ELSE IF (WORD .EQ. 'SAWTOOTH') THEN
         SAWTOOTH=.TRUE.
         IF(NITEMS.GT.1) CALL READF(SAWTOOTH_TMAX)
         IF(NITEMS.GT.2) CALL READF(SAWTOOTH_TFAC)
         IF(NITEMS.GT.3) CALL READI(SAWTOOTH_NREJMAX)
         IF(NITEMS.GT.4) THEN
            CALL READF(SAWTOOTH_SFAC)
            SAWTOOTH_SFAC2=EXP(-LOG(SAWTOOTH_SFAC)/DBLE(SAWTOOTH_NREJMAX))
         ENDIF
! ds656> Stress tensor calculation
...
! End CHAPERONIN
      ...
      ELSE IF (WORD.EQ.'BRANCHNBOUND') THEN
         BNB_ALIGNT = .TRUE.
         IF (NITEMS.GT.1) CALL READI(BNB_NSTEPS)  ! Default 2000
...
! location before the initial quench is done.
      ...
      ELSE IF (WORD.EQ.'QCIADDREP') THEN
         ...
         WRITE(MYUNIT,'(A,I6,A)') 'keywords> Adding ',QCIADDREP,' repulsive sites along constraints'
...
!
      ELSE IF (WORD.EQ.'AMBERMUTATION') THEN
        AMBERMUTATIONT = .TRUE.
        MUTUNIT = GETUNIT()
        CALL READI(MUTATIONFREQ)
        CALL READI(MUTTESTSTEPS)
        AMBER12T=.TRUE.
        SETCHIRAL=.TRUE.
        IF(.NOT.ALLOCATED(COORDS1)) ALLOCATE(COORDS1(3*NATOMS))
        IF(ALLOCATED(COORDS)) DEALLOCATE(COORDS)
        ...
        DO J1=1,NPAR
           COORDS(:,J1) = COORDS1(:)
        END DO
        OPEN(MUTUNIT,FILE='BHmutation.log',STATUS='REPLACE',ACTION='WRITE')
        CALL AMBERMUTATION_SETUP()  !we initialise the group rotations here as well
        
      ELSE IF (WORD.EQ.'AMBERMUTIGB') THEN
        CALL READI(AMBERMUTIGB)
        IF (AMBERMUTIGB.EQ.2) THEN
           WRITE(MYUNIT,'(A)') 'keyword> use igb2 with mbondi2 to create topology files for mutations'
        ELSE IF (AMBERMUTIGB.EQ.8) THEN
           WRITE(MYUNIT,'(A)') 'keyword> use igb8 with mbondi3 to create topology files for mutations'
        ELSE
           WRITE(MYUNIT,'(A)') 'keyword> invalid choice for igb'
           STOP
        ENDIF

      ELSE IF (WORD.EQ.'AMBERMUTFF') THEN
        CALL READI(AMBERMUTFF)
        IF (AMBERMUTFF.EQ.14) THEN
           WRITE(MYUNIT,'(A)') 'keyword> use ff14SB force field for mutations'
        ELSE IF (AMBERMUTFF.EQ.99) THEN
           WRITE(MYUNIT,'(A)') 'keyword> use ff99SB force field for mutations'
        ELSE
           WRITE(MYUNIT,'(A)') 'keyword> invalid force field choice'
           STOP
        ENDIF
      
      ELSE IF (WORD.EQ.'AMBERMUTENERGY') THEN
        CALL READI(MUTENERGY)
        CALL READI(MUTTERMID)
        IF (MUTENERGY.EQ.1) THEN
           WRITE(MYUNIT,'(A)') 'keyword> use random numbers to score mutations'
        ELSE IF (MUTENERGY.EQ.2) THEN
           WRITE(MYUNIT,'(A)') 'keyword> use decomposed energy to score mutations'
        ELSE IF (MUTENERGY.EQ.3) THEN
           WRITE(MYUNIT,'(A)') 'keyword> use interaction energy between parts to score mutations'
           WRITE(MYUNIT,'(A,I6)') 'keyword> first component ends at residue ' , MUTTERMID
        ELSE IF (MUTENERGY.EQ.4) THEN
           WRITE(MYUNIT,'(A)') 'keyword> use RMSD with respect to ideal helix'
        ELSE IF (MUTENERGY.EQ.5) THEN
           WRITE(MYUNIT,'(A)') 'keyword> use DSSP alpha helix'
        ELSE IF (MUTENERGY.EQ.6) THEN
           WRITE(MYUNIT,'(A)') 'keyword> use DSSP 3-10 helix'
        ELSE IF (MUTENERGY.EQ.7) THEN
           WRITE(MYUNIT,'(A)') 'keyword> use DSSP pi helix'
        ELSE
           WRITE(MYUNIT,'(A)') 'keyword> option not available for scoring mutation'
           STOP
        ENDIF

      ELSE IF (WORD.EQ.'AMBERMUTRIGID') THEN
        AMBERMUTRIGIDT = .TRUE.
        RIGIDINIT = .TRUE.
        ATOMRIGIDCOORDT = .TRUE.
        AACONVERGENCET = .TRUE.
        !CALL AMBERMUTRB_SETUP()
...
! ds656> Molecular Dynamics
      ELSE IF (WORD.EQ.'MD') THEN
         MDT=.TRUE.
         CALL READI(MD_NSTEPS)
         IF(NITEMS .GT. 2) THEN
            CALL READI(MD_NWAIT)
            CALL READI(MD_NFREQ)
         ELSE
...
!
      ELSE IF ((WORD.EQ.'MLP3').OR.(WORD.EQ.'MLPB3')) THEN
         ...
         IF (MLPB3T) THEN
            WRITE(MYUNIT,'(A,4I8,G20.10)') 'MLP3 potential bias nodes and Nin, Nhidden, Nout, Ndata, lambda=',
     &                                   MLPIN,MLPHIDDEN,MLPOUT,MLPDATA,MLPLAMBDA
            NMLP=MLPHIDDEN*(MLPIN+MLPOUT)+1
         ELSE
            WRITE(MYUNIT,'(A,4I8,G20.10)') 'MLP3 potential with Nin, Nhidden, Nout, Ndata, lambda=',
     &                                   MLPIN,MLPHIDDEN,MLPOUT,MLPDATA,MLPLAMBDA
         ...
         DO J1=1,MLPDATA
            READ(LUNIT,*) MLPDAT(J1,1:MLPIN),MLPOUTCOME(J1)
            MLPOUTCOME(J1)=MLPOUTCOME(J1)+1 ! to shift the range to 1 to 4
!           WRITE(*,'(9G20.10,I8)') MLPDAT(J1,1:MLPIN),MLPOUTCOME(J1)
            DO J2=1,MLPIN
         ...
         IF (MLPNORM) THEN
            ...
            WRITE(MYUNIT,'(A)') 'keyword> Rescaling inputs by mean absolute values:'
            WRITE(MYUNIT,'(6G20.10)') MLPMEAN(1:MLPIN)
            DO J1=1,MLPIN
      ...
      ELSE IF (WORD.EQ.'MLPB3NEW') THEN
         ...
         CALL READI(MLPDATA)
         IF (NITEMS.GT.5) CALL READF(MLPLAMBDA)
         WRITE(MYUNIT,'(A,5I8,G20.10)') 'MLP3 new potential bias nodes and Nin, Ninstart, Nhidden, Nout, Ndata, lambda=',
     &                                MLPIN,MLPSTART,MLPHIDDEN,MLPOUT,MLPDATA,MLPLAMBDA
      ...
      ELSE IF ((WORD.EQ.'MLPVB3').OR.(WORD.EQ.'MLPVB3NN')) THEN
         ...
         WRITE(MYUNIT,'(A,5I8,G20.10)') ' keywords> MLP3 vector bias nodes and Nin, Ninstart, Nhidden, Nout, Ndata, lambda=',
     &                    MLPIN,MLPSTART,MLPHIDDEN,MLPOUT,MLPDATA,MLPLAMBDA
         IF (WORD.EQ.'MLPVB3NN') WRITE(MYUNIT,'(A,I8)') ' keywords> Nearest-neighbours=',MLPNEIGH
         NMLP=MLPHIDDEN*(MLPIN+MLPOUT)+MLPHIDDEN+MLPOUT
         IF (NMLP.NE.NATOMS) THEN
            WRITE(MYUNIT,'(A,2I8)') 'keywords> ERROR *** NATOMS,NMLP=',NATOMS,NMLP
            STOP
         ENDIF
         LUNIT=GETUNIT()
         OPEN(LUNIT,FILE='MLPdata',STATUS='OLD')
...
!
         IF (WORD.EQ.'MLPVB3NN') THEN
            ...
            WRITE(MYUNIT,'(A)') 'Original nearest-neighbour fomulation:'
            FREEZE=.TRUE.
            ...
            DO J1=1,MLPHIDDEN
               ...
               J3=(J1-1)*MLPIN+J2
               FROZEN(J3)=.FALSE.
               WRITE(MYUNIT, '(A,I10,A,I10,A,I10)') 'keywords> Unfrozen weight ',J3,' input ',J2,' to hidden node ',J1
               ...
               WRITE(MYUNIT, '(A,I10,A,I10,A,I10)') 'keywords> Unfrozen weight ',J3,' hidden node ',J1,' to output ',J2
               FROZEN(J3)=.FALSE.
            ENDDO
            WRITE(MYUNIT,'(A,I6,A)') 'keywords> New nearest-neighbour formulation with ',MLPNEIGH,' neighbours' 
...
!
               ...
               DO J2=1,MIN(MLPNEIGH,MLPIN)
                  WRITE(MYUNIT,'(A,I8,A,I8,A,I8,A,G20.10)') 'hidden ',J1,' input neighbour ',J2,' is ',MLPINDEXI(J2),' distance ',
     &                                                      MLPDISTHI(J2)
...
!
               DO I1=1,MLPOUT
                  MLPINDEXO(I1)=I1
                  MLPDISTHO(I1)=( (J1-1.0D0)/(MLPHIDDEN-1.0D0) - (I1-1.0D0)/(MLPOUT-1.0D0) )**2 - I1*1.0D-6 ! to break degeneracy
               ENDDO
               CALL SORT4(MLPOUT,MLPOUT,MLPDISTHO,MLPINDEXO)
               DO J2=1,MIN(MLPNEIGH,MLPOUT)
                  WRITE(MYUNIT,'(A,I8,A,I8,A,I8,A,G20.10)') 'hidden ',J1,' output neighbour ',J2,' is ',MLPINDEXO(J2),' distance ',
     &                                                      MLPDISTHO(J2)
      ...
      ELSE IF (WORD.EQ.'MLPNEWREG') THEN
         MLPNEWREG=.TRUE.
         WRITE(MYUNIT,'(A)') 'keyword> Including reciprocals in regularisation'
!
! MLPNORM directs OPTIM to rescale the input data columns by dividing each one by the
! average of the mean magnitude
! Arranged so that MLPNORM can come before of after MLPB3/MLP3
!
...
!           DO J1=1,MLPDATA
!              READ(LUNIT,*) MLPDAT(J1,1:MLPIN),MLPOUTCOME(J1)
!              MLPOUTCOME(J1)=MLPOUTCOME(J1)+1 ! to shift the range to start from 1 instead of zero
!              DO J2=1,MLPIN
!                 MLPMEAN(J2)=MLPMEAN(J2)+ABS(MLPDAT(J1,J2))
!              ENDDO
!           ENDDO
...
!           DEALLOCATE(MLPMEAN)
      ...
      ELSE IF (WORD.EQ.'MLQ') THEN
         ...
         WRITE(MYUNIT,'(A,4I8,G20.10)') ' keywords> MLQ Nin, Ninstart, Nout, Ndata, lambda=',
     &                                MLQIN,MLQSTART,MLQOUT,MLQDATA,MLQLAMBDA
         NMLQ=MLQOUT*(1+(MLQIN*(MLQIN+3))/2)
         WRITE(MYUNIT,'(A,5I8,G20.10)') ' keywords> MLQ variables=',NMLQ
         IF (NMLQ.NE.NATOMS) THEN
            WRITE(MYUNIT,'(A,2I8)') 'keywords> ERROR *** NATOMS,NMLQ=',NATOMS,NMLQ
            STOP
         ENDIF
         ...
         IF (MLQNORM) THEN
            ...
            WRITE(MYUNIT,'(A)') 'keyword> Rescaling inputs by mean absolute values:'
            WRITE(MYUNIT,'(6G20.10)') MLQMEAN(1:MLQIN)
            DO J1=1,MLQIN
...
!
      ELSE IF (WORD.EQ.'MLQNORM') THEN
         ...
         IF (MLQDONE) THEN
            ...
            DO J1=1,MLQDATA+MLQSTART-1
               IF (J1.GE.MLQSTART) THEN
                  J2=J1-MLQSTART+1
...
! sf344> model for macroions. Syntax: MACROION nmacroions macroiondist
!        Should be used in conjunction with LIGMOVE
      ELSE IF(WORD.EQ.'MACROION') THEN
        MACROIONT=.TRUE.
        BLOCKMOVET=.TRUE.
        CALL READI(nmacroions)   ! number of macroions in the system (the topology file should contain these first!)
        CALL READF(macroiondist) ! distance threshold for determining bound pairs of macroions-counterions in TAKESTEPAMBER
        WRITE(MYUNIT,'(A15,I6,A46)') ' keyword> ligand defined with ',nmovableatoms,' atoms'
...
!
         ELSE IF (WORD.EQ.'QCIRADSHIFT') THEN
            ...
            WRITE(MYUNIT,'(A,G20.10)') ' keyword> Shifting unconstrained atoms away from added atoms by ',QCIRADSHIFT
!
...
! Set the right parts of the DONTMOVERES array to FALSE
      ...
      ELSE IF (WORD.EQ.'DUMPMQ') THEN
         DUMPMQT=.TRUE.
         MQUNIT=GETUNIT()
         OPEN(MQUNIT,FILE='dumpMQ',STATUS='UNKNOWN')
...
! ----------------------------------------------------------------------------------------
! dj337: frequency with which to take BH steps in box parameters

      ELSE IF (WORD.EQ.'BOXSTEP') THEN

         IF (NITEMS.GT.1) CALL READI(BOXSTEPFREQ)

! ----------------------------------------------------------------------------------------
! dj337: pahagenrigid for benzene using Ewald summation

      ELSE IF (WORD.EQ.'BENZRIGIDEWALD') THEN

         IF (.NOT.PERIODIC) THEN
            WRITE(MYUNIT, '(A)') 'keyword> ERROR PERIODIC must be defined first in data file'
            STOP
         ENDIF
         ...
         ! CALCULATE ALPHA = 5.6/L_MIN
         EWALDALPHA = 5.6D0/12.0d0
...
! ----------------------------------------------------------------------------------------
      ...
      ELSE IF (WORD.EQ.'EWALD') THEN
         ...
         IF (.NOT.PERIODIC) THEN
            WRITE(MYUNIT, '(A)') 'keyword> ERROR PERIODIC must be defined before EWALD in data file'
            STOP
         ENDIF
...
! ----------------------------------------------------------------------------------------
      ...
      ELSE IF (WORD.EQ.'WATERMETHANE') THEN
         WATERMETHANET=.TRUE.
      ELSE IF (WORD.EQ.'CLATHRATE') THEN
         CLATHRATET=.TRUE.
         CALL READI(NWATER)
         CALL init_ttm3f(NWATER)
      ...
      ELSE IF (WORD.EQ.'FASTOVERLAP') THEN
         ...
         IF (NITEMS .GT. 1) THEN ! Read a parameter to determine how many displacements/rotations to consider for bulk/cluster
            ...
            NDISPLACEMENTS=NPEAKS  ! Default = 1. This will be used if we have a periodic system
            NROTATIONS=NPEAKS      ! Default = 1. This will be used if we have a cluster (but we don't know PERIODIC yet)
            ...
            IF (NITEMS .GT. 2) THEN
               ...
               IF (NITEMS .GT.3) THEN
                  ...
                  IF(PERIODIC) WRITE(*,*) "keywords> Warning: maximum angular moment value explicitly specified but",
     &                                    "this parameter is not used for periodic systems."
               ENDIF
            ENDIF
         ENDIF
...
!
      ELSE IF (WORD.EQ.'LJADD') THEN
         ...
         DO J1=1,NATOMS
            DO J2=1,NATOMS
               ...
               WRITE(MYUNIT,'(2I6,G20.10)') J1,J2,LJADDEPS(J2,J1)
            ENDDO
         ENDDO
         CLOSE(LUNIT)
      ...
      ELSE IF (WORD.EQ.'LJADD4') THEN
            ...
            DO J2=1,NUMNN
               ...
               WRITE(MYUNIT,'(2I6)') LJADDNN(J2,1), LJADDNN(J2,2)
            ENDDO
            READ(LUNIT,*) LJADDREFNORM
            WRITE(MYUNIT,'(A,I6,3G20.10)')
     ...
     &                                NUMNN,TANHFAC,LJADDCUTOFF,LJADDREFNORM
      ...
      ELSE IF (WORD.EQ.'LJADD3') THEN
         ...
         ENDDO
         CLOSE(LUNIT)
      ELSE IF (WORD.EQ.'LJADD3GUIDE') THEN
         LJADD3GUIDET=.TRUE.
         NADDTARGET=NATOMS
         CALL READF(ALPHAATT) ! just change non-nearest-neighbour attraction
         CALL READF(NNCUTOFF) ! cutoff to define nearest-neighbours in reference
         WRITE(MYUNIT,'(2(A,G20.10))') 'keyword> LJADD3 guiding potential, NN cutoff=',NNCUTOFF,' NNN attractive fraction=',ALPHAATT
         IF (.NOT.ALLOCATED(LJADDREP)) ALLOCATE(LJADDREP(NADDTARGET,NADDTARGET))
         IF (.NOT.ALLOCATED(LJADDATT)) ALLOCATE(LJADDATT(NADDTARGET,NADDTARGET))
!
! LJAT
...
!
      ...
      ELSE IF (WORD.EQ.'MINDENSITY') THEN
         MINDENSITYT=.TRUE.
! sf344> Modular global optimization algorithm
      ELSE IF (WORD.EQ.'MODULAR') THEN
         MODULART=.TRUE.
            CALL READI(MODULARNMIN)
            CALL READI(MODULARNMAX)
            CALL READI(MODULARNINCR)
         IF(NITEMS.GT.4) THEN
            CALL READI(MODULARLOWEST)
            IF(NITEMS.GT.5) THEN
                 CALL READA(MODULARRESTARTFILE)
                 MODULARRESTARTT=.TRUE.
            END IF
         END IF
         IF(MODULARNMAX.GT.NATOMS) THEN
            WRITE(MYUNIT,'(A)') 'keywords> Something is wrong, NMAX>NATOMS, is the coords file properly set up? - quitting!'
            STOP
         END IF
         FREEZE=.TRUE.  ! freeze all coordinates even without using FREEZEALL
         FREEZEALL=.TRUE.
         NFREEZE=NATOMS
         DO J1=1,NATOMS
            FROZEN(J1)=.TRUE.
...
!
      ELSE IF (WORD.EQ.'MPI') THEN
         MPIT=.TRUE.
         DEALLOCATE(FIXSTEP,FIXTEMP,FIXBOTH,TEMP,ACCRAT,STEP,ASTEP,OSTEP,BLOCK,NT,JUMPMOVE,JUMPINT,JDUMP,COORDS,LABELS,NQ,
     &        JUMPTO,EPREV,COORDSO,LABELSO,VAT,VATO,SHELLMOVES,PTGROUP,NSURFMOVES,NCORE)
     ...
     &        JUMPTO(NPAR),EPREV(NPAR),COORDSO(3*NATOMSALLOC,NPAR),VAT(NATOMSALLOC,NPAR),VATO(NATOMSALLOC,NPAR),
     &        LABELS(NATOMSALLOC,NPAR),LABELSO(NATOMSALLOC,NPAR) )
...
!
      ELSE IF (WORD.EQ.'NEWMOVES') THEN
         NEWMOVEST=.TRUE.
      ELSE IF (WORD.EQ.'GBHRESTART') THEN !<ds656
         GBH_RESTART=.TRUE.
         CALL READI(GBH_NREJMAX)
         IF(NITEMS.GT.2) CALL READI(GBH_NAVOID)
...
!
      ...
      ELSE IF (WORD.EQ.'OPEP') THEN
         ...
         IF (OPEP_DUMMY.EQ.'RNA') THEN
            OPEP_RNAT = .TRUE.
            WRITE(MYUNIT,'(A)') 'keyword> RNA simulation using OPEP'
         ELSE
            WRITE(MYUNIT,'(A)') 'keyword> Protein simulation using OPEP'
         ENDIF
         IF(.NOT.ALLOCATED(COORDS1)) ALLOCATE(COORDS1(3*NATOMS))
         IF(ALLOCATED(COORDS)) DEALLOCATE(COORDS)
         CALL OPEP_INIT(NATOMS, COORDS1(:),OPEP_RNAT)
         ALLOCATE(COORDS(3*NATOMS,NPAR))
         DO J1=1,NPAR
            COORDS(:,J1) = COORDS1(:)
         END DO
...
!
     ...
     &        COORDS(3*NATOMSALLOC,NPAR),LABELS(NATOMSALLOC,NPAR),
     &        COORDSO(3*NATOMSALLOC,NPAR),LABELSO(NATOMSALLOC,NPAR),VAT(NATOMSALLOC,NPAR),VATO(NATOMSALLOC,NPAR),EPREV(NPAR),
     &        NSURFMOVES(NPAR),NCORE(NPAR),REPLOW(NPAR),SHELLMOVES(NPAR),PTGROUP(NPAR) )
         NATOMS=NATOMS/NPAR
         DO JP=1,NPAR
            FIXSTEP(JP)=.FALSE.
            FIXTEMP(JP)=.FALSE.
      ...
      ELSE IF (WORD.EQ.'PERCOLATE') THEN
         PERCOLATET=.TRUE.
         PERCACCEPTED=.FALSE.
         PERCCOMPMARKOV=.FALSE.
         IF (NITEMS.GT.1) CALL READF(PERCCUT)
         IF (NITEMS.GT.2) CALL READF(K_COMP)
         IF (NITEMS.GT.3) CALL READF(GUIDECUT)

      ELSE IF (WORD.EQ.'PERCGROUP') THEN
         PERCGROUPT=.TRUE.
         CALL READF(PERCGROUPCUT)
         CALL READF(LIGTRANSSTEP)
         CALL READF(LIGROTSCALE)
         IF(NITEMS.GT.4) THEN !seed groups with predefined coordinates
             PERCGROUPRESEEDT=.TRUE.
             CALL READF(PERCGROUPSF)
         END IF
      ...
      ELSE IF (WORD.EQ.'PERIODIC') THEN
         ...
         ! dj337: also put into box_params vector
         box_params(1) = boxlx
         box_params(2) = boxly
         box_params(3) = boxlz
         box_params(4) = palpha
...
! upper triangular lattice matrix
     ...
     1     -COS(PGAMMA)*COS(PGAMMA)+2.D0*COS(PALPHA)*COS(PBETA)*COS(PGAMMA))
         ...
         WRITE(MYUNIT,*) 'keywords> Lattice vectors calculated as:'
         WRITE(MYUNIT,*)LAT(:,1)
         WRITE(MYUNIT,*)LAT(:,2)
         WRITE(MYUNIT,*)LAT(:,3)
...
!
      ...
      ELSE IF (((WORD.EQ.'PERMDIST').OR.(WORD.EQ.'LOCALPERMDIST').OR.(WORD.EQ.'LPERMDIST')).AND.(.NOT.PERMOPT)) THEN
         PERMDIST=.TRUE.
         IF (WORD.EQ.'LPERMDIST') THEN
            ...
            WRITE(MYUNIT,'(A,F15.5)') ' keyword> Local permutational alignment: alignment threshold=',LOCALPERMCUT
            WRITE(MYUNIT,'(A,F15.5)') ' keyword> Local permutational alignment: alignment cutoff=   ',LOCALPERMCUT2
            WRITE(MYUNIT,'(A,F15.5)') ' keyword> Distance tolerance for distinguishing atoms in the same orbit=',ORBITTOL
         ELSEIF (WORD.EQ.'PERMDIST') THEN
            IF (NITEMS.GT.1) CALL READF(ORBITTOL)
            IF (NITEMS.GT.2) CALL READI(MAXNSETS)
            WRITE(MYUNIT, '(A,F15.5)') ' keyword> Distance tolerance for distinguising atoms in the same orbit=',ORBITTOL
            WRITE(MYUNIT, '(A,I3)') ' keyword> Maximum number of secondary sets in perm.allow file=', MAXNSETS
         ENDIF

         INQUIRE(FILE='perm.allow',EXIST=PERMFILE)
         IF (.NOT.ALLOCATED(NPERMSIZE)) THEN
            ALLOCATE(NPERMSIZE(NATOMSALLOC),PERMGROUP(NATOMSALLOC),NSETS(NATOMSALLOC),SETS(NATOMSALLOC,MAXNSETS))
         ENDIF
         IF (PERMFILE) THEN
            OPEN(UNIT=1,FILE='perm.allow',STATUS='OLD')
            READ(1,*) NPERMGROUP
...
!
      ELSE IF (WORD.EQ.'QCIAMBER') THEN
         QCIAMBERT=.TRUE.
         WRITE(MYUNIT,'(A)') ' keyword> Use topology file for constraints in QCI'
...
! If SPECMASS is specified we read the mass for each species.
      ELSE IF(WORD.EQ.'SPECMASS') THEN
         ...
         IF (ALLOCATED(ATMASS)) DEALLOCATE(ATMASS)
         ALLOCATE(ATMASS(NATOMSALLOC))
         ATMASS(:) = 1.0D0 ! Just initialisation
!
! Reorder addressable potentials in an attempt to cluster particle in order.
!

      ELSE IF (WORD.EQ.'REORDERADD') THEN
         REORDERADDT=.TRUE.
!
...
! hk286 > Generalised rigid body   !
      ...
      ELSE IF (WORD.EQ.'RMS') THEN
       ...
       ELSE IF (WORD.EQ.'ROTATEHINGE') THEN
          ROTATEHINGET=.TRUE.
!         Read ROTATEHINGEFREQ
          IF (NITEMS.GT.1) CALL READI(ROTATEHINGEFREQ)
!         Read in ROTATEFACTOR
          IF (NITEMS.GT.2) CALL READF(HINGEROTATEFACTOR)
          WRITE(MYUNIT,'(A)') ' keywords> Enabled hinge rotation moves for the plate potential'
          WRITE(MYUNIT,'(A,I2,A)') ' keywords> Hinges will be rotated every ',ROTATEHINGEFREQ,' steps'
          WRITE(MYUNIT,'(A,F20.10)') ' keywords> Hinge angle rotatefactor =',HINGEROTATEFACTOR
          CALL HINGE_INITIALISE
...
!same.
      ...
      ELSE IF (WORD .EQ. 'SPARSEBENCH') THEN
         SPARSE_BENCH=.TRUE.
...
! jwrm2>
      ELSE IF (WORD.EQ.'LJGAUSS') THEN
          LJ_GAUSST = .TRUE.
          CALL READI(LJ_GAUSS_MODE)
          CALL READF(LJ_GAUSS_RCUT)
          CALL READF(LJ_GAUSS_EPS)
          CALL READF(LJ_GAUSS_R0)
          CALL READF(LJ_GAUSS_SIGMASQ)
          IF (LJ_GAUSS_MODE .EQ. 3) CALL READI(LJ_GAUSS_PARAMS)
          CALL LJ_GAUSS_INITIALISE()

      ELSE IF (WORD.EQ.'OPP') THEN
          OPPT = .TRUE.
          CALL READI(OPP_MODE)
          CALL READF(OPP_RCUT)
          CALL READF(OPP_RON)
          ...
          IF (OPP_MODE .EQ. 2 .OR. OPP_MODE .EQ. 3) THEN
              CALL READI(OPP_PARAMS)
              IF (BTEST(OPP_PARAMS, 0)) THEN
                CALL READF(OPP_K_LOWER)
                CALL READF(OPP_K_UPPER)
              END IF
          END IF
          CALL OPP_INITIALISE()
! jwrm2> end
...
!     THE REFERENCE GEOMETRY IS ON THE Y-Z PLANE
         ...
         ENDIF
         IF (PERMDIST.OR. LOCALPERMDIST .OR. LPERMDIST) THEN ! correct all permutations allowed if perm.allow is not given explicitly
...
!     ----------------------------------------------------------------------------------------------
! dj337: PAHA genrigid potential

      ELSE IF (WORD .EQ. 'PAHAGENRIGID') THEN
...
! jwrm2>
      ELSE IF (WORD .EQ. 'POLYHEDRA') THEN

        POLYT = .TRUE.
        RIGID = .TRUE.
        CALL READF(K_OVERLAP)
        CALL READF(K_COMPRESS)
        CALL INITIALISE_POLYHEDRA()
! jwrm2> end
...
!         ALLOCATE(SITE(NRBSITES,3))
      ...
      ELSE IF (WORD .EQ. 'PY') THEN
         ...
         IF (NITEMS.GT.4) THEN
            ...
            ENDIF
         ENDIF
      ELSE IF (WORD .EQ. 'PYCGDIMER') THEN
         CALL GENERATECGDIMER
         STOP
...
!
      ELSE IF (WORD.EQ.'PYADD') THEN
         ...
         CALL READI(NADDTARGET)
         WRITE(MYUNIT,'(A,I6)') 'keyword> Target cluster size is ',NADDTARGET
         IF (MOD(NATOMSALLOC/2,NADDTARGET).NE.0) THEN
            WRITE(MYUNIT,'(A,I6)') 'keyword> ERROR, target cluster size is not a factor of the number of PY particles ',
     &          NATOMSALLOC/2
            STOP
         ENDIF
         LUNIT=GETUNIT()
         ...
         DO J1=1,NADDTARGET
            DO J2=1,NADDTARGET
               ...
               WRITE(MYUNIT,'(2I6,G20.10)') J1,J2,PYADDEPS(J2,J1)
            ENDDO
         ENDDO
         CLOSE(LUNIT)
     ...
     &          NATOMSALLOC/2
         ...
         DO J1=1,NADDTARGET
            DO J2=1,NADDTARGET
               ...
               WRITE(MYUNIT,'(2I6,2G20.10)') J1,J2,PYADDREP(J2,J1),PYADDATT(J2,J1)
            ENDDO
         ENDDO
         CLOSE(LUNIT)
...
!
      ...
      ELSE IF (WORD .EQ. 'PRINT_MINDATA') THEN
         PRINT_MINDATA = .TRUE.
         IF(NITEMS.GT.1) CALL READF(HESS_EIGEN_TOL)
...
!
      ELSE IF (WORD.EQ.'UPDATES') THEN
         CALL READI(MUPDATE)
!
!  Using SQNM Minimization algorithm jk669
!
      ELSE IF (WORD .EQ. 'SQNM') THEN
         SQNMT=.TRUE.
         IF(NITEMS .GE. 1) CALL READI(SQNM_HISTMAX) !history read length
         J1=2
         IF(NITEMS .LE. 1) GOTO 351
350      CALL READA(SQNM_DEBUG_CHECK)
         J1=J1+1
        SQNM_DEBUG_CHECK=TRIM(ADJUSTL(SQNM_DEBUG_CHECK))
         SELECT CASE(SQNM_DEBUG_CHECK)
            CASE('DEBUG')
               SQNM_DEBUGT=.TRUE. !debug on
            CASE('RUN','RUNS', 'QUENCH') !which quench to debug
               CALL READI(SQNM_DEBUGRUN)
               J1=J1+1 !increment since extra item is read
            CASE('LEVEL','DEBUGLEVEL') !amount of text that will be output as debugging info.
               CALL READI(SQNM_DEBUGLEVEL)
               J1=J1+1
            CASE('WRITEMAX','MAXWRITE') !length of arrays that will be written out. 
               CALL READI(SQNM_WRITEMAX)
               J1=J1+1 
            CASE('BIOOFF','BIOF','BIOFF','BIOFALSE') !Turns off the bio portion of the sqnm algorithm, even if AMBER is used
               SQNM_BIOT=.FALSE.
         END SELECT
         IF (J1<NITEMS) GOTO 350
351      CONTINUE
!
...
!         CLOSE(626)
      ...
      ELSE IF (WORD.EQ.'MBPOL') THEN
         ...
         TYPECH(2::3)(1:1)="H"
         TYPECH(3::3)(1:1)="H"
      ELSE IF (WORD.EQ.'RIGIDMBPOL') THEN
         CALL MBPOLINIT
         RIGIDMBPOLT=.TRUE.
         RIGID= .TRUE.
         MOLECULART=.TRUE.
...
! cs675>
      ELSE IF (WORD.EQ.'ORBITALS') THEN
         ...
         IF (NROTS.NE.NATOMS) THEN
            WRITE(MYUNIT,'(A,2I8)') 'keywords> ERROR *** NATOMS,NROTS=',NATOMS,NROTS
            STOP
         ENDIF
      ELSE
