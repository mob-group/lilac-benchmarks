...
SUBROUTINE TRANSFORMRIGIDTOC (CMIN, CMAX, XCOORDS, XRIGIDCOORDS)
  ...
  USE COMMONS, ONLY: NATOMS, ORTHO, BOXDERIVT
  USE CARTDIST
  IMPLICIT NONE
  ...
  DOUBLE PRECISION :: XRFRAC(3*NATOMS), H(3,3), H_GRAD(3,3,6) ! dj337: fractional rigid body coords
  ...
  ! dj337: convert rigidbody coords from fractional to absolute
  if (boxderivt.and.rigidinit) then
     xrfrac(1:degfreedoms) = xrigidcoords(1:degfreedoms)
     if (ortho) then
        call frac2cart_rb_ortho(nrigidbody, xrigidcoords, xrfrac)
     else
        call build_H(H, H_grad, .false.)
        call frac2cart_rb_tri(nrigidbody, xrigidcoords, xrfrac, H)
     endif
  endif
...
! vr274 > this copies lattice coordinates as well which is stored in last 2 atoms
  ...
  ! dj337: restore rigid body coords as fractional
  if (boxderivt.and.rigidinit) xrigidcoords(1:degfreedoms) = xrfrac(1:degfreedoms)
...
SUBROUTINE TRANSFORMCTORIGID (XCOORDS, XRIGIDCOORDS)
  USE COMMONS, ONLY: NATOMS, PERMDIST, MYUNIT, ORTHO, BOXDERIVT
  USE VEC3
  USE ROTATIONS
  USE CARTDIST
  IMPLICIT NONE
  ...
  DOUBLE PRECISION :: XRIGIDCOORDS(DEGFREEDOMS), XCOORDS(3*NATOMS)
  DOUBLE PRECISION :: XRFRAC(3*NATOMS) ! dj337: fractional rigidbody coords
  DOUBLE PRECISION :: H(3,3), H_GRAD(3,3,6), H_INV(3,3) ! dj337: for fractional rb
...
! vr274 > copy lattice coords
  ...
  ! dj337: if computing box derivatives, convert rb coords to fractional
  if (boxderivt.and.rigidinit) then
     if (ortho) then
        call cart2frac_rb_ortho(nrigidbody, xrigidcoords, xrfrac)
     else
        call build_H(H, H_grad, .false.)
        call inversematrix(H, H_inv)
        call cart2frac_rb_tri(nrigidbody, xrigidcoords, xrfrac, H_inv)
     endif
     xrigidcoords(1:degfreedoms) = xrfrac(1:degfreedoms)
  endif
...
SUBROUTINE AACONVERGENCE (G, XR, GR, RMS)
  ...
  DOUBLE PRECISION :: DR1(3),DR2(3),DR3(3), RMI3(3,3)
  double precision, intent(out) :: RMS 

  !print *, 'here in aaconv'
  ...
  DO J1 = 1, NRIGIDBODY
     ...
     ENDDO
     !print *, j1, 'torque1: ', torque(:3) 
     TORQUE = MATMUL(TRANSPOSE(RMI), TORQUE)
     !print *, j1, 'torque2: ', torque(:3)
     !print *, 'index: ', j1
     !print *, 'rmi: ', rmi(:3,:3)
     !print *, 'inertia: ', transpose(iinverse(j1,:3,:3))
     ...
     !print *, 'j1: ', j1, 1.0d0/nsiteperbody(j1)
     !print *, 'grad: ', gr(3*j1-2:3*j1)
     !print *, 'dot product: ', dot_product(gr(3*j1-2:3*j1), gr(3*j1-2:3*j1))
     !print *, 'rms: ', rms
  ENDDO

!  IF (DEGFREEDOMS > 6 * NRIGIDBODY) THEN
!     print *, 'help -- in the extra loop!'
!     DO J1 = 1, (DEGFREEDOMS - 6*NRIGIDBODY)/3
...
! -------------------------------------------------------------
! dj337: second AACONVERGENCE subroutine for systems that do not have the gradient
! in Cartesian coordinates.

subroutine aaconvergence2(xr, gr, rms)

  implicit none

  integer                       :: j1
  double precision, intent(in)  :: xr(degfreedoms), gr(degfreedoms)
  double precision              :: pi(3), rmi(3,3), drmi1(3,3), drmi2(3,3), drmi3(3,3)
  double precision              :: torque(3), mat1(3,3), mat2(3,3), mat3(3,3)
  double precision, intent(out) :: rms
  ...
  do j1 = 1, nrigidbody
     ...
     ! compute RMS contribution due to translational degrees of freedom
     rms = rms + 1.0d0/nsiteperbody(j1) * dot_product(gr(3*j1-2:3*j1), gr(3*j1-2:3*j1))
  
     ! compute RMS contribution due to rotational degrees of freedom
     ! convert AA gradient to instantaneous frame
     torque(:) = 0.0d0
     mat1(:,:) = 0.0d0; mat2(:,:) = 0.0d0; mat3(:,:) = 0.0d0
     pi = xr(3*nrigidbody+3*j1-2:3*nrigidbody+3*j1)
     call rmdrvt(pi, rmi, drmi1, drmi2, drmi3, .true.)
 
     mat1 = matmul(drmi1, transpose(rmi))
     !print *, 'mat1: ', mat(:3,:3)
     torque(1) = torque(1) + gr(3*nrigidbody+3*j1-2)*mat1(3,2)
     torque(2) = torque(2) + gr(3*nrigidbody+3*j1-2)*mat1(1,3)
     torque(3) = torque(3) + gr(3*nrigidbody+3*j1-2)*mat1(2,1)
  
     mat2 = matmul(drmi2, transpose(rmi))
     !print *, 'mat2: ', mat(:3,:3)
     torque(1) = torque(1) + gr(3*nrigidbody+3*j1-1)*mat2(3,2)
     torque(2) = torque(2) + gr(3*nrigidbody+3*j1-1)*mat2(1,3)
     torque(3) = torque(3) + gr(3*nrigidbody+3*j1-1)*mat2(2,1)
  
     mat3 = matmul(drmi3, transpose(rmi))
     !print *, 'mat3: ', mat(:3,:3)
     torque(1) = torque(1) + gr(3*nrigidbody+3*j1)*mat3(3,2)
     torque(2) = torque(2) + gr(3*nrigidbody+3*j1)*mat3(1,3)
     torque(3) = torque(3) + gr(3*nrigidbody+3*j1)*mat3(2,1)

     !print *, 'index: ', j1
     !print *, 'rmi: ', rmi(:3,:3)
     !print *, 'inertia: ', iinverse(j1,:3,:3)
     torque = matmul(transpose(rmi), torque)
     !print *, j1, 'grad: ', gr(3*nrigidbody+3*j1-2:3*nrigidbody+3*j1)
     !print *, j1, 'torque: ', torque(:3) 
     rms = rms + dot_product(torque, matmul(iinverse(j1, :, :), torque))
  
  enddo
  
  rms = max(dsqrt(rms/(degfreedoms)), 1.0d-100)

end subroutine aaconvergence2 
