...
DISTANCE = 0.0D0
RMAT=0.0D0
!cv320: commented out this write statement, looks like it was just for debugging.
! WRITE(*,*) 'RMAT', RMAT
! sf344 temporary debug
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!  WRITE(MYUNIT,'(A)') 'A initial'
!  DO J3=1,NATOMS
!      WRITE(MYUNIT,'(A2,2X,3F20.10)') 'LA',COORDSA(3*(J3-1)+1),COORDSA(3*(J3-1)+2),COORDSA(3*(J3-1)+3)
!  ENDDO
!  WRITE(MYUNIT,'(A)') 'B initial'
!  DO J3=1,NATOMS
!      WRITE(MYUNIT,'(A2,2X,3F20.10)') 'LA',COORDSB(3*(J3-1)+1),COORDSB(3*(J3-1)+2),COORDSB(3*(J3-1)+3)
!  ENDDO
...
ELSE
   ...
   RMAT(1,1)=1.0D0; RMAT(2,2)=1.0D0; RMAT(3,3)=1.0D0
   CMX=0.0D0; CMY=0.0D0; CMZ=0.0D0
   IF ((.NOT.MKTRAPT).AND.(NFREEZE.EQ.0)) THEN
      DO I=1,NATOMS
