...
C
      ...
      COMMON /MYPOT/ POTEL
      LOGICAL GUIDECHANGET, GUIDET, CSMDOGUIDET,PASS
...
! jwrm2> No compression for percolate in final quenches
      IF(PERCOLATET) K_COMP = 0.D0
...
! csw34> Adding in clarification of what is being tightly quenched
      ...
      DO J1=1,NSAVE
         IF (QMIN(J1).LT.1.0D10) THEN
            ...
            ENDDO
            ! dj337: update box parameters with saved ones
            if (boxderivt) box_params(1:6) = boxq(j1,1:6)
            
            !ds656>
            !write(MYUNIT,*) (QMINT(J1,J2),J2=1,NATOMS)
            IF(NSPECIES(0)>1) THEN
               CALL SET_ATOMLISTS(QMINT(J1,1:NATOMS),1)
               CALL SET_LABELS(QMINT(J1,1:NATOMS),NP)
            ENDIF
            !<ds656
...
! hk286
            ...
            IF(AMBER12T) THEN
               IF (CHECKCHIRALITY) THEN
                  PASS=.TRUE.
                  IF (SETCHIRAL) THEN
                     CALL CHIRALITY_CHECK(SCREENC, PASS)
                     IF (.NOT. PASS) THEN
                        WRITE(MYUNIT,*) ' finalq> WARNING: chirality differs from initial structure!'
                     ENDIF
                  ENDIF
               ENDIF
               IF (NOCISTRANS) THEN
                  PASS=.TRUE.
                  CALL CIS_TRANS_CHECK(SCREENC, PASS)
                  IF (.NOT. PASS) THEN
                     WRITE(MYUNIT,*) ' finalq> WARNING: cis/trans differs from initial structure!'
                  END IF
               ENDIF
            ENDIF
...
!            ENDIF
            ...
            ! dj337: save box parameters from end of quench
            if (boxderivt) boxq(j1,1:6) = box_params(1:6)
