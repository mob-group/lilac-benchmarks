...
USE COMMONS, ONLY : INTCONSEP, NREPMAX, NREPULSIVE, CONDISTREF, REPCON, INTCONSTRAINTREP, &
  ...
  & NATOMS,DEBUG,BOXLX,BOXLY,BOXLZ,MYUNIT, FASTOVERLAPT, BNB_ALIGNT
...
WRITE(MYUNIT,'(A,I6,2L5,I6)') 'make_conpot> NCONGEOM,CALLED,CONDATT,NCPFIT=',NCONGEOM,CALLED,CONDATT,NCPFIT
...
!
   IF ((CALLED.OR.CONDATT).AND.(NCPFIT.EQ.2)) THEN
      IF (FASTOVERLAPT .OR. BNB_ALIGNT) THEN
         WRITE(*,*) "make_conpot> Warning: this subroutine has not been tested with the new alignment routines."
         WRITE(*,*) "If they are not suitable for your use case, please change make_conpot to call MINPERMDIST instead of ALIGN_DECIDE"
      ENDIF
      CALL ALIGN_DECIDE(CONGEOM(1,1:3*NATOMS),MINCOORDS(1,1:3*NATOMS),NATOMS,DEBUG, &
  &                    BOXLX,BOXLY,BOXLZ,PERIODIC,TWOD,D,D2,RIGID,RMAT)
      CALL ALIGN_DECIDE(CONGEOM(1,1:3*NATOMS),MINCOORDS(2,1:3*NATOMS),NATOMS,DEBUG, &
  &                    BOXLX,BOXLY,BOXLZ,PERIODIC,TWOD,D,D2,RIGID,RMAT)
      ! sn402: could this be changed to ALIGN_DECIDE as well?
...
!           WRITE(MYUNIT,'(A,I6,6G20.10)') 'J1,DF,INTFREEZETOL=',J1,DF,INTFREEZETOL
!        ENDIF
...
CONCUTLOCAL(1:NCONSTRAINT)=CONCUT(1:NCONSTRAINT)
NREPULSIVEFIX=0
WRITE(MYUNIT,*) 'make_conpot> INTCONSTRAINTREP,NREPULSIVEFIX=',INTCONSTRAINTREP,NREPULSIVEFIX
...
USE COMMONS, ONLY : NREPMAX, NREPULSIVE, CONDISTREF, INTCONSTRAINTDEL, CONCUT, CONCUTLOCAL, &
  ...
  & CONCUTABST, CONCUTABS, CONCUTFRAC, CONCUTFRACT, MYUNIT
...
!  INTCONST=NREPCUT(J2)**13
   ...
   R2AX=COORDS2(NI+1); R2AY=COORDS2(NI+2); R2AZ=COORDS2(NI+3)
   R2BX=COORDS2(NJ+1); R2BY=COORDS2(NJ+2); R2BZ=COORDS2(NJ+3)
   WRITE(MYUNIT,'(A,5I6)') 'make_conpot> J2,NI,NJ,NREPI,NREPJ=',J2,NI,NJ,NREPI(J2),NREPJ(J2)
