...
implicit none
   WRITE(MYUNIT,*) 'initialising variables for PY',NATOMS/2,' ellipsoids'
! allocate arrays
...
! plus extra LJ site
!
! This version is addressable with interactions scaled by the input epsilon matrix PYADDEPS
!
      SUBROUTINE PYGPERIODICADD (X, G, ENERGY, GTEST)

       use commons, only: BOXLX,BOXLY,BOXLZ,PARAMONOVPBCX,PARAMONOVPBCY,PARAMONOVPBCZ,PCUTOFF,PARAMONOVCUTOFF,&
                &         natoms,pysignot,pyepsnot,radift,LJSITE,& 
                &         MAXINTERACTIONS,VT,&
                &         FROZEN, PYADDEPS, NADDTARGET
!                &         PSIGMAATTR, PEPSILONATTR, MYUNIT, PYBINARYTYPE1, PYBINARYT,&
!                &         PSCALEFAC2, PSCALEFAC1, PEPSILON1, BLJSITE, PYA2BIN, PYA1BIN
...
123 CONTINUE 
             ...
             END IF ! IF(LJSITE)
               IF (MJ1.EQ.MJ2) THEN ! same address - take just repulsion?
                   VDUMMY = VDUMMY + 4.0D0 * PYEPSNOT * RHO112*PYADDEPS(MJ2,MJ1)
               ELSE
                   VDUMMY = VDUMMY + 4.0D0 * PYEPSNOT * (RHO112 - 1.0D0 * RHO26)*PYADDEPS(MJ2,MJ1)
               ENDIF
...
!     CALCULATE GRADIENT
           ...
           END IF !if LJSITE 
               IF (MJ1.EQ.MJ2) THEN ! same address - take just repulsion?
                   FIJ    = FIJ + 24.0D0*PYEPSNOT*2.D0*RHO112*RHO1*DG1DR*PYADDEPS(MJ2,MJ1)
                   TIJ(1) = DVDF1*DF1PI1 
                   TIJ(2) = DVDF1*DF1PI2 
                   TIJ(3) = DVDF1*DF1PI3 
                   TJI(1) = DVDF1*DF1PJ1 
                   TJI(2) = DVDF1*DF1PJ2 
                   TJI(3) = DVDF1*DF1PJ3 
                   TIJ = dVDUMMY(7:9) + 24.0D0 * PYEPSNOT * TIJ*PYADDEPS(MJ2,MJ1)
                   TJI = dVDUMMY(10:12) + 24.0D0 * PYEPSNOT * TJI*PYADDEPS(MJ2,MJ1)
               ELSE
                   FIJ    = FIJ + 24.0D0*PYEPSNOT*(2.D0*RHO112*RHO1*DG1DR - 1.0D0 * RHO26*RHO2*DG2DR)*PYADDEPS(MJ2,MJ1)
                   TIJ(1) = DVDF1*DF1PI1 + 1.0D0* DVDF2*DF2PI1
                   TIJ(2) = DVDF1*DF1PI2 + 1.0D0*  DVDF2*DF2PI2
                   TIJ(3) = DVDF1*DF1PI3 + 1.0D0*  DVDF2*DF2PI3
                   TJI(1) = DVDF1*DF1PJ1 + 1.0D0*  DVDF2*DF2PJ1
...
! This version is addressable with interactions scaled by the input epsilon matrix PYADDEPS
!
      SUBROUTINE PYGPERIODICADD2 (X, G, ENERGY, GTEST)

       use commons, only: BOXLX,BOXLY,BOXLZ,PARAMONOVPBCX,PARAMONOVPBCY,PARAMONOVPBCZ,PCUTOFF,PARAMONOVCUTOFF,&
                &         natoms,pysignot,pyepsnot,radift,LJSITE,& 
                &         MAXINTERACTIONS,VT,&
                &         FROZEN, PYADDREP, PYADDATT, NADDTARGET
!                &         PSIGMAATTR, PEPSILONATTR, MYUNIT, PYBINARYTYPE1, PYBINARYT,&
!                &         PSCALEFAC2, PSCALEFAC1, PEPSILON1, BLJSITE, PYA2BIN, PYA1BIN
...
123 CONTINUE 
             ...
             END IF ! IF(LJSITE)
                VDUMMY = VDUMMY + 4.0D0 * PYEPSNOT * (RHO112*PYADDREP(MJ2,MJ1) - RHO26*PYADDATT(MJ2,MJ1))
...
!     CALCULATE GRADIENT
           ...
           END IF !if LJSITE 
                   FIJ    = FIJ + 24.0D0*PYEPSNOT*(2.D0*RHO112*RHO1*DG1DR*PYADDREP(MJ2,MJ1) - 1.0D0 * RHO26*RHO2*DG2DR*PYADDATT(MJ2,MJ1))
                   TIJ(1) = DVDF1*DF1PI1*PYADDREP(MJ2,MJ1) + 1.0D0* DVDF2*DF2PI1*PYADDATT(MJ2,MJ1)
                   TIJ(2) = DVDF1*DF1PI2*PYADDREP(MJ2,MJ1) + 1.0D0*  DVDF2*DF2PI2*PYADDATT(MJ2,MJ1)
                   TIJ(3) = DVDF1*DF1PI3*PYADDREP(MJ2,MJ1) + 1.0D0*  DVDF2*DF2PI3*PYADDATT(MJ2,MJ1)
                   TJI(1) = DVDF1*DF1PJ1*PYADDREP(MJ2,MJ1) + 1.0D0*  DVDF2*DF2PJ1*PYADDATT(MJ2,MJ1)
                   TJI(2) = DVDF1*DF1PJ2*PYADDREP(MJ2,MJ1) + 1.0D0*  DVDF2*DF2PJ2*PYADDATT(MJ2,MJ1)
                   TJI(3) = DVDF1*DF1PJ3*PYADDREP(MJ2,MJ1) + 1.0D0*  DVDF2*DF2PJ3*PYADDATT(MJ2,MJ1)
                   TIJ = dVDUMMY(7:9) + 24.0D0 * PYEPSNOT * TIJ
                   TJI = dVDUMMY(10:12) + 24.0D0 * PYEPSNOT * TJI
...
!     END OUTER LOOP OVER PARTICLES
        ...
        END DO
        RETURN
      END SUBROUTINE PYGPERIODICADD2
...
!     THIS ROUTINE TAKES STEP FOR SINGLE-SITE ELLIPSOIDAL BODIES ENSURING NO OVERLAP
      ...
      use modamber9, only : ligtransstep, ligrotscale
      use rotations
...
!      INTEGER          :: PTINDX, I, J, K
      ...
      DOUBLE PRECISION :: PI, DUMMY, DUMMY2, SAVECOORDS(3*NATOMS), Y(3*NATOMS)
      ...
      DOUBLE PRECISION :: P1(3), P2(3), ABSRIJ, RCUT, XMIN, FMIN, ECFVAL, ligandcoords(3), ligandcoordsrotated(3)
! sf344 additions
      LOGICAL          :: DISSOCIATED(NATOMS/2),DISSOC
      INTEGER          :: CLOSESTATOMINDEX, K1, K2, i,j, k
      ...
      double precision :: st,ct,sph,cph,sps,cosps, randomx, randomy, randomz, randomphi, randomtheta, randompsi
...
! do the group moves if PERCGROUPT=.TRUE.
   IF(PERCGROUPT) THEN
        nblocks=atomingroup(REALNATOMS) !the last number in the atomingroup array is the total number of groups as well
        WRITE(MYUNIT,'(A,I6,A)') 'takestepelpsd> taking cooperative rotational and translational steps for ', nblocks, ' blocks' 
        if(allocated(rotationmatrix)) deallocate(rotationmatrix)
        if(allocated(atomblockcentre)) deallocate(atomblockcentre)
        allocate(atomblockcentre(nblocks,3),rotationmatrix(nblocks,3,3))
!WRITE(MYUNIT,*) 'before rotation and translation:'
!WRITE(MYUNIT,*) y(:)
OVRLPT=.TRUE.
94  DO WHILE (OVRLPT)

     y(:)=COORDS(:,NP)
! random rotation matrices for each group
     IF(ligrotscale>0.0D0) then
       DO i=1,nblocks  
        randomphi=(DPRAND()-0.5)*twopi*ligrotscale
        randomtheta=(DPRAND()-0.5)*pi*ligrotscale
        randompsi=(DPRAND()-0.5)*twopi*ligrotscale
        st=sin(randomtheta)
        ct=cos(randomtheta)
        sph=sin(randomphi)
        cph=cos(randomphi)
        ...
        rotationmatrix(i,1,1)=cosps*cph-ct*sph*sps
        rotationmatrix(i,2,1)=cosps*sph+ct*cph*sps
        rotationmatrix(i,3,1)=sps*st
        rotationmatrix(i,1,2)=-sps*cph-ct*sph*cosps
        rotationmatrix(i,2,2)=-sps*sph+ct*cph*cosps
        rotationmatrix(i,3,2)=cosps*st
       ...
       END DO
          ...
          do i=1,nblocks
             j=0
             do k=1,REALNATOMS
               if(i==atomingroup(k)) then
                j=j+1 ! counter of atoms in each block
                atomblockcentre(i,1)=atomblockcentre(i,1)+y(3*k-2)
                atomblockcentre(i,2)=atomblockcentre(i,2)+y(3*k-1)
                atomblockcentre(i,3)=atomblockcentre(i,3)+y(3*k  )
               end if
             end do
!                write(MYUNIT,*) 'rotation matrix:', rotationmatrix(i,:,:)
                atomblockcentre(i,:)=atomblockcentre(i,:)/j
          end do
          do i=1,nblocks
             do k=1,REALNATOMS
               if(i==atomingroup(k)) then
...
! move each block to the origin
                ligandcoords(1)=y(3*k-2)-atomblockcentre(i,1)
                ligandcoords(2)=y(3*k-1)-atomblockcentre(i,2)
                ligandcoords(3)=y(3*k  )-atomblockcentre(i,3)
! rotate the block of atoms with random rotation matrix
                ligandcoordsrotated=MATMUL(rotationmatrix(i,:,:),ligandcoords)
!                WRITE(MYUNIT,*) 'ligandcoordsrotated: ', i, k, ligandcoordsrotated(:)
!                WRITE(MYUNIT,*) 'atomblockcentre:', atomblockcentre(i,:)
! translate back the block of atoms to the original centre of their coordinates
                y(3*k-2)=ligandcoordsrotated(1)+atomblockcentre(i,1)
                y(3*k-1)=ligandcoordsrotated(2)+atomblockcentre(i,2)
                y(3*k  )=ligandcoordsrotated(3)+atomblockcentre(i,3)
                y(3*k+3*REALNATOMS-2:3*k+3*REALNATOMS)=rot_rotate_aa(y(3*k+3*REALNATOMS-2:3*k+3*REALNATOMS),rot_mx2aa(rotationmatrix(i,:,:)))
               end if
             end do
          end do
!                WRITE(MYUNIT,*) 'after rotation:'
!                WRITE(MYUNIT,*) y(:)
     END IF !ligrotscale
! translate each group
        LOCALSTEP=1.0D0
           ...
           do i=1,nblocks
             ...
             do k=1,REALNATOMS
              if(i==atomingroup(k)) then
                 y(3*k-2)=y(3*k-2)+2*randomx*ligtransstep*LOCALSTEP-ligtransstep*LOCALSTEP
                 y(3*k-1)=y(3*k-1)+2*randomy*ligtransstep*LOCALSTEP-ligtransstep*LOCALSTEP
                 y(3*k  )=y(3*k  )+2*randomz*ligtransstep*LOCALSTEP-ligtransstep*LOCALSTEP
              end if
             end do
           end do
...
! allow for a slight overlap 
                  IF (ECFVAL < PYOVERLAPTHRESH) THEN
                     IF(DEBUG) WRITE(MYUNIT,*) 'takestepelpsd> PERCGROUP - particles overlapping', J1, J2, ECFVAL, ABSRIJ
                     OVRLPT = .TRUE.
                     GO TO 94
                  ENDIF
    ...
    END DO ! DO WHILE(OVERLAPT)
   END IF ! PERCGROUPT
!WRITE(MYUNIT,*) 'rotated and translated coordinates:'
!WRITE(MYUNIT,*) y(:)
...
! allow for a slight overlap 
                  IF (ECFVAL < PYOVERLAPTHRESH) THEN
                        WRITE(MYUNIT,*) 'takestepelpsd> particles overlapping', J1, J2, ECFVAL, ABSRIJ
                     OVRLPT = .TRUE.
                     GO TO 95
                  ENDIF
...
!         END IF
!        END DO
!        WRITE(MYUNIT,'(A,I6,A,F10.6,I6,F10.6)') 'Atom ', J1, ' is dissociated, distance to the closest atom: ', &
!        & MINDISTANCE, CLOSESTATOMINDEX, RCUT
!        DISSOC=.TRUE.
!        ! move the dissociated atom closer to the closest atom
!        COORDS(J2-2,NP)=0.5D0*(COORDS(3*CLOSESTATOMINDEX-2,NP)+COORDS(J2-2,NP))
!        COORDS(J2-1,NP)=0.5D0*(COORDS(3*CLOSESTATOMINDEX-1,NP)+COORDS(J2-1,NP))
...
!     ----------------------------------------------------------------------------------------------
      ...
      SUBROUTINE TAKESTEPPERCGROUPRESEED (NP)
...
! do the group moves if PERCGROUPT=.TRUE.
   IF(PERCGROUPT) THEN
        nblocks=atomingroup(REALNATOMS) !the last number in the atomingroup array is the total number of groups as well
        WRITE(MYUNIT,'(A,I6,A)') 'takesteppercgroupreseed> reseeding block centres from coords.reseed, and doing random rotation for ', nblocks, ' blocks' 
        if(allocated(rotationmatrix)) deallocate(rotationmatrix)
        if(allocated(atomblockcentre)) deallocate(atomblockcentre)
        allocate(atomblockcentre(nblocks,3),rotationmatrix(nblocks,3,3))
!WRITE(MYUNIT,*) 'before rotation and translation:'
...
!                write(MYUNIT,*) 'rotation matrix:', rotationmatrix(i,:,:)
                atomblockcentre(i,:)=atomblockcentre(i,:)/j
                atomblockcentre(i,:)=atomblockcentre(i,:)
          end do
          do i=1,nblocks
             do k=1,REALNATOMS
               if(i==atomingroup(k)) then
...
! rotate the block of atoms with random rotation matrix
                ligandcoordsrotated=MATMUL(rotationmatrix(i,:,:),ligandcoords)
!                WRITE(MYUNIT,*) 'ligandcoordsrotated: ', i, k, ligandcoordsrotated(:)
!                WRITE(MYUNIT,*) 'reseedcoords:', reseedcoords(:)
! translate back the block of atoms to the original centre of their coordinates
                y(3*k-2)=ligandcoordsrotated(1)+reseedcoords(3*i-2)*PERCGROUPSF
                y(3*k-1)=ligandcoordsrotated(2)+reseedcoords(3*i-1)*PERCGROUPSF
                y(3*k  )=ligandcoordsrotated(3)+reseedcoords(3*i  )*PERCGROUPSF
                y(3*k+3*REALNATOMS-2:3*k+3*REALNATOMS)=rot_rotate_aa(y(3*k+3*REALNATOMS-2:3*k+3*REALNATOMS),rot_mx2aa(rotationmatrix(i,:,:)))
               end if
             end do
          end do
...
! allow for a slight overlap 
                  IF (ECFVAL < PYOVERLAPTHRESH) THEN
                   OVRLPT = .TRUE.
                   PERCGROUPSF=PERCGROUPSF*1.1D0
                   IF(DEBUG) WRITE(MYUNIT,*) 'takestepelpsd> PERCGROUP - particles overlapping, scaling up the reseeded centres'
                   IF(DEBUG) WRITE(MYUNIT,*) 'takestepelpsd> scale factor, J1, J2, ECF', PERCGROUPSF,J1, J2, ECFVAL
                   GO TO 94
                  ENDIF
...
RETURN
      ...
      END SUBROUTINE TAKESTEPPERCGROUPRESEED
...
SUBROUTINE GENERATECGDIMER
! this will be useful if we have a reference dimer and would like to get the
! coarse-grained coordinates for it (virus capsid protein dimers, for example),
! by starting from the angle-axis information about the rigid monomer containing
! an arbitrary number of ellipsoids. For this, we need as input the coordinate centres for both units,
! and the rotation matrix describing the rotation of the first unit to the second one. Output 
! will be the angle-axis coordinates of the second unit.
! Individual orientations of ellipsoids should go in the pysites.xyz file.
use rotations
implicit none
integer i,j,k, realnatoms, nlines
double precision :: y(12),atomblockcentre(2,3), rotationmatrix(3,3)

open(255,file='coords',status='old')

call determinelines(255,nlines)
if(nlines>4) write(*,*) 'ERROR - we should be generating a rigid body dimer, so we need 4 coordinates in the coords file'

do i=1,nlines
   read(255,*) y(3*i-2), y(3*i-1), y(3*i)
end do

close(255)

open(256,file='rotationmatrix',status='old')
 do j=1,3
   read(256,*) rotationmatrix(1:3,j)
 end do
 write(*,*) 'read rotation matrix:'
 write(*,*) rotationmatrix(:,:)
 realnatoms=nlines/2
    y(4:6)=MATMUL(y(1:3),rotationmatrix)+y(4:6) 
    y(3*2+3*REALNATOMS-2:3*2+3*REALNATOMS)=rot_rotate_aa(y(3*2+3*REALNATOMS-2:3*2+3*REALNATOMS),rot_mx2aa(rotationmatrix(:,:)))
write(*,*) 'finished generating dimer coordinates'
do i=1,nlines
   write(*,*) y(3*i-2), y(3*i-1), y(3*i)
end do
...
END SUBROUTINE GENERATECGDIMER
