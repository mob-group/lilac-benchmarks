...
!
     ...
     &   LJPSHIFT_UPDATE_E_AA_offset, LJPSHIFT_UPDATE_E_BB_offset, &
         ...
         SUBROUTINE UPDATE_POTENTIAL_GRADIENT( X, J1, J2, POTEL, V, T )
            ...
            TYPE(INTERACTION_DEF), INTENT(IN) ::  T
            DOUBLE PRECISION, INTENT(IN) :: X(3*N)
            DOUBLE PRECISION, INTENT(INOUT) :: POTEL, V(3*N)
            INTEGER, INTENT(IN) :: J1, J2
         ...
         SUBROUTINE LJPSHIFT_CLASS_SETUP(CUTOFF, EPSAA, EPSBB, EPSAB, SIGAA, SIGBB, SIGAB, NATOMS, BOXLX_I, BOXLY_I, BOXLZ_I)
            ...
            !print *, 'line 238'
            !print *, 'cutoff: ', cutoff
            !print *, 'eps: ', epsaa, epsbb, epsab
            !print *, 'sig: ', sigaa, sigbb, sigab
            !print *, 'natoms: ', natoms
            !print *, 'boxlengths: ', boxlx_i, boxly_i, boxlz_i
         ...
         !this module defines the potential LJPSHIFT! IMPLICIT NONE
...
!*************************************************************************
     ...
     &    ONE_ATOM_TAKESTEP
      use genrigid, only: atomrigidcoordt, nrigidbody, transformrigidtoc, transformctorigid
      USE LJPSHIFT_CLASS
      USE FREEZE_NL_MOD
      ...
      double precision :: oldpotel, xr(3*natoms)
      ...
      !print *, 'master coords: ', x(:3*natoms)
...
!
      ...
      !IF (.not.(ATOMRIGIDCOORDT)) THEN ! if input is cartesian
...
NAA, BBLIST, NBB, ABLIST, NAB, EOFFSET)
   ...
   !print *, 'aalist1: ', aalist(1, :naa)
   !print *, 'aalist2: ', aalist(2, :naa)

   !print *, 'intl potel: ', potel
