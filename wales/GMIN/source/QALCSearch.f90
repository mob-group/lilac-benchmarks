...
SUBROUTINE QALCS(NP,ITER,TIME,BRUN,QDONE,SCREENC)
  ...
  CALL MYCPU_TIME(TIME)
  WRITE(MYUNIT,'(A,F20.10,A,F11.1)') &
       'QALCS> Final E= ',POTEL,' t= ',TIME-TSTART
  !
  RETURN
  !
...
SUBROUTINE AB_SEARCH2(NP,ITER,TIME,BRUN,QDONE,SCREENC,LISTS_AB,I_AB)
  ...
  DO I=1, LISTS_AB(1,0)
     DO J=1, LISTS_AB(2,0)
        ...
        IF(QALCSMODE==3) THEN ! Sort list
           ...
           CALL SWAP_LABELS(LISTS_AB(1,I),LISTS_AB(2,J),NP)
  ...
  DO I=1,NTOT
     ...
     ENDIF
     !
     CALL SWAP_LABELS(SWAPS(J,1),SWAPS(J,2),NP)
     !                                                            
     NQTOT = NQTOT + 1
     NQ(NP) = NQ(NP) + 1
     CALL QUENCH(.FALSE.,NP,ITER,TIME,BRUN,QDONE,SCREENC)
...
SUBROUTINE AB_SEARCH(NP,ITER,TIME,BRUN,QDONE,SCREENC, & 
  ...
  IF(EBEST < E0 - ECONV) THEN
     SEQLENGTH = SEQLENGTH + 1
     CALL SWAP_LABELS( I_AB(1), I_AB(2), NP )
...
!=============================================================
!
SUBROUTINE SWAP_COORDS(I,J,NP)
  !
  USE COMMONS, ONLY : NATOMS, COORDS
  !
  IMPLICIT NONE
  !
...
!=============================================================
!
SUBROUTINE FLIP_LABEL(I,NEWTYPE,NP)
  !
  ...
  USE COMMONS, ONLY: ATOMLISTS, INVATOMLISTS, NSPECIES, ATMASS, &
       READMASST, SPECMASST, SPECMASS, LABELS
  !
  IMPLICIT NONE
  !
  INTEGER, INTENT(IN) :: I, NEWTYPE,NP
...
SUBROUTINE SPAN_SWAPS(NP,ITER,TIME,BRUN,QDONE,SCREENC,STEP)
  ...
  IF(STEP) THEN
     IF(NNEG > 0) THEN
        ...
        CALL SWAP_LABELS(BESTSWAP(1,1),BESTSWAP(1,2),NP)
...
SUBROUTINE SPAN_FLIPS(NP,ITER,TIME,BRUN,QDONE,SCREENC,STEP)
  ...
  IF(STEP) THEN
     IF(NNEG > 0) THEN
        ...
        POTEL = EMIN(1)
        COORDS(1:3*NATOMS, NP) = XMIN(1:3*NATOMS)
        CALL FLIP_LABEL(BESTFLIP(1,1),BESTFLIP(1,2),NP)
        WRITE(MYUNIT, '(A, G20.10)') &
...
SUBROUTINE SCAN_SWAP_NBRHD(NP,ITER,TIME,BRUN,QDONE,SCREENC,STEP)
  ...
  USE COMMONS, ONLY : NATOMS, COORDS, QALCSV, NQ, ECONV, &
       ...
       NSPECIES, QALCS_NBRHD, QALCS_PARAM, FIXLABELS
  !
  IMPLICIT NONE
  !
  ! Parsed variables
  ...
  DO I=1,NTOT
     ...
     !
     IF(FIXLABELS) THEN
        CALL SWAP_COORDS(SWAPS(J,1),SWAPS(J,2),NP)
     ELSE
        CALL SWAP_LABELS(SWAPS(J,1),SWAPS(J,2),NP)
     ENDIF
     !                                                            
...
SUBROUTINE SCAN_FLIP_NBRHD(NP,ITER,TIME,BRUN,QDONE,SCREENC,STEP)
  ...
  DO I=1,NTOT
     ...
     CALL FLIP_LABEL(FLIPS(J,1),TB,NP)
     !                                                            
     NQTOT = NQTOT + 1
     NQ(NP) = NQ(NP) + 1
     CALL QUENCH(.FALSE.,NP,ITER,TIME,BRUN,QDONE,SCREENC)
     ...
     ELSE 
        ...
        CALL FLIP_LABEL(FLIPS(J,1),TA,NP)
        COORDS(1:3*NATOMS, NP) = X0(1:3*NATOMS)
        POTEL = E0
        !
        IF(QALCSMODE==7) THEN
