...
subroutine amber12_setup(natoms, inpcrd_name_input, inpcrd_name_length) bind(C, name='amber12_setup')
!
...
! inpcrd_name_input(in): file name for the inpcrd file.
! inpcrd_name_length(in): length of the file name
!
  use iso_c_binding, only: c_int, c_char, c_null_char
  implicit none
! Arguments
  integer(c_int), intent(out)          :: natoms
  integer(c_int), intent(in)           :: inpcrd_name_length
  character(kind=c_char, len=1), &
            dimension(inpcrd_name_length), &
            intent(in)                 :: inpcrd_name_input  
! Variables  
...
subroutine amber12_get_atomdata(atoms,natoms)
  implicit none
  integer :: natoms
  type(amber12_atom), intent(out)     :: atoms(natoms)

  atoms = amber12_atoms
end subroutine amber12_get_atomdata

subroutine amber12_get_resdata(residues,nres)
  implicit none
  integer :: nres
  type(amber12_residue), intent(out)     :: residues(nres)

  residues = amber12_residues
end subroutine amber12_get_resdata

subroutine amber12_atomstores(atomstores,natoms)
  implicit none
  integer, intent(in) :: natoms
  integer, intent(out) :: atomstores(natoms)
  integer :: j1, nres
  
  nres = 1
  do j1=1,natoms
     if ((amber12_residues(nres)%end_index).ge.j1) then
        atomstores(j1) = nres
     else
        nres = nres + 1
        atomstores(j1) =nres
...
end subroutine amber12_atomstores
...
subroutine amber12_mut_finish() bind(C, name='amber12_mut_finish')
end subroutine amber12_mut_finish
...
subroutine amber12_write_restart_mut(coords, newatoms , rst_name, rst_name_length) bind(C, name='amber12_write_restart_mut')
!
! Writes a restart file containing the coordinates provided in the coords array.
...
! Arguments
  real(c_double), intent(in)              :: coords(1)
  integer(c_int), intent(in)              :: rst_name_length
  integer(c_int), intent(in)              :: newatoms
  character(kind=c_char, len=1), &
            dimension(rst_name_length), &
            intent(in)                    :: rst_name
end subroutine amber12_write_restart_mut
