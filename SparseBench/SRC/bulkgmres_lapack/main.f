...
C Local
      ...
      integer gmres_iwork,gmres_rwork,lrwork,gmres_qr_scale
      integer i
...
C allocate temps, and call method
     ...
     >        (restart+1)*(restart+1),rptr,rsize,
     >        'Temp tri')
         gmres_iwork = allocatei(imemory,
     >        restart+1,iptr,isize,'Lapack pivots')
         lrwork = (restart+1)*(restart+1)
         gmres_rwork = allocater(rmemory,
     >        lrwork,rptr,rsize,'Lapack work')
         gmres_qr_scale = allocater(rmemory,
     >        restart+1,rptr,rsize,'QR scaling')
     ...
     >        imemory(idx_ptr),nnz,imemory(ptr_ptr),imemory(dia_ptr),
     >        rmemory(gmres_qr_scale),
     >        imemory(gmres_iwork),rmemory(gmres_rwork),lrwork,
     >        t_iter)
      else
