...
C     Orthonormalise Krylov sequence; store coefficients in H
         t_ortho = starttimer()
         flops = 0
         call vec_hess(block(1,v),restart,len,H,u,ut,flops)
...
C
      subroutine vec_hess(v,wid,len,h,u,ut,flops)
      implicit none
C Arguments
...
C Invert V
      ...
      do it=wid+1,1,-1
         ut(it,it) = 1.d0/ut(it,it)
         do i=1,it-1
            ut(i,it) = -ut(i,it)*ut(it,it)
         end do
         do j=it+1,wid+1
            do i=1,it-1
               ut(i,j) = ut(i,j) + ut(i,it)*ut(it,j)
            end do
            ut(it,j) = ut(it,it)*ut(it,j)
         end do
      end do
