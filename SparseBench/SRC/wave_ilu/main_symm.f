...
C Tue Jan 23 13:30:08 EST 2001
C
C----------------------------------------------------------------
...
C
     ...
     >           3*(xside+1)*(yside+1)*(zside+1),
     >           rptr,rsize,'reg coefs')
            mat_ptr = allocater(rmemory,
     >           4*vecsize,rptr,rsize,
     >           '7point matrix')
     ...
     >           4*vecsize,rptr,rsize,
     >           '7point matrix')
            call rmove_left(rmemory,ptr_ptr,mat_ptr,4*vecsize)
         else
            mat_ptr = allocater(rmemory,
     >           4*vecsize,rptr,rsize,
     >           '7point matrix')
...
C
     ...
     >        vecsize,rptr,rsize,'preconditioner')
         if (structure.eq.1) then
            if (xside.ne.yside .or. xside.ne.zside) then
               print *,'Sorry, can only handle real cubes for this'
               print *,'form of regular ILU'
               stop
            end if
            fac_time = starttimer()
...
C Iterative method
C
      write(6,*) 'Conjugate Gradient method chosen.'
      method = 1
...
C allocate temps, and call method
     ...
     >     'cg temp space')
      call cg(structure,preconditioner,
...
c      call factor_flops()
 ...
 999  continue
      ...
      write(6,*) '**                                              **'
      write(6,*) '**  symmetric code                              **'
      write(6,*) '**  -- wavefront implementation of regular ilu  **'
      write(6,*) '**                                              **'
      write(6,*) '**************************************************'
      write(6,*) ' '
      return
